...
/**
 * A realistic upstream `agent-teams/*` event log for one team lifecycle,
 * shaped exactly like the `dsh-agent-teams` runtime emits (best-effort
 * session events; names instead of session ids in message/task payloads).
 *
 * Used by `src/e2e-replay.test.ts` as the deterministic replay fixture so the
 * whole pipeline (events → projection → view → insights → commands) can be
 * verified without a live DSH profile. The expected view state is documented
 * in `docs/verification-checklist.md`.
 */
export const upstreamTeamLifecycleEvents: SessionEvent[] = [
  // Open the team.
  {
  ...
  // working state (deletion is history-only, covered by upstream tests).
