...
function selectBestPracticesForRole(entries, role) {
	...
	return [...matched].sort((left, right) => verdictOrder[left.verdict] - verdictOrder[right.verdict] || right.updatedAt - left.updatedAt).slice(0, 3);
}
/** 校验一条经验条目形状(读盘边界)。 */
		...
		async execute(args, exec) {
			...
			const created = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (roleText !== "" && !isCommissarRole(roleText)) {
					const execCap = config.maxExecPerRole ?? 1;
