...
* @param stateRoot - resolved absolute state root directory.
*/
async function readProviderGrants(stateRoot) {
	try {
		const parsed = JSON.parse(stripLeadingBom(await readFile(join(stateRoot, PROVIDER_GRANTS_FILE), "utf8")));
		if (!Array.isArray(parsed) || parsed.some((value) => typeof value !== "string" || value === "")) throw new Error("invalid AgentTeams provider grant index");
		return new Set(parsed);
	} catch (error) {
		if (error instanceof Error && "code" in error && error.code === "ENOENT") return /* @__PURE__ */ new Set();
		throw error;
...
async function setProviderGrant(stateRoot, provider, enabled) {
	const normalized = provider.trim();
	if (normalized === "") throw new Error("provider must not be empty");
	if (normalized === "deepseek-official") return;
	await withTeamLock(`provider-grants:${stateRoot}`, async () => {
		...
		await atomicWriteText(join(stateRoot, PROVIDER_GRANTS_FILE), `${JSON.stringify([...grants].sort(), null, 2)}\n`);
	});
}
/**
...
*   pass the Host fence and present the boot token before any body is read.
*/
async function handleProviderGrant(stateDirName, workspaceRegistry, req, res, auth) {
	...
	const stateRoot = roots.length > 0 ? join(roots[0].path, stateDirName) : void 0;
	if (stateRoot === void 0) {
...
function wireSettingsGranted(settingsCtx, holder) {
	const scope = registerProviderGrantsSettings(settingsCtx);
	holder.providerGrantedFor = (provider) => grantedFromScope(scope, provider);
	const dispose = () => {
		holder.providerGrantedFor = void 0;
	};
