...
function registerAgentTeamsTools(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const roleDefaults = config.roleDefaultsFor?.(roleKey) ?? config.roleLlmDefaults?.[roleKey] ?? DEFAULT_ROLE_LLM[roleKey];
			...
			const explicitlyRouted = args.provider !== void 0 || roleDefaults !== void 0 && roleDefaults.provider !== void 0;
			let effectiveSelection = selection;
			if (explicitlyRouted && selection.provider !== "deepseek-official") {
				if (!(config.modelGrantedFor?.(selection.provider, selection.model) ?? selection.provider === "deepseek-official")) {
					...
					const fallbackModel = roleDefaults?.model ?? config.memberModel ?? captain.session.requestHeader()?.config?.model ?? captain.options.model;
					try {
