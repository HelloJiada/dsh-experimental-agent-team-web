...
function memberRouteForRequest(captain, request) {
	...
	const model = explicitModel ?? (roleProvider !== void 0 ? roleModel : void 0) ?? defaultModel ?? currentModel;
	...
	const reasoningEffort = explicitEffort === void 0 ? roleEffort !== void 0 && roleEffort !== "" ? roleEffort : provider === currentProvider && model === currentModel ? current?.reasoningEffort : void 0 : explicitEffort;
	return {
		provider,
		model,
...
async function resolveMemberLlmCandidates(ctx, captain, candidates, isGranted, signal) {
	const failures = [];
	for (const candidate of candidates) try {
		const route = memberRouteForRequest(captain, candidate.request);
		if (!isGranted(route.provider, route.model)) throw new Error(`model ${route.provider}/${route.model} is not authorized`);
...
function memberRouteCandidates(config, roleKey, explicit) {
	...
	for (const [label, role] of roleSources) {
		if (role?.provider === void 0 || role.model === void 0) continue;
		candidates.push({
			label,
			request: { roleDefaults: role }
		});
	}
	candidates.push({
		label: "captain route",
			...
			return withTeamLock(captainLockKey(stateRoot, captain.id), async () => {
				...
				return withTeamLock(teamLockKey$1(stateRoot, teamId), async () => {
					...
					if (mode !== "light") {
						...
						const commissarSelection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates(config, "commissar", {}), grant, exec.signal);
						commissar = {
							id: "",
							name: "政委",
							role: "commissar",
	...
	scopeCtx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (prepared.needsCommissar) {
				...
				const selection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates(config, "commissar", {}), grant, exec.signal);
				const draft = {
					id: "",
					name: "政委",
					role: "commissar",
...
function settingsAccessFromConfig(config) {
	...
	return {
		modelGrantedFor: (provider, model) => modelGrantedFromValue({ enabledModels: enabledModels() }, provider, model),
...
function wireAgentTeamSettings(settingsCtx, access) {
	...
	access.setModelGrant = async (provider, model, enabled) => {
		...
		await settings.update(String(AGENT_TEAM_SETTINGS_NS), { enabledModels: {
			...current,
			[key]: enabled
		} });
	};
...
async function listModelsSafe(ctx, providerId) {
	try {
		...
		return (await llm.listModels(providerId)).map((model) => model.id);
	} catch {
		return [];
	}
}
...
async function collectProviders(ctx, enabledModels) {
	...
	for (const provider of providers) {
		...
		const grantedAny = provider.id === "deepseek-official" || models.some((model) => enabledMap[`${provider.id}/${model}`] === true) || Object.keys(enabledMap).some((key) => key.startsWith(`${provider.id}/`) && enabledMap[key] === true);
		views.push({
			id: provider.id,
			name: provider.name,
			enabled: grantedAny,
