...
function effectiveCaptainSessionId(state) {
	if (state.captainSessionId !== void 0 && state.captainSessionId !== null) return state.captainSessionId;
	return state.teamId === null ? null : SessionId(state.teamId);
}
	...
	switch (event.type) {
		case "agent-teams/team-created": {
			...
			return {
				...
				captainSessionId: SessionId(data.captainSessionId),
				hasTeamEvents: true
			};
		}
		case "agent-teams/member-added": {
			...
			return {
				...state,
				teamId: state.teamId ?? TeamId(data.teamId),
				hasTeamEvents: true,
				members: {
					...state.members,
		...
		case "agent-teams/message-sent": {
			...
			const message = {
				id: data.messageId,
				senderId: memberIdForName(state, data.from) ?? captainSessionId ?? SessionId(data.teamId),
				senderName: data.from,
				targetId: memberIdForName(state, data.to) ?? captainSessionId ?? SessionId(data.teamId),
				delivery: "quiet",
				content: [{
					type: "text",
					text: data.content
		...
		case "agent-teams/team-deleted": return {
			...state,
			teamId: state.teamId ?? TeamId(dataOf(event).teamId),
			hasTeamEvents: true
		};
		default: return null;
	}
...
function teamIdOf(state, event) {
	...
	if (data.teamId !== void 0) return TeamId(data.teamId);
	return state.teamId;
}
...
function viewAgentTeam(state) {
	...
	}, ...Object.values(state.members).filter((member) => member.id !== captainSessionId).map(memberView)].sort(memberComparator);
...
//#endregion
//#region src/upstream-event-registration.ts
...
var AgentTeamHostCompatibilityError = class extends Error {
	name = "AgentTeamHostCompatibilityError";
};
function compatibilityError(missing) {
	return new AgentTeamHostCompatibilityError(`${pluginName} requires ${supportedSessionVersion} runtime event compatibility. Missing event types: ${missing.join(", ")}. Use a supported Host or upgrade this plugin.`);
}
function mutableCatalogueOf(value) {
	if (typeof value !== "object" || value === null) throw compatibilityError(UPSTREAM_TEAM_EVENT_TYPES);
	try {
		if (typeof value.add !== "function" || typeof value.has !== "function") throw compatibilityError(UPSTREAM_TEAM_EVENT_TYPES);
	} catch (error) {
...
function missingTypes(catalogue) {
	try {
...
function registerUpstreamAgentTeamEventTypes(value = KNOWN_SESSION_EVENT_TYPES) {
...
//#endregion
export { TeamId, agentTeamProjectionDefinition, apply, inject };
