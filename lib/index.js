...
enabledModelsSchema.meta.volatile = true;
roleDefaultsSchema.meta.volatile = true;
const AgentTeamSettingsSchema = z.object({
...
function registerAgentTeamSettings(sctx) {
	const settings = sctx.settings;
	if (settings === void 0) throw new Error("settings service is not available to register agent-team-web");
...
function modelGrantedFromScope(scope, provider, model) {
	if (provider === "deepseek-official") return true;
	return scope.get()?.enabledModels?.[modelKey(provider, model)] === true;
}
...
function wireAgentTeamSettings(settingsCtx, access) {
	...
	access.modelGrantedFor = (provider, model) => modelGrantedFromScope(scope, provider, model);
	access.roleDefaultsFor = (roleKey) => {
		return scope.get()?.roleDefaults?.[roleKey];
	};
	access.enabledModels = () => {
		return scope.get()?.enabledModels ?? {};
	};
	access.roleDefaults = () => {
		return scope.get()?.roleDefaults ?? {};
	};
	access.setModelGrant = async (provider, model, enabled) => {
		if (provider === "deepseek-official") return;
		const current = scope.get();
		const key = modelKey(provider, model);
		await scope.update({ enabledModels: {
			...current?.enabledModels ?? {},
			[key]: enabled
		} });
	};
	access.setRoleDefault = async (roleKey, value) => {
		const next = { ...scope.get()?.roleDefaults ?? {} };
		if (value === void 0) delete next[roleKey];
		else next[roleKey] = value;
		await scope.update({ roleDefaults: next });
	};
	const dispose = () => {
		access.modelGrantedFor = void 0;
		access.roleDefaultsFor = void 0;
		access.enabledModels = void 0;
		access.roleDefaults = void 0;
