...
function sanitizeKey(name) {
	...
	if (points.length > MAX_KEY_LENGTH) return `${points.slice(0, MAX_KEY_LENGTH).join("")}-${keyDigest(name)}`;
	return cleaned;
}
/**
...
*/
function memberPersona(team, member, stateDir) {
...
*/
async function spawnMember(ctx, config, selections, llmSelection, captain, team, member, stateDir, signal) {
...
function nextReadyTask(tasks, memberName) {
	const ready = tasks.filter((task) => task.status === "pending" && task.reassigning !== true && unsatisfiedDependencies([...tasks], task.dependencies).length === 0);
...
async function writeBestPractices(stateRoot, entries) {
...
function registerAgentTeamsTools(ctx, config) {
		...
		async execute(args, exec) {
			...
			return withTeamLock(captainLockKey(stateRoot, captain.id), async () => {
				...
				return withTeamLock(teamLockKey$1(stateRoot, teamId), async () => {
					...
					await spawnMember(ctx, memberRuntime(config), memberSelections, commissarSelection, captain, state, commissar, config.stateDir, exec.signal);
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			name: {
				type: "string",
				description: "Unique member name inside the team; when omitted (or just the role name) an auto-numbered name is generated (e.g. 技术员 一号)."
			},
			role: {
				type: "string",
				description: "Role of the member — military-role system: researcher 侦察参谋 / data 情报分析员 / engineer 技术员 / qa 质检员 / designer 文宣干事 / security 警卫员 / docs 文书 / operator 后勤保障员; reviewer 审查员 is a task-level dynamic role (add when dedicated review is needed)."
			},
			provider: {
				type: "string",
		...
		async execute(args, exec) {
			...
			const created = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (roleText !== "" && !isCommissarRole(roleText)) {
					const execCap = config.maxExecPerRole ?? 2;
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (target !== "captain") await scheduler.kickMember(workspace, team.id, target, captain);
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const updated = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (args.signal_note !== void 0 && args.signal_note.trim() !== "") task.signals = {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (captain !== void 0 && prepared.recipient.id !== "") {
				const senderText = prepared.from === "captain" ? args.content : `Message from team member ${prepared.from}:\n\n${args.content}`;
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			return withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (entryIndex >= 0) {
					let next = library;
					if (args.verdict === "useless") next = library.filter((entry, index) => index !== entryIndex);
					else next = updateBestPracticeVerdict(library, library[entryIndex].id, args.verdict, task.retro.cause);
					...
					if (practice !== void 0) {
						await writeBestPractices(stateRoot, upsertBestPractice(library, {
							...practice,
							verdict: args.verdict
						}));
						practiceUpdated = true;
...
*/
async function handleCloseTeam(ctx, config, workspaceRegistry, req, res) {
	if (req.method !== "POST") {
		sendJson(res, 405, {
			ok: false,
