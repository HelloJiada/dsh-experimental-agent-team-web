...
async function resolveMemberLlmSelection(ctx, captain, request, signal) {
	...
	const model = explicitModel ?? (roleProvider !== void 0 ? roleModel : void 0) ?? (captainProvider !== void 0 ? captainModel : void 0) ?? defaultModel ?? currentModel;
	...
	const roleEffortApplies = !(captainProvider !== void 0);
	const captainEffort = captainDefaults?.reasoningEffort?.trim();
	const captainInherited = captainDefaults?.provider?.trim() !== void 0 || captainDefaults?.model?.trim() !== void 0;
	const reasoningEffort = explicitEffort === void 0 ? roleEffort !== void 0 && roleEffort !== "" && roleEffortApplies ? roleEffort === "default" ? void 0 : ReasoningEffortId(roleEffort) : captainEffort !== void 0 && captainEffort !== "" && captainInherited && provider === inheritProvider && model === inheritModel ? captainEffort === "default" ? void 0 : ReasoningEffortId(captainEffort) : provider === currentProvider && model === currentModel ? current?.reasoningEffort : void 0 : explicitEffort === "default" ? void 0 : ReasoningEffortId(explicitEffort);
...
* @returns 卸载函数。
*/
function registerCaptainRoute(ctx, captainConfig) {
	const disposeCreated = ctx.on("agent/created", (payload) => {
		installForAgent(payload.agent);
	});
	for (const agent of ctx.agents?.list?.() ?? []) installForAgent(agent);
	return () => {
		disposeCreated?.();
	...
	function installForAgent(agent) {
		...
		const disposeRequest = agent.ctx?.on("agent/request", async (_payload, next) => {
			...
			if (selected.provider === void 0 || selected.model === void 0) return resolved;
			const { reasoningEffort: _inherited, ...withoutInherited } = resolved;
			return {
				...withoutInherited,
				provider: selected.provider,
				model: selected.model,
				...selected.reasoningEffort !== void 0 ? { reasoningEffort: ReasoningEffortId(selected.reasoningEffort) } : {}
			};
		});
...
* 现有会话继承)。
*/
function captainDefaultsFor(config) {
			...
			return withTeamLock(captainLockKey(stateRoot, captain.id), async () => {
				...
				return withTeamLock(teamLockKey$1(stateRoot, teamId), async () => {
					...
					const commissarSelection = await resolveMemberLlmSelection(ctx, captain, {
						...
						...captainDefaultsFor(config) === void 0 ? {} : { captainDefaults: captainDefaultsFor(config) }
					}, exec.signal);
					const commissar = {
						id: "",
						name: "政委",
				...
				try {
					...
					for (const entry of entries) {
						...
						try {
							...
							if (team?.captainSessionId !== void 0 && team.captainSessionId !== "") markCaptainAgent(team.captainSessionId);
						} catch {}
					}
				} catch {}
			}
