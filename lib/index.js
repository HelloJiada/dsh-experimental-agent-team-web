...
*/
function memberRouteCandidates(explicit) {
	const candidates = [];
...
function registerAgentTeamsTools(scopeCtx, config, runtime) {
				...
				return withTeamLock(teamLockKey$1(stateRoot, teamId), async () => {
					...
					if (mode !== "light") {
						...
						const grant = (provider, model) => {
							...
							return provider === (captainRoute?.provider ?? captain.options.provider) && model === (captainRoute?.model ?? captain.options.model);
						};
						const commissarSelection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates({}), grant, exec.signal, config.modelCapabilitiesFor);
						commissar = {
							id: "",
							name: "政委",
							role: "commissar",
	...
	scopeCtx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			canonicalExecRole(args.role);
			const captainRoute = captain.session.requestHeader()?.config;
			const grant = (provider, model) => {
				if (config.modelGrantedFor !== void 0) return config.modelGrantedFor(provider, model);
				return provider === (captainRoute?.provider ?? captain.options.provider) && model === (captainRoute?.model ?? captain.options.model);
			};
			const effectiveSelection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates({
	...
	scopeCtx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (prepared.needsCommissar) {
				...
				const selection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates({}), grant, exec.signal, config.modelCapabilitiesFor);
				const draft = {
					id: "",
					name: "政委",
					role: "commissar",
...
function modelCapabilityFromValue(value, provider, model) {
	...
	return {
		enabled: value?.enabledModels?.[key] === true,
		legacy: true
	};
}
...
async function collectProviders(ctx, enabledModels) {
	...
	for (const provider of providers) {
		...
		const grantedAny = models.some((model) => enabledMap[`${provider.id}/${model.id}`] === true) || Object.keys(enabledMap).some((key) => key.startsWith(`${provider.id}/`) && enabledMap[key] === true);
		views.push({
			id: provider.id,
			name: provider.name,
			enabled: grantedAny,
...
function apply(ctx, config) {
	...
	const toolsConfig = {
		...resolved,
		modelGrantedFor: (provider, model) => settingsAccess.modelGrantedFor?.(provider, model) ?? false,
