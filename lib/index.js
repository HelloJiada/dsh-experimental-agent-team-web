...
function installRetiredMemberGuard(ctx, stateDir) {
	const runtime = ctx.subagents;
	if (runtime.followup === void 0) return;
	ctx.effect(() => {
		...
		const guarded = async (parent, childId, ...rest) => {
			...
			const now = Date.now();
			if (cached === void 0 || now - cached.loadedAt >= 1e3) {
				cached = {
					ids: await readRetiredMemberIds(stateRoot),
					loadedAt: now
				};
				retiredIndexCache.set(stateRoot, cached);
			}
			if (cached.ids.has(String(childId))) throw new SubagentError(`AgentTeams member "${childId}" was retired and cannot be resumed`, "NOT_RESUMABLE");
			return await followup.call(ctx.subagents, parent, childId, ...rest);
		};
		runtime.followup = guarded;
		return () => {
			if (runtime.followup === guarded) runtime.followup = followup;
		};
