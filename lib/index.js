...
const DEFAULT_MAX_EXEC_PER_ROLE = 1;
/**
...
const OVERRUN_WARN_FACTOR = 1;
/** 严重超时阈值:实际/已用超过预算 1.5 倍即红色警示。 */
const OVERRUN_OVER_FACTOR = 1.5;
/**
...
function resolveTaskTiming(task, now) {
	...
	const overrunMs = actualMs !== void 0 && task.estimatedMs !== void 0 ? actualMs - task.estimatedMs : void 0;
	return {
		completedAt,
...
function buildTaskRetro(facts, cause, now = Date.now()) {
	...
	const deviation = timing.actualMs !== void 0 && estimatedMs !== void 0 ? timing.actualMs > estimatedMs ? `超出预估 ${formatDuration(timing.actualMs - estimatedMs)}` : `提前 ${formatDuration(estimatedMs - timing.actualMs)}` : void 0;
...
async function withTeamLock(key, fn) {
	...
	let release;
	const gate = new Promise((resolve$1) => {
		release = resolve$1;
	});
...
function keyDigest(name$1) {
	return createHash("sha256").update(name$1).digest("hex").slice(0, 8);
}
/**
...
*/
function sanitizeKey(name$1) {
	const cleaned = name$1.normalize("NFC").trim().toLowerCase().replace(/[^\p{L}\p{N}]+/gu, "-").replace(/^-+|-+$/g, "");
	if (cleaned === "") return `k-${keyDigest(name$1)}`;
	const points = [...cleaned];
	if (points.length > MAX_KEY_LENGTH) return `${points.slice(0, MAX_KEY_LENGTH).join("")}-${keyDigest(name$1)}`;
	return cleaned;
}
/**
...
*/
async function findTeamByCaptain(stateRoot, captainSessionId) {
	let entries;
	try {
...
*/
async function findTeamByParticipant(stateRoot, agentSessionId) {
	let entries;
	try {
		entries = await readdir(stateRoot, { withFileTypes: true });
	} catch (error) {
...
function memberStateDenial(toolName, args, workspace, stateDir) {
	...
	if (toolName === "bash") {
		...
		const stateDirToken = stateDirName === "" ? null : /* @__PURE__ */ new RegExp(`(^|[\\s"'${escapeRegExp(sep)}/])${escapeRegExp(stateDirName)}([\\s"'${escapeRegExp(sep)}/]|$)`);
...
async function resolveMemberLlmSelection(ctx, captain, request, signal) {
	...
	const reasoningEffort = explicitEffort === void 0 ? provider === currentProvider && model === currentModel ? current?.reasoningEffort : void 0 : explicitEffort === "default" ? void 0 : ReasoningEffortId(explicitEffort);
...
7. You are a worker: do not create or delete teams, reassign tasks, or add/remove members — that is the captain's job.`;
	...
	const memoryLines = memories.map((entry) => {
		return `- ${entry.level !== void 0 ? `[${entry.level}] ` : ""}${entry.practice} (来源任务「${entry.sourceTaskSubject}」· 归因 ${entry.cause})`;
	}).join("\n");
	return `${base}
...
*/
async function notifyCommissarPendingReview(ctx, stateRoot, team, task, signal) {
	const commissar = team.members.find(isActiveCommissar);
	if (commissar === void 0) return false;
	...
	if (captain !== void 0 && commissar.id !== "") await deliverToMember(ctx, captain, commissar.id, `AgentTeams 门禁通知：\n\n${message.content}`, signal);
	return true;
}
...
*/
function isRoleOnlyName(name$1, role) {
...
function isHelppableTask(task, team, helperName, now, parkedAttempts, liveStatus, stallThresholdMs) {
	...
	if (task.assignee === void 0 || task.assignee === helperName || task.assignee === CAPTAIN_KEY) return false;
...
function installTeamScheduler(ctx, config) {
	...
	const runtime = {
		...
		async kickMember(workspace, teamId, memberName, suppliedCaptain) {
			...
			await serializeMember(memberQueueKey(stateRoot, teamId, memberName), async () => {
...
async function persistBestPractices(stateRoot, entries) {
	...
	const { writeFile: writeFile$1, rm: rm$1, rename: rename$1, mkdir: mkdir$1 } = await import("node:fs/promises");
	await mkdir$1(stateRoot, { recursive: true });
	await writeFile$1(temporary, `${JSON.stringify(entries, null, 2)}\n`, {
		encoding: "utf8",
		flag: "wx"
	});
	await replaceFileAtomicOrDirect(temporary, join(stateRoot, BEST_PRACTICES_FILE), `${JSON.stringify(entries, null, 2)}\n`, {
		rename: (from, to) => rename$1(from, to),
		writeFile: (target, payload) => writeFile$1(target, payload, "utf8"),
		remove: (path) => rm$1(path, { force: true })
	});
}
/**
...
*/
const MIN_MEMBER_MEMORY_SAMPLES = 2;
/** 单成员注入的经验条目上限(保持 persona 精简,防止记忆淹没规则)。 */
const MAX_MEMBER_MEMORY_ENTRIES = 3;
/**
...
function selectBestPracticesForRole(entries, role) {
	...
	const matched = entries.filter((entry) => entry.role === normalized && entry.verdict !== "useless");
	...
	return [...matched].sort((left, right) => verdictOrder[left.verdict] - verdictOrder[right.verdict] || right.updatedAt - left.updatedAt).slice(0, MAX_MEMBER_MEMORY_ENTRIES);
}
/** 校验一条经验条目形状(读盘边界)。 */
...
/** 终结态任务不参与建议。 */
const TERMINAL_TASK_STATUSES$1 = new Set([
	"completed",
	"failed",
	"cancelled"
]);
...
function suggestAssignments(tasks, members) {
	...
	for (const task of tasks) {
		if (task.status !== void 0 && TERMINAL_TASK_STATUSES$1.has(task.status)) continue;
	...
	return tasks.map((task) => {
		if (task.status !== void 0 && TERMINAL_TASK_STATUSES$1.has(task.status)) return {
			taskId: task.id,
...
function requireMember(team, name$1) {
	const member = team.members.find((candidate) => candidate.name === name$1 && candidate.status !== "removed");
	if (member === void 0) throw new Error(`no active member named "${name$1}" in team "${team.name}"`);
	return member;
}
...
function steerCaptainReport(captain, from, content) {
	try {
		captain.steer(createUserMessage({
			content: [{
				type: "text",
				text: `AgentTeams message from member ${from}:\n\n${content}`
			}],
			source: {
				kind: "plugin",
				plugin: "@deepseek-ai/dsh-experimental-agent-team-web"
...
function registerAgentTeamsTools(ctx, config) {
		...
		async execute(args, exec) {
			...
			return withTeamLock(captainLockKey(stateRoot, captain.id), async () => {
				...
				return withTeamLock(teamLockKey$1(stateRoot, teamId), async () => {
					...
					const commissarSelection = await resolveMemberLlmSelection(ctx, captain, { defaultModel: config.memberModel }, exec.signal);
					const commissar = {
						id: "",
						name: "政委",
						role: "commissar",
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const created = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (memberKey === CAPTAIN_KEY) throw new Error(`member name "${args.name}" is reserved for the captain`);
				...
				if (roleText !== "" && !isCommissarRole(roleText)) {
					...
					if (countActiveExecRoleMembers(fresh.members, roleText) >= execCap) throw new Error(`executing role "${args.role}" already has ${execCap} active members — 该执行角色已达上限（每个执行角色最多 ${execCap} 名成员）`);
				}
				...
				await spawnMember(ctx, memberRuntime(config), memberSelections, selection, captain, fresh, member, config.stateDir, exec.signal, memories);
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const created = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				for (const dependency of dependencies) if (!fresh.tasks.some((task$1) => task$1.id === dependency)) throw new Error(`dependency "${dependency}" does not exist in team "${fresh.name}"`);
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const revoked = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
				const task$1 = requireTask(fresh, args.task_id);
				if (task$1.status === "completed") throw new Error(`completed task ${task$1.id} is immutable and cannot be reassigned`);
				if (task$1.reassigning === true) throw new Error(`task ${task$1.id} is already being reassigned`);
				const targetMember = target === CAPTAIN_KEY ? void 0 : requireMember(fresh, target);
				if (targetMember !== void 0) {
					const busy = memberOpenTask(fresh, targetMember.name, task$1.id);
					if (busy !== void 0) throw new Error(`member "${targetMember.name}" is busy with ${busy.id}; finish or reassign it first`);
				}
				const previousAssignee = task$1.assignee ?? "";
				const previousMember = task$1.status !== "claimed" && task$1.status !== "in_progress" || task$1.assignee === void 0 || task$1.assignee === CAPTAIN_KEY ? void 0 : fresh.members.find((member) => member.name === task$1.assignee && member.status !== "removed");
				invalidateTaskAttempt(task$1, target, true);
				await writeTeam(stateRoot, fresh);
				return {
					previousAssignee,
					previousMember: previousMember === void 0 ? void 0 : { ...previousMember },
					handoffId: task$1.handoffId
				};
			});
			let quiescenceError;
			...
			await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
				const task$1 = requireTask(fresh, args.task_id);
				if (task$1.handoffId !== revoked.handoffId || task$1.assignee !== target || task$1.reassigning !== true) throw new Error(`task ${task$1.id} changed during reassignment; refusing to overwrite the newer state`);
				task$1.reassigning = false;
				if (quiescenceError === void 0 && target === CAPTAIN_KEY) beginTaskAttempt(task$1, CAPTAIN_KEY);
				await writeTeam(stateRoot, fresh);
				appendTeamEvent(ctx, captain.session, "agent-team-web/task-updated", {
					teamId: fresh.id,
					taskId: task$1.id,
					status: task$1.status,
					assignee: task$1.assignee,
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const updated = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				const { team: fresh, identity } = await requireFreshParticipant(stateRoot, team.id, caller.id);
				const task = requireTask(fresh, args.task_id);
				if (identity.kind === "captain" && task.assignee !== void 0 && task.assignee !== CAPTAIN_KEY) throw new Error(`task ${task.id} is owned by member "${task.assignee}"; call agent_teams_reassign_task with assignee="captain" before takeover`);
				...
				if (args.status === "completed" && gateBlocksCompletion(task)) {
					...
					task.updatedAt = Date.now();
					await writeTeam(stateRoot, fresh);
					if (!await notifyCommissarPendingReview(ctx, stateRoot, fresh, task, exec.signal)) throw new Error(`task ${task.id} requires commissar review (需要政委复核) before completing, but the team has no active commissar — add one with agent_teams_add_member(role=commissar) first`);
				...
				if (TERMINAL_TASK_STATUSES.includes(task.status)) {
					...
					if (task.retro === void 0 && task.actualMs !== void 0 && task.claimedAt !== void 0) {
						...
						if (task.status !== "cancelled") {
							if (task.retro.retroNote !== void 0 || task.retro.recommendation !== "") {
				...
				return {
					...
					...task.signals === void 0 ? {} : { signals: {
						...task.signals.turns !== void 0 ? { turns: task.signals.turns } : {},
						...task.signals.toolCalls !== void 0 ? { tool_calls: task.signals.toolCalls } : {},
						output_bytes: task.signals.outputBytes,
						...task.signals.selfReport !== void 0 ? { self_report: task.signals.selfReport } : {}
					} },
					...
					...task.retro === void 0 ? {} : { retro: {
						attempt: task.retro.attempt,
						actual_ms: task.retro.actualMs,
						...task.retro.estimateLevel !== void 0 ? { estimate_level: task.retro.estimateLevel } : {},
						...task.retro.estimatedMs !== void 0 ? { estimated_ms: task.retro.estimatedMs } : {},
						...task.retro.overrunMs !== void 0 ? { overrun_ms: task.retro.overrunMs } : {},
						...task.retro.levelDeviation !== void 0 ? { level_deviation: task.retro.levelDeviation } : {},
						overran: task.retro.overran,
						cause: task.retro.cause,
						summary: task.retro.summary,
						...task.retro.retroNote !== void 0 ? { retro_note: task.retro.retroNote } : {},
						...task.retro.captainVerdict !== void 0 ? { captain_verdict: task.retro.captainVerdict } : {},
			...
			await scheduler.kickTeam(workspace, team.id, team.captainSessionId === caller.id ? caller : void 0);
			return updated;
		}
	}));
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			return await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				const task = requireTask(fresh, args.task_id);
				if (TERMINAL_TASK_STATUSES.includes(task.status)) throw new Error(`terminal task ${task.id} is immutable and needs no review`);
		...
		async execute(args, exec) {
			...
			const prepared = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (to === CAPTAIN_KEY) {
					...
					await appendMailbox(stateRoot, fresh.id, CAPTAIN_KEY, message$1);
			...
			if (prepared.kind === "captain") {
				let delivered$1 = "mailbox";
				if (captain !== void 0 && prepared.identity.kind === "member") delivered$1 = steerCaptainReport(captain, prepared.from, args.content) ? "live" : "mailbox";
				if (delivered$1 === "live") await withTeamLock(teamLockKey$1(stateRoot, prepared.fresh.id), () => acknowledgeMailbox(stateRoot, prepared.fresh.id, CAPTAIN_KEY, [prepared.message.id]));
			...
			if (captain !== void 0 && prepared.recipient.id !== "") {
				const senderText = prepared.from === CAPTAIN_KEY ? args.content : `Message from team member ${prepared.from}:\n\n${args.content}`;
	...
	ctx.tools.register(defineTool({
		...
		async execute(_args, exec) {
			...
			if (located.captainSessionId === caller.id) await scheduler.kickTeam(workspace, located.id, caller);
			...
			const acknowledged = identity.kind === "captain" ? captainInbox.map((message) => message.id) : await readUnreadMailbox(stateRoot, team.id, identity.name).then((messages) => messages.map((message) => message.id));
...
function roleOfTask(team, task) {
	if (task.assignee === void 0 || task.assignee === CAPTAIN_KEY) return "captain";
...
function renderStatus(value) {
	...
	const lines = [
		...
		...team.tasks.map((task) => {
			...
			if (task.status === "in_progress" && task.claimed_at !== void 0) {
	...
	for (const [name$1, inbox] of Object.entries(team.member_inboxes)) lines.push(`Member inbox ${name$1} (${inbox.count}): latest — ${inbox.latest.slice(0, 120)}`);
...
async function handleCloseTeam(ctx, config, workspaceRegistry, req, res, auth) {
	...
	const liveCaptain$1 = ctx.agents.get(captainSessionId);
	...
	if (liveCaptain$1 !== void 0) for (const member of roster) {
		if (member.id === "") continue;
		interruptMember(ctx, liveCaptain$1, member.id);
	}
	await withTeamLock(teamLockKey(stateRoot, teamId), async () => {
		const fresh = await readTeam(stateRoot, teamId);
		if (fresh !== void 0 && liveCaptain$1 !== void 0) appendTeamEvent(ctx, liveCaptain$1.session, "agent-team-web/team-deleted", { teamId: fresh.id });
...
function taskInsight(snapshot, task, memberNames) {
	...
	if (task.status === "completed") {
		const reasons$1 = ["任务已完成。"];
		if (task.retro !== void 0 && task.retro.overran) reasons$1.push(`任务超时完成:实际 ${formatDuration(task.retro.actualMs)} vs 预估 ${task.retro.estimatedMs !== void 0 ? formatDuration(task.retro.estimatedMs) : "未预估"},已生成复盘(原因:${task.retro.cause})。`);
		return {
			taskId: task.id,
			subject: task.subject,
			status: task.status,
...
async function deriveTaskToolCalls(ctx, stateRoot, teamId, roster, tasks) {
	...
	for (const task of tasks) {
		...
		let count = 0;
		for (const text of texts) if (idPattern.test(text) || subjectNeedle.length >= 4 && text.includes(subjectNeedle)) count += 1;
...
function apply(ctx, config) {
	...
	const registerWebSurface = () => {
		...
		webRegistered = true;
		const inject$1 = ctx.on;
		inject$1("webserver/index-inject", (table) => {
			table.push({
				kind: "global",
				name: TOKEN_GLOBAL,
				value: webToken
		...
		ctx.effect(() => webServer.register({
			...
			handler: async (req, res) => {
				let name$1;
				try {
					name$1 = decodeURIComponent(new URL(req.url ?? "/", "http://x").pathname.split("/").pop() ?? "");
				} catch {
					res.writeHead(404);
					res.end();
					return;
				}
				if (!ART_ALLOWLIST.has(name$1)) {
					res.writeHead(404);
					res.end();
					return;
				}
				try {
					const data = await readFile(join(artDir, name$1));
					res.writeHead(200, {
					...
					ctx.logger.warn(`agent-teams: artwork read failed for ${name$1}: ${String(error)}`);
					res.writeHead(404);
					res.end();
				}
			}
	...
	registerWebSurface();
	ctx.on("internal/service", (name$1) => {
		if (WEB_SERVER_KEYS.includes(name$1) || WORKSPACE_KEYS.includes(name$1)) registerWebSurface();
	});
}
