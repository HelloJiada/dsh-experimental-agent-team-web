...
async function providerGranted(stateRoot, provider) {
	if (provider === "deepseek-official") return true;
	return (await readProviderGrants(stateRoot)).has(provider);
}
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (explicitlyRouted && selection.provider !== "deepseek-official") {
				if (!await providerGranted(stateRoot, selection.provider)) {
