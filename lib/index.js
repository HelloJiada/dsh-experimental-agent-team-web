...
function buildTaskRetro(facts, cause, now = Date.now()) {
	...
	return {
		...
		overran,
		cause: resolvedCause,
		...facts.causeSource !== void 0 ? { causeSource: facts.causeSource } : {},
		summary,
...
function mainChainTaskBudget(team) {
	return Number.isSafeInteger(team.mainChainTaskBudget) && (team.mainChainTaskBudget ?? 0) > 0 ? team.mainChainTaskBudget : 3;
}
/** Effective impact class; legacy tasks remain execution-blocking main-chain work. */
function taskImpact(task) {
	return task.impact === "changes-decision" || task.impact === "blocks-execution" || task.impact === "evidence-quality" || task.impact === "maintenance" ? task.impact : "blocks-execution";
}
...
function deliverableKey(value) {
	...
	if (normalized === void 0 || normalized === "" || normalized.startsWith("/") || normalized.split("/").some((part) => part === "" || part === "." || part === "..")) return void 0;
	return normalized.toLowerCase();
}
/** Reject a main task when any transitive dependency is maintenance-only. */
function maintenanceDependencyPath(tasks, dependencies) {
	...
	for (const dependency of dependencies) {
		const found = visit(dependency, [], /* @__PURE__ */ new Set());
		if (found !== void 0) return found;
	}
}
...
function archiveAndClearReview(task) {
	if (task.review !== void 0) {
...
function isTeamTask(value) {
	...
	const validReview = review === void 0 || isRecord(review) && typeof review["reviewerName"] === "string" && review["reviewerName"] !== "" && (review["verdict"] === "pass" || review["verdict"] === "reject") && (review["comment"] === void 0 || typeof review["comment"] === "string") && isFiniteNumber(review["reviewedAt"]) && (review["attempt"] === void 0 || Number.isSafeInteger(review["attempt"])) && (review["contractRevision"] === void 0 || Number.isSafeInteger(review["contractRevision"]));
	const reviewHistory = value["reviewHistory"];
	const validReviewHistory = reviewHistory === void 0 || Array.isArray(reviewHistory) && reviewHistory.every((item) => {
		if (!isRecord(item)) return false;
		return typeof item["reviewerName"] === "string" && item["reviewerName"] !== "" && (item["verdict"] === "pass" || item["verdict"] === "reject") && (item["comment"] === void 0 || typeof item["comment"] === "string") && isFiniteNumber(item["reviewedAt"]) && (item["attempt"] === void 0 || Number.isSafeInteger(item["attempt"])) && (item["contractRevision"] === void 0 || Number.isSafeInteger(item["contractRevision"]));
	});
	...
	return typeof value["id"] === "string" && typeof value["subject"] === "string" && isOptionalString(value["description"]) && (value["status"] === "pending" || value["status"] === "claimed" || value["status"] === "in_progress" || value["status"] === "completed" || value["status"] === "failed" || value["status"] === "cancelled") && isOptionalString(value["assignee"]) && Array.isArray(value["dependencies"]) && value["dependencies"].every((dependency) => typeof dependency === "string") && isOptionalString(value["output"]) && (value["attempt"] === void 0 || Number.isSafeInteger(value["attempt"]) && value["attempt"] >= 0) && isOptionalString(value["attemptId"]) && isOptionalString(value["handoffId"]) && (value["reassigning"] === void 0 || typeof value["reassigning"] === "boolean") && (value["riskLevel"] === void 0 || value["riskLevel"] === "low" || value["riskLevel"] === "medium" || value["riskLevel"] === "high" || value["riskLevel"] === "critical") && (value["milestone"] === void 0 || typeof value["milestone"] === "boolean") && (value["reviewRequired"] === void 0 || typeof value["reviewRequired"] === "boolean") && validReview && validReviewHistory && (value["contractRevision"] === void 0 || Number.isSafeInteger(value["contractRevision"]) && value["contractRevision"] >= 0) && (value["impact"] === void 0 || value["impact"] === "main" || value["impact"] === "changes-decision" || value["impact"] === "blocks-execution" || value["impact"] === "evidence-quality" || value["impact"] === "maintenance") && (value["budgetException"] === void 0 || isRecord(value["budgetException"]) && typeof value["budgetException"]["decisionImpact"] === "string" && isFiniteNumber(value["budgetException"]["grantedAt"])) && (value["deliverable"] === void 0 || typeof value["deliverable"] === "string") && (value["deliverableKey"] === void 0 || typeof value["deliverableKey"] === "string") && (value["acceptance"] === void 0 || typeof value["acceptance"] === "string") && (value["acceptanceRevision"] === void 0 || Number.isSafeInteger(value["acceptanceRevision"]) && value["acceptanceRevision"] >= 0) && (value["acceptanceHistory"] === void 0 || Array.isArray(value["acceptanceHistory"])) && (value["verification"] === void 0 || isRecord(value["verification"]) && Array.isArray(value["verification"]["checked"]) && value["verification"]["checked"].every((entry) => typeof entry === "string") && Array.isArray(value["verification"]["unchecked"]) && value["verification"]["unchecked"].every((entry) => typeof entry === "string")) && (value["scopeContract"] === void 0 || isRecord(value["scopeContract"]) && typeof value["scopeContract"]["originalAcceptance"] === "string" && isOptionalString(value["scopeContract"]["originalDeliverable"]) && Array.isArray(value["scopeContract"]["excluded"]) && value["scopeContract"]["excluded"].every((item) => typeof item === "string") && Number.isSafeInteger(value["scopeContract"]["maxInvestigations"]) && Number.isSafeInteger(value["scopeContract"]["investigationsUsed"]) && Array.isArray(value["scopeContract"]["findings"]) && value["scopeContract"]["findings"].every((item) => isRecord(item) && typeof item["summary"] === "string" && typeof item["evidence"] === "string" && (item["relation"] === "blocks-acceptance" || item["relation"] === "data-loss" || item["relation"] === "security" || item["relation"] === "unrelated" || item["relation"] === "uncertain") && (item["disposition"] === "pending-captain" || item["disposition"] === "approved" || item["disposition"] === "deferred")) && typeof value["scopeContract"]["awaitingCaptain"] === "boolean" && typeof value["scopeContract"]["originalAcceptanceMet"] === "boolean") && (value["blockedByReview"] === void 0 || typeof value["blockedByReview"] === "boolean") && (value["awaitingInput"] === void 0 || typeof value["awaitingInput"] === "boolean") && isOptionalString(value["helper"]) && (value["helperSince"] === void 0 || isFiniteNumber(value["helperSince"])) && (value["helperEver"] === void 0 || typeof value["helperEver"] === "boolean") && (value["estimateLevel"] === void 0 || value["estimateLevel"] === "S" || value["estimateLevel"] === "M" || value["estimateLevel"] === "L") && (value["estimatedMs"] === void 0 || isFiniteNumber(value["estimatedMs"])) && (value["claimedAt"] === void 0 || isFiniteNumber(value["claimedAt"])) && (value["startedAt"] === void 0 || isFiniteNumber(value["startedAt"])) && (value["completedAt"] === void 0 || isFiniteNumber(value["completedAt"])) && (value["actualMs"] === void 0 || isFiniteNumber(value["actualMs"])) && (value["overrunMs"] === void 0 || isFiniteNumber(value["overrunMs"])) && validSignals && validRetro && isFiniteNumber(value["createdAt"]) && isFiniteNumber(value["updatedAt"]);
}
...
function taskReviewPassed(task) {
	const review = task.review;
	if (review?.verdict !== "pass") return false;
	const attemptBound = review.attempt === void 0 || review.attempt === (task.attempt ?? 0);
	const contractBound = review.contractRevision === void 0 || review.contractRevision === (task.contractRevision ?? 0);
...
function descriptionAwaitingInput(description) {
	...
	if (description.length <= AWAITING_INPUT_SHORT_DESC_CHARS) return AWAITING_INPUT_HINTS.some((hint) => normalized.includes(hint));
...
function isTeamState(value, expectedId) {
	if (!isRecord(value)) return false;
	if (!(value["id"] === expectedId && typeof value["name"] === "string" && value["name"].trim() !== "" && isOptionalString(value["description"]) && isOptionalString(value["goal"]) && (value["workflowMode"] === void 0 || value["workflowMode"] === "decision" || value["workflowMode"] === "governance-maintenance") && (value["mainChainTaskBudget"] === void 0 || Number.isSafeInteger(value["mainChainTaskBudget"]) && value["mainChainTaskBudget"] > 0) && (value["mainChainTaskUsed"] === void 0 || Number.isSafeInteger(value["mainChainTaskUsed"]) && value["mainChainTaskUsed"] >= 0) && typeof value["captainSessionId"] === "string" && value["captainSessionId"] !== "" && isFiniteNumber(value["createdAt"]) && Array.isArray(value["members"]) && value["members"].every(isTeamMember) && Array.isArray(value["tasks"]) && value["tasks"].every(isTeamTask) && Number.isSafeInteger(value["taskSeq"]) && value["taskSeq"] >= 0)) return false;
...
*/
async function ensureMemberAgent(id, workspace, stateDir) {
...
*/
function selectBestPracticesForRole(entries, role, now = Date.now()) {
	if (role === void 0 || role.trim() === "") return [];
	const normalized = role.trim();
	const matched = entries.filter((entry) => entry.role === normalized && INJECTABLE_BEST_PRACTICE_VERDICTS.has(entry.verdict) && isOptionalMetadataValid(entry) && isInjectableSource(entry) && !isExpiredOrDisabled(entry, now) && hasNoUnmatchedConditions(entry));
	if (matched.length < 2) return [];
	return [...matched].sort((left, right) => right.updatedAt - left.updatedAt).slice(0, 3);
}
/** 来源校验：显式 evidence 须非空、结构有效且每项 taskId 与源任务一致。 */
function isValidEvidence(entry) {
	return Array.isArray(entry.evidence) && entry.evidence.length > 0 && entry.evidence.every((item) => typeof item === "object" && item !== null && typeof item.taskId === "string" && item.taskId === entry.sourceTaskId && (item.attempt === void 0 || Number.isInteger(item.attempt) && item.attempt > 0) && (item.excerpt === void 0 || typeof item.excerpt === "string" && item.excerpt.length <= 500) && (item.observedAt === void 0 || typeof item.observedAt === "number" && Number.isFinite(item.observedAt)));
}
/** 来源校验：旧条目依赖原有非空来源标识，继续支持历史引用路径。 */
function isInjectableSource(entry) {
	if (typeof entry.sourceTeamId !== "string" || !entry.sourceTeamId.trim() || typeof entry.sourceTaskId !== "string" || !entry.sourceTaskId.trim() || typeof entry.sourceTaskSubject !== "string" || !entry.sourceTaskSubject.trim()) return false;
	return entry.evidence === void 0 || isValidEvidence(entry);
}
/** 可选字段运行时校验：错误的新字段保留可读，但禁止注入。 */
function isOptionalMetadataValid(entry) {
	if (entry.evidence !== void 0 && !isValidEvidence(entry)) return false;
	if (entry.appliesWhen !== void 0 && (!Array.isArray(entry.appliesWhen) || !entry.appliesWhen.every((item) => typeof item === "string"))) return false;
	if (entry.counterexamples !== void 0 && (!Array.isArray(entry.counterexamples) || !entry.counterexamples.every((item) => typeof item === "object" && item !== null && typeof item.context === "string" && typeof item.reason === "string"))) return false;
	if (entry.expiresAt !== void 0 && (typeof entry.expiresAt !== "number" || !Number.isFinite(entry.expiresAt))) return false;
	if (entry.disabledAt !== void 0 && (typeof entry.disabledAt !== "number" || !Number.isFinite(entry.disabledAt))) return false;
	if (entry.disabledReason !== void 0 && typeof entry.disabledReason !== "string") return false;
	if (entry.revision !== void 0 && (!Number.isInteger(entry.revision) || entry.revision < 0)) return false;
	if (entry.reviewHistory !== void 0 && (!Array.isArray(entry.reviewHistory) || !entry.reviewHistory.every((item) => typeof item === "object" && item !== null && typeof item.actor === "string" && item.actor.trim() !== "" && typeof item.action === "string" && item.action.trim() !== "" && typeof item.at === "number" && Number.isFinite(item.at) && (item.summary === void 0 || typeof item.summary === "string")))) return false;
	return true;
}
/** 注入时点过期/撤销状态门禁。 */
function isExpiredOrDisabled(entry, now) {
	return entry.disabledAt !== void 0 || entry.expiresAt !== void 0 && entry.expiresAt <= now;
}
/** 当前调用路径没有任务条件上下文；非空 appliesWhen 保守地拒绝注入。 */
function hasNoUnmatchedConditions(entry) {
	return entry.appliesWhen === void 0 || Array.isArray(entry.appliesWhen) && entry.appliesWhen.length === 0;
}
...
function memberRouteForRequest(captain, request) {
	...
	const reasoningEffort = explicitEffort === void 0 ? provider === currentProvider && model === currentModel ? current?.reasoningEffort : void 0 : explicitEffort;
	return {
		provider,
		model,
...
async function resolveMemberRoute(ctx, route, signal) {
	const effort = route.reasoningEffort === void 0 || route.reasoningEffort === "default" ? void 0 : ReasoningEffortId(route.reasoningEffort);
...
async function resolveMemberLlmCandidates(ctx, captain, candidates, isGranted, signal, capabilityFor) {
	const failures = [];
	for (const candidate of candidates) try {
		...
		if (capabilityFor !== void 0) {
			if (capability?.enabled !== true) throw new Error(`model ${route.provider}/${route.model} is not authorized`);
		} else if (!isGranted(route.provider, route.model)) throw new Error(`model ${route.provider}/${route.model} is not authorized`);
		...
		if (ceiling !== void 0 && !offered.includes(ceiling)) throw new Error(`model ${route.provider}/${route.model} has no configured ceiling effort ${ceiling}`);
		const requested = candidate.request.reasoningEffort;
		if (info.reasoning === void 0 || offered.length === 0) {
			if (requested !== void 0 && requested !== "default") throw new Error(`model ${route.provider}/${route.model} does not advertise reasoning effort`);
			return await resolveMemberRoute(ctx, {
				...route,
				reasoningEffort: "default"
			}, signal);
		}
		if (requested !== void 0 && requested !== "default") {
			if (!offered.includes(requested)) throw new Error(`model ${route.provider}/${route.model} does not support effort ${requested}`);
			if (ceiling === void 0) throw new Error(`model ${route.provider}/${route.model} has no configured reasoning ceiling; select adapter default`);
		...
		const adapterDefault = info.reasoning.defaultEffort === void 0 ? void 0 : String(info.reasoning.defaultEffort);
		const safeDefault = adapterDefault !== void 0 && ceiling !== void 0 && offered.indexOf(adapterDefault) > offered.indexOf(ceiling) ? ceiling : adapterDefault ?? ceiling;
		return await resolveMemberRoute(ctx, {
			...route,
			reasoningEffort: safeDefault ?? "default"
		}, signal);
	} catch (error) {
		const reason = String(error instanceof Error ? error.message : error);
		if (candidate.explicit === true) throw new Error(`explicit member route ${candidate.label} rejected: ${reason}`);
...
*/
function installMemberSelectionRuntime(_ctx, _stateDir) {
	const pending = /* @__PURE__ */ new Set();
	return { async withPending(parentSessionId, label, _selection, operation) {
...
8. You are a worker: do not create or delete teams, reassign tasks, or add/remove members — that is the captain's job.`;
	...
	const memoryLines = memories.map((entry) => {
		return `- ${entry.level !== void 0 ? `[${entry.level}] ` : ""}${truncatePracticeForInjection(entry.practice)} (历史经验 · 归因 ${entry.cause})`;
	}).join("\n");
	return `${base}
	...
	member.id = (await selections.withPending(captain.id, label, llmSelection, () => ctx.subagents.startContinuable({
		...
		request: {
			...
			agentOptions: {
				...
				...llmSelection.reasoningEffort === void 0 ? {} : { reasoningEffort: ReasoningEffortId(llmSelection.reasoningEffort) }
			},
...
async function deliverToMember(ctx, captain, childId, text, signal) {
	try {
		...
		if (send === void 0) throw new Error("subagent runtime exposes neither sendMessage nor legacy followup");
		await send.call(ctx.subagents, captain, brandedSessionId(childId), [{
			type: "text",
			text
		}], { signal });
		return true;
...
async function assertMemberNotRetired(parent, childId, stateDir) {
	if ((await readRetiredMemberIds(join(parent.session.header.cwd ?? process.cwd(), stateDir))).has(childId)) throw new SubagentError(`AgentTeams member "${childId}" was retired and cannot be resumed`, "NOT_RESUMABLE");
}
/**
...
function installRetiredMemberGuard(ctx, stateDir) {
	...
	ctx.effect(() => {
		const restorers = [];
		for (const method of ["sendMessage", "followup"]) {
			const original = runtime[method];
			if (typeof original !== "function") continue;
			const guarded = async (parent, childId, ...rest) => {
...
*/
function previewStatusText(text, limit) {
...
function renderStatus(value) {
	...
	const lines = [
		...
		...team.tasks.map((task) => {
			...
			const output = task.output !== void 0 ? `\n      output: ${previewStatusText(task.output, 300)}` : "";
			const acceptance = task.acceptance !== void 0 ? `\n      acceptance (rev ${task.acceptance_revision ?? 0}): ${previewStatusText(task.acceptance, 300)}` : "";
			const deliverable = task.deliverable !== void 0 ? `\n      deliverable: ${previewStatusText(task.deliverable, 160)}` : "";
			...
			const gate = task.review_required === true ? task.review?.verdict === "pass" ? " · review passed" : task.blocked_by_review === true ? ` · awaiting review (已提交待复核)${task.review !== void 0 ? ` · last verdict ${task.review.verdict}` : ""}` : ` · review required (将需复核·尚未提交)${task.review !== void 0 ? ` · last verdict ${task.review.verdict}` : ""}` : "";
...
const kickRetryCounts = /* @__PURE__ */ new Map();
function kickRetryKey(workspace, teamId, memberName) {
...
*/
function scheduleKickRetry(retry, workspace, teamId, memberName, ctx) {
...
function nextReadyTask(tasks, memberName) {
	const ready = tasks.filter((task) => task.status === "pending" && task.scopeContract?.awaitingCaptain !== true && task.reassigning !== true && !taskAwaitingInput(task) && unsatisfiedDependencies([...tasks], task.dependencies).length === 0);
	const main = ready.filter((task) => task.impact !== "maintenance");
	const maintenance = ready.filter((task) => task.impact === "maintenance");
	return main.find((task) => task.assignee === memberName) ?? main.find((task) => task.assignee === void 0) ?? maintenance.find((task) => task.assignee === memberName) ?? maintenance.find((task) => task.assignee === void 0);
}
/**
...
function nextHelpTask(tasks, team, helperName, now, parkedAttempts, liveStatus, stallThresholdMs) {
	...
	return tasks.filter((task) => isHelppableTask(task, team, helperName, now, parkedAttempts, liveStatus, stallThresholdMs)).sort((a, b) => (a.impact === "maintenance" ? 1 : 0) - (b.impact === "maintenance" ? 1 : 0) || a.updatedAt - b.updatedAt)[0];
}
	...
	let runtime = {
		...
		async kickMember(workspace, teamId, memberName, suppliedCaptain) {
			...
			await serializeMember(queueKey, async () => {
				...
				if (member === void 0 || member.id === "") return;
				if (!isMemberAvailable(ctx, member)) {
					scheduleKickRetry(() => runtime.kickMember(workspace, teamId, memberName, suppliedCaptain), workspace, teamId, memberName, ctx);
					return;
				}
				...
				const ticket = await withTeamLock(teamLockKey$2(stateRoot, team.id), async () => {
					...
					if (isCommissarRole(currentMember.role)) return void 0;
...
*/
function memberRouteCandidates(config, roleKey, explicit) {
	const candidates = [];
	if (explicit.provider !== void 0 || explicit.model !== void 0 || explicit.reasoningEffort !== void 0) candidates.push({
		label: "explicit",
		explicit: true,
		request: explicit
	});
...
function liveMemberByName(team, name) {
...
*/
function assertAssigneeIsNotCommissar(team, assignee, action) {
	if (assignee === void 0 || assignee === "captain") return;
	const target = liveMemberByName(team, assignee);
	if (target !== void 0 && isCommissarRole(target.role)) throw new Error(`${action}: "${target.name}" is the commissar and cannot own task work — the commissar gates task completion, so executing the same task would destroy review independence`);
}
/**
...
* @param action - the tool action being refused, for the error message.
*/
function assertCallerIsNotCommissar(team, identity, action) {
	if (identity.kind !== "member") return;
	const caller = liveMemberByName(team, identity.name);
	if (caller !== void 0 && isCommissarRole(caller.role)) throw new Error(`the commissar cannot ${action} — independent oversight must not execute the work it reviews`);
}
...
*/
function installAgentTeamsRuntime(ctx, config) {
	...
	return {
		serviceCtx: ctx,
		memberSelections,
		kickTeamAsync: (workspace, teamId, captain) => {
		...
		kickMemberAsync: (workspace, teamId, memberName, captain) => {
...
*/
function registerAgentTeamsTools(scopeCtx, config, runtime) {
	...
	scopeCtx.tools.register(defineTool({
		...
		parameters: {
			...
			mode: {
				...
				description: "Collaboration mode; light omits the automatic commissar and disallows gated tasks."
			},
			goal: {
				type: "string",
				description: "Explicit workflow goal (defaults to description)."
			},
			workflow_mode: {
				type: "string",
				enum: ["decision", "governance-maintenance"],
				description: "User-selected workflow mode; decision prioritizes a bounded main chain."
			},
			main_chain_task_budget: {
				type: "number",
				description: "Maximum main-chain tasks; legacy/default is 3."
			}
		},
		output: {
			schema: {
		...
		async execute(args, exec) {
			...
			const mode = args.mode === "light" || args.mode === "governed" ? args.mode : "standard";
			...
			return withTeamLock(captainLockKey(stateRoot, captain.id), async () => {
				...
				return withTeamLock(teamLockKey$1(stateRoot, teamId), async () => {
					...
					const state = {
						...
						mainChainTaskBudget: typeof args.main_chain_task_budget === "number" && Number.isFinite(args.main_chain_task_budget) && args.main_chain_task_budget > 0 ? Math.floor(args.main_chain_task_budget) : 3,
						mainChainTaskUsed: 0,
						mode,
					...
					if (mode !== "light") {
						...
						const grant = (provider, model) => {
							if (config.modelGrantedFor !== void 0) return config.modelGrantedFor(provider, model);
							return provider === "deepseek-official" || provider === (captainRoute?.provider ?? captain.options.provider) && model === (captainRoute?.model ?? captain.options.model);
						};
						const commissarSelection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates(config, "commissar", {}), grant, exec.signal, config.modelCapabilitiesFor);
						commissar = {
							id: "",
							name: "政委",
							role: "commissar",
					...
					try {
						...
						if (commissar !== void 0 && commissar.id !== "") {
					...
					if (commissar !== void 0) appendTeamEvent(ctx, captain.session, "agent-team-web/member-added", {
						teamId: state.id,
	...
	scopeCtx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const effectiveSelection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates(config, roleKey, {
				provider: args.provider,
				model: args.model,
				reasoningEffort: args.reasoning_effort
			}), grant, exec.signal, config.modelCapabilitiesFor);
			const member = {
				id: "",
			...
			await spawnMember(ctx, memberRuntime(config), memberSelections, effectiveSelection, captain, prepared.fresh, member, config.stateDir, exec.signal, memories);
			const created = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				return {
					...
					provider: effectiveSelection.provider,
					model: effectiveSelection.model,
					...effectiveSelection.reasoningEffort === void 0 ? {} : { reasoning_effort: effectiveSelection.reasoningEffort },
	...
	scopeCtx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const revoked = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (teamMode(fresh) === "governed" && isActiveCommissar(member)) throw new Error(`team "${fresh.name}" is in governed mode: the commissar provides the required independent review and cannot be removed. Keep the commissar, or start a new team in light/standard mode if oversight is not needed.`);
				const requeued = [];
	...
	scopeCtx.tools.register(defineTool({
		...
		parameters: { mode: {
			...
			description: "Target mode; only standard or governed may be requested (downgrade to light is refused)."
		} },
		output: {
			schema: {
				type: "object",
			...
			render: (args, value) => [{
				type: "text",
				text: value.changed ? `Team ${value.team_id} switched to ${value.mode} mode${value.commissar_created ? " (commissar created)" : ""}.` : `Team ${value.team_id} already runs in ${value.mode} mode.`
			}]
		},
		async execute(args, exec) {
			const captain = requireCaptain(exec);
			...
			const stateRoot = stateRootOf(workspace, config);
			const team = await requireCaptainTeam(workspace, config, captain, warnSkippedTeamDir(ctx));
			const target = args.mode === "governed" ? "governed" : args.mode === "light" ? "light" : "standard";
			if (target === "light") throw new Error(`team "${team.name}" cannot be downgraded to light mode: an active commissar and any gated task would lose their review authority. Start a new light team instead if lighter coordination is what you need.`);
			const prepared = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
				const current = teamMode(fresh);
				if (MODE_RANK[target] < MODE_RANK[current]) throw new Error(`team "${fresh.name}" cannot switch from ${current} to ${target} mode: modes only ever tighten (light → standard → governed). Start a new team if a weaker policy is really required.`);
				const hasCommissar = fresh.members.some((member) => isActiveCommissar(member));
				if (current === target && hasCommissar) return {
					fresh,
					needsCommissar: false,
					changed: false,
					modeWritten: false
				};
				if (hasCommissar) {
			...
			if (prepared.needsCommissar) {
				...
				const selection = await resolveMemberLlmCandidates(ctx, captain, memberRouteCandidates(config, "commissar", {}), grant, exec.signal, config.modelCapabilitiesFor);
				const draft = {
					id: "",
					name: "政委",
					role: "commissar",
				...
				await spawnMember(ctx, memberRuntime(config), memberSelections, selection, captain, prepared.fresh, draft, config.stateDir, exec.signal, memories);
				let attached = false;
				try {
					attached = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
						const fresh = await requireFreshCaptainTeam(stateRoot, team.id, captain.id);
					...
					if (draft.id !== "") {
						await recordRetiredMemberIds(stateRoot, [draft.id]).catch(() => void 0);
			...
			return {
				...
				commissar_created: commissarCreated
			};
		}
	}));
	scopeCtx.tools.register(defineTool({
		...
		parameters: {
			...
			assignee: {
				type: "string",
				description: "Optional member name this task is intended for; \"captain\" assigns it to the captain (who still claims it before working)."
			},
			risk: {
				type: "string",
				enum: [
			...
			impact: {
				...
				description: "Workflow impact. The first three consume main-chain budget; maintenance is delayed and budget-free."
			},
			deliverable: {
				type: "string",
				description: "Optional output path/key; duplicate active writers are rejected."
			},
			acceptance: {
				type: "string",
				description: "Acceptance contract for this task."
			},
			checked: {
				type: "array",
				items: { type: "string" },
				description: "What this task verifies or covers."
			},
			unchecked: {
				type: "array",
				items: { type: "string" },
				description: "Known scope not verified by this task."
			},
			over_budget_decision_impact: {
				type: "string",
				description: "Required auditable explanation when a main task exceeds the decision-mode budget."
			},
			max_investigations: {
				type: "number",
				description: "Bounded follow-up investigations within this task (default 2); only meaningful with acceptance."
			}
		},
		output: {
			schema: {
		...
		async execute(args, exec) {
			...
			const created = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (args.assignee !== void 0 && args.assignee !== "captain") requireMember(fresh, args.assignee);
				assertAssigneeIsNotCommissar(fresh, args.assignee, "creating a task");
				const impact = args.impact === "changes-decision" || args.impact === "blocks-execution" || args.impact === "evidence-quality" || args.impact === "maintenance" ? args.impact : "blocks-execution";
				const normalizedDeliverable = typeof args.deliverable === "string" ? args.deliverable.trim() : void 0;
				const outputKey = deliverableKey(normalizedDeliverable);
				if (normalizedDeliverable !== void 0 && outputKey === void 0) throw new Error(`deliverable must be an exact workspace-relative file path, got ${JSON.stringify(args.deliverable)}`);
				if (outputKey !== void 0 && fresh.tasks.some((existing) => !TERMINAL_TASK_STATUSES$1.includes(existing.status) && existing.deliverableKey === outputKey)) throw new Error(`deliverable "${args.deliverable}" already has an active writer`);
				...
				if (impact !== "maintenance") {
					...
					if (maintenancePath !== void 0) throw new Error(`main-chain task cannot depend on maintenance task path ${maintenancePath.join(" → ")}`);
					...
					if (decisionMode && usedBefore >= budget && exception === void 0) throw new Error(`main-chain task budget exhausted (${usedBefore}/${budget}); provide over_budget_decision_impact or create maintenance work`);
					if (decisionMode && usedBefore >= budget && exception !== void 0) budgetException = {
						decisionImpact: exception,
						grantedAt: Date.now()
					};
				...
				if (reviewRequired && teamMode(fresh) === "light") throw new Error(`team "${fresh.name}" is in light mode and cannot create a task that needs commissar review (risk=${args.risk ?? "-"}${milestone ? ", milestone" : ""}). Upgrade the team with agent_teams_set_mode(mode="standard"|"governed") first, or create the task without risk=high/critical and milestone=true.`);
				if (reviewRequired && !fresh.members.some((member) => isActiveCommissar(member))) throw new Error(`team "${fresh.name}" has no active commissar, so a task that needs review cannot be created (risk=${args.risk ?? "-"}${milestone ? ", milestone" : ""}). Add the commissar back (agent_teams_set_mode) or create the task without risk=high/critical and milestone=true.`);
				if (args.max_investigations !== void 0 && (!Number.isSafeInteger(args.max_investigations) || args.max_investigations < 0 || args.max_investigations > 10)) throw new Error("max_investigations must be an integer between 0 and 10");
				const task = {
					...
					...outputKey !== void 0 ? {
						...
						deliverableKey: outputKey
					} : {},
					...typeof args.acceptance === "string" && args.acceptance.trim() !== "" ? {
						acceptance: args.acceptance.trim(),
					...
					...typeof args.acceptance === "string" && args.acceptance.trim() !== "" ? { scopeContract: {
						...
						...normalizedDeliverable === void 0 ? {} : { originalDeliverable: normalizedDeliverable },
					...
					...budgetException !== void 0 ? { budgetException } : {},
					contractRevision: 0,
					createdAt: Date.now(),
					updatedAt: Date.now()
				};
	...
	scopeCtx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: `Task ${value.task_id} discovery ${value.decision}; awaiting captain: ${value.awaiting_captain}`
			}]
		},
		async execute(args, exec) {
			const captain = requireCaptain(exec);
			...
			return withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (scope === void 0) throw new Error(`task ${task.id} is legacy and has no frozen scope contract`);
				if (TERMINAL_TASK_STATUSES$1.includes(task.status)) throw new Error(`terminal task ${task.id} cannot be triaged`);
				const finding = scope.findings[args.finding_index];
				if (!Number.isSafeInteger(args.finding_index) || finding === void 0 || finding.disposition !== "pending-captain") throw new Error("finding index must identify a pending captain decision");
				if (args.reason.trim() === "") throw new Error("scope triage reason must not be empty");
				if (args.decision === "approve" && finding.relation !== "blocks-acceptance" && finding.relation !== "data-loss" && finding.relation !== "security") throw new Error("only an acceptance blocker, data loss or security finding may be approved in this task");
				const findings = scope.findings.map((item, index) => index === args.finding_index ? {
					...item,
					disposition: args.decision === "approve" ? "approved" : "deferred",
					evidence: `${item.evidence}\n队长裁决: ${args.reason.trim()}`
				} : item);
				task.scopeContract = {
					...scope,
					findings,
					awaitingCaptain: scope.investigationsUsed >= scope.maxInvestigations || findings.some((item) => item.disposition === "pending-captain")
				};
				task.updatedAt = Date.now();
				await writeTeam(stateRoot, fresh);
				return {
					...
					awaiting_captain: task.scopeContract.awaitingCaptain
				};
			});
		}
	}));
	scopeCtx.tools.register(defineTool({
		...
		parameters: {
			...
			reason: {
				type: "string",
				description: "Why the task is being retried or reassigned; legacy callers may omit it."
			},
			acceptance: {
				type: "string",
				description: "Optional revised acceptance contract; revision is audited in place."
			}
		},
		output: {
			schema: {
		...
		async execute(args, exec) {
			...
			if (args.acceptance !== void 0 && (args.acceptance?.trim() === "" || args.reason?.trim() === void 0 || args.reason.trim() === "")) throw new Error("reassigning acceptance requires non-empty acceptance and reason");
			...
			const revoked = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (args.acceptance !== void 0 && args.acceptance.trim() !== "") {
					const prior = task.acceptance;
					const nextRevision = (task.acceptanceRevision ?? 0) + 1;
					task.acceptanceHistory = [...task.acceptanceHistory ?? [], {
						revision: task.acceptanceRevision ?? 0,
						acceptance: prior,
	...
	scopeCtx.tools.register(defineTool({
		...
		parameters: {
			...
			assignee: {
				type: "string",
				description: "Member to claim for (captain only; defaults to the task's assignee); \"captain\" claims it for the captain."
			}
		},
		output: {
			schema: {
		...
		async execute(args, exec) {
			...
			return withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				if (identity.kind === "captain") {
					if (args.assignee !== void 0 && args.assignee !== "captain") {
						requireMember(fresh, args.assignee);
						assignee = args.assignee;
					} else if (args.assignee === "captain") assignee = CAPTAIN_KEY;
				} else {
				...
				if (task.scopeContract?.awaitingCaptain === true) throw new Error(`task ${task.id} awaits captain scope decision and cannot be claimed`);
				if (taskAwaitingInput(task)) {
					if (identity.kind !== "captain") throw new Error(`task ${task.id} is awaiting input (待输入) — the captain must answer the pending question first (update_task with input_answered=true) before a member can claim it`);
	...
	scopeCtx.tools.register(defineTool({
		...
		parameters: {
			...
			finding: {
				type: "string",
				description: "A new discovery; report before acting on it."
			},
			finding_evidence: {
				type: "string",
				description: "Concrete evidence for the discovery."
			},
			finding_relation: {
				type: "string",
				enum: [
				...
				description: "Relation to the original acceptance; classification never auto-approves expansion."
			},
			original_acceptance_met: {
				type: "boolean",
				description: "Owner reports original acceptance is met; completion then stops scope expansion."
			},
			input_answered: {
				type: "boolean",
		...
		async execute(args, exec) {
			...
			const updated = await withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				const scope = task.scopeContract;
				if (scope !== void 0) {
					if (scope.awaitingCaptain && args.status !== "failed" && args.status !== "cancelled") throw new Error(`task ${task.id} awaits captain scope decision; no further work or completion before captain triage`);
					if (args.finding !== void 0) {
						...
						if (args.finding.trim() === "" || args.finding_evidence?.trim() === "" || args.finding_evidence === void 0 || args.finding_relation === void 0) throw new Error("finding requires nonempty summary, evidence and relation");
						...
						task.scopeContract = {
							...
							findings: [...scope.findings, {
								summary: args.finding.trim(),
								evidence: args.finding_evidence.trim(),
								relation,
					...
					if (args.original_acceptance_met === true) task.scopeContract = {
						...scope,
						originalAcceptanceMet: true
					};
					if (args.status === "completed" && task.scopeContract?.originalAcceptanceMet !== true) throw new Error(`task ${task.id} must confirm original_acceptance_met before completion`);
				}
				if (args.status === "completed" && gateBlocksCompletion(task)) {
					task.blockedByReview = true;
					const outputSaved = args.output !== void 0 && args.output !== task.output;
				...
				if (TERMINAL_TASK_STATUSES$1.includes(task.status)) {
					...
					if (task.retro === void 0 && task.actualMs !== void 0 && task.claimedAt !== void 0) {
						task.retro = buildTaskRetro({
							...
							causeSource: args.retro_cause === void 0 ? "auto" : identity.kind === "captain" ? "captain" : "member"
						}, args.retro_cause);
			...
			if (updated.kind === "gate-blocked") {
				const saved = updated.outputSaved ? " Your submitted output WAS saved on the task, so you do not need to resend it." : "";
		...
		async execute(_args, exec) {
			...
			const tasks = team.tasks.map((task) => {
				...
				return {
					...
					...task.budgetException !== void 0 ? { budget_exception: {
						decision_impact: task.budgetException.decisionImpact,
						granted_at: task.budgetException.grantedAt
					} } : {},
					...task.deliverable !== void 0 ? {
						deliverable: task.deliverable,
						deliverable_key: task.deliverableKey ?? task.deliverable.toLowerCase()
					} : {},
					...task.acceptance !== void 0 ? {
						acceptance: task.acceptance,
						acceptance_revision: task.acceptanceRevision ?? 0
					} : {},
					verification: task.verification === void 0 ? {
						checked: [],
						unchecked: []
					} : {
						checked: task.verification.checked,
						unchecked: task.verification.unchecked
					},
					...task.contractRevision !== void 0 ? { contract_revision: task.contractRevision } : {},
					...task.acceptanceHistory !== void 0 ? { acceptance_history: task.acceptanceHistory.map((entry) => ({
						revision: entry.revision,
						...entry.acceptance !== void 0 ? { acceptance: entry.acceptance } : {},
						reason: entry.reason,
						changed_at: entry.changedAt
					})) } : {},
					...task.reviewHistory !== void 0 ? { review_history: task.reviewHistory.map((entry) => ({
						reviewer_name: entry.reviewerName,
						verdict: entry.verdict,
						...entry.comment !== void 0 ? { comment: entry.comment } : {},
						reviewed_at: entry.reviewedAt,
						...entry.attempt !== void 0 ? { attempt: entry.attempt } : {},
						...entry.contractRevision !== void 0 ? { contract_revision: entry.contractRevision } : {}
					})) } : {},
					...
					...task.review === void 0 ? {} : { review: {
						...
						verdict: task.review.verdict,
						...task.review.attempt !== void 0 ? { attempt: task.review.attempt } : {},
						...task.review.contractRevision !== void 0 ? { contract_revision: task.review.contractRevision } : {},
						...task.review.comment !== void 0 ? { comment: task.review.comment } : {},
						reviewed_at: task.review.reviewedAt
					} },
		...
		async execute(args, exec) {
			...
			return withTeamLock(teamLockKey$1(stateRoot, team.id), async () => {
				...
				task.retro = {
					...
					...args.verdict === "revised" ? { causeSource: "captain" } : {}
				};
				task.updatedAt = Date.now();
				await writeTeam(stateRoot, fresh);
				const retro = task.retro;
				...
				await mutateBestPractices(stateRoot, (library) => {
					...
					if (entryIndex >= 0) {
						if (library[entryIndex].disabledAt !== void 0 && args.verdict !== "useless") return;
...
*/
const AGENT_TEAMS_USAGE_SECTION = "agent-teams:usage";
/** Name of the always-on activation tool. */
const AGENT_TEAMS_ACTIVATION_TOOL = "agent_teams_activate";
...
function activationHintText() {
	return ["AgentTeams (multi-agent teamwork: you become the captain of a team of durable members sharing a task board) is installed but NOT loaded — its tool set and the full captain protocol load on demand, so ordinary sessions do not pay for them.", "When the user asks for AgentTeams (e.g. \"use AgentTeams to do X\") or wants multi-agent / orchestrated work, call agent_teams_activate first; a `/agent-teams <goal>` message activates the same surface. Activation is per session and idempotent, and the agent_teams_* tools plus the full protocol appear from your next step — never call agent_teams_* before activating."].join("\n");
}
/**
...
*/
const activatedAgents = /* @__PURE__ */ new WeakSet();
/**
...
*/
function activateAgentTeams(agent, config, sectionOrder, runtime) {
...
* @param runtime - process-wide runtime from {@link installAgentTeamsRuntime}.
*/
function registerAgentTeamsSurface(ctx, config, sectionOrder, runtime) {
...
* @param runtime - process-wide runtime installed once at plugin mount.
*/
function registerAgentTeamsActivation(ctx, config, sectionOrder, runtime) {
	ctx.systemPrompt.section({
		name: AGENT_TEAMS_USAGE_SECTION,
		order: sectionOrder,
	...
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: value.activated ? `AgentTeams activated: the ${value.tool_count} agent_teams_* tools and the full captain protocol are now installed for this session and appear from your next step. Re-read the AgentTeams section of your instructions, then lead: create the team with agent_teams_create, add the members the goal needs, break the goal into tasks with dependencies, and drive it by delegation.` : `AgentTeams was already active in this session (${value.tool_count} tools installed). Continue as captain; do not create a second team for the same goal.`
			}]
		},
		async execute(_args, exec) {
			const agent = exec.agent;
			if (agent === void 0) throw new Error(`${AGENT_TEAMS_ACTIVATION_TOOL} requires a calling agent (exec.agent was undefined)`);
			return {
				activated: activateAgentTeams(agent, config, sectionOrder, runtime),
...
function buildActivationDirective(goal) {
	return ["The user invoked the `/agent-teams` command. The AgentTeams tools and the full captain protocol are now loaded for this session: you are the captain of a multi-agent team.", goal === "" ? "The goal was not given — ask the user what the team should accomplish." : `Goal: ${goal}`].join("\n");
}
/**
...
*/
function registerAgentTeamsCommand(ctx, onActivate) {
...
*/
function installAgentTeamsGestureBoundary(ctx, onActivate) {
...
//#endregion
//#region src/workspace-identity.ts
/** Canonical workspace identity helpers shared by host snapshot and close. */
async function canonicalWorkspaceId(stateRoot) {
...
async function handleCloseTeam(ctx, config, workspaceRegistry, req, res, auth) {
	...
	if (workspaceId !== void 0 && (typeof workspaceId !== "string" || workspaceId === "")) {
		sendJson(res, 400, {
			ok: false,
			reason: "workspaceId must be a non-empty canonical token"
		});
		return;
	}
	...
	const requestedRoot = workspaceId === void 0 ? void 0 : canonicalRoots.find((root) => root.workspaceId === workspaceId);
	if (workspaceId !== void 0 && requestedRoot === void 0) {
		sendJson(res, 403, {
			ok: false,
			reason: "workspaceId does not match a registered canonical workspace"
		});
		return;
	}
	let located;
	try {
		located = await locateTeam(requestedRoot === void 0 ? canonicalRoots : [requestedRoot], teamId);
	} catch {
		sendJson(res, 400, {
			ok: false,
...
//#endregion
//#region src/provider-grant-route.ts
/** Model-grant 请求体上限(与 close 路由同量级,防内存放大)。 */
const PROVIDER_GRANT_BODY_CAP_BYTES = 16 * 1024;
/**
...
*   pass the Host fence and present the boot token before any body is read.
*/
async function handleProviderGrant(access, req, res, auth) {
	if (auth === void 0 || !webRequestAuthorized(req, auth.token, auth.trustedHosts)) {
	...
	let payload;
	try {
		payload = await readJsonBody(req, PROVIDER_GRANT_BODY_CAP_BYTES);
	} catch {
	...
	if (access.setModelGrant === void 0) {
...
//#endregion
//#region src/practices-route.ts
var PracticeError = class extends Error {
	status;
...
const validConditions = (value) => Array.isArray(value) && value.length <= 20 && value.every((item) => validText(item, 200));
const validCounterexamples = (value) => Array.isArray(value) && value.length <= 20 && value.every((item) => isObject(item) && Object.keys(item).every((key) => key === "context" || key === "reason") && validText(item.context, 200) && validText(item.reason, 500));
...
function parsePracticeMutation(input) {
	if (!isObject(input) || Object.keys(input).some((key) => ![
		"workspace",
		"id",
		"expectedRevision",
		"action",
		"patch",
		"reason"
	].includes(key)) || !validText(input.workspace, 4096) || !isAbsolute(input.workspace) || !validText(input.id, 128) || !isTimestamp(input.expectedRevision) || ![
		"edit",
		"disable",
		"restore"
	].includes(String(input.action))) throw new PracticeError(400, "invalid practice mutation");
	if (input.action === "edit") {
		if (!isObject(input.patch) || Object.keys(input.patch).length === 0 || Object.keys(input.patch).some((key) => !patchKeys.has(key))) throw new PracticeError(400, "invalid practice patch");
		const patch = input.patch;
		if (patch.practice !== void 0 && !validText(patch.practice, 4e3) || patch.appliesWhen !== void 0 && !validConditions(patch.appliesWhen) || patch.counterexamples !== void 0 && !validCounterexamples(patch.counterexamples) || patch.expiresAt !== void 0 && patch.expiresAt !== null && !isTimestamp(patch.expiresAt)) throw new PracticeError(400, "invalid practice patch fields");
	} else if (input.patch !== void 0) throw new PracticeError(400, "patch only allowed for edit");
...
function practiceView(entry) {
	return {
		id: entry.id,
		sourceTeamId: entry.sourceTeamId,
		sourceTaskId: entry.sourceTaskId,
		sourceTaskSubject: entry.sourceTaskSubject,
		role: entry.role,
		practice: entry.practice,
		verdict: entry.verdict,
...
function applyPracticeMutation(entry, mutation, now, actor = "local-browser-operator") {
	...
	if (mutation.action === "edit") {
		...
		if (patch.practice !== void 0) changed.practice = patch.practice.trim();
		if (patch.appliesWhen !== void 0) changed.appliesWhen = patch.appliesWhen;
		if (patch.counterexamples !== void 0) changed.counterexamples = patch.counterexamples;
		if (patch.expiresAt !== void 0) changed.expiresAt = patch.expiresAt === null ? void 0 : patch.expiresAt;
		if (Object.keys(changed).some((key) => key !== "expiresAt")) changed.verdict = "pending";
	} else if (mutation.action === "disable") {
		if (entry.disabledAt !== void 0) throw new PracticeError(409, "already disabled");
		changed.disabledAt = now;
		changed.disabledReason = mutation.reason;
	} else {
		if (entry.disabledAt === void 0) throw new PracticeError(409, "not disabled");
		changed.disabledAt = void 0;
		changed.disabledReason = void 0;
...
async function handlePractices(registry, stateDir, req, res, auth) {
	...
	if (admission === void 0 || !("peer" in admission) || admission.peer !== auth.connection?.operator) {
		send(res, "rejection" in (admission ?? {}) ? admission.rejection : 403, { error: "operator authentication required" });
		return;
	}
	const operator = admission.peer;
	const workspaces = registry.list().map((row) => ({
		path: resolve(row.path),
		title: row.title
	}));
	const locate = (path) => workspaces.find((row) => row.path === path)?.path;
	try {
		if (req.method === "GET") {
			const requested = new URL(req.url ?? "/", "http://localhost").searchParams.get("workspace");
			if (requested === null) {
				send(res, 200, { workspaces });
				return;
			}
			const workspace = locate(requested);
			if (workspace === void 0) throw new PracticeError(403, "unregistered workspace");
			send(res, 200, {
				workspace,
				entries: (await readBestPractices(join(workspace, stateDir))).map(practiceView)
			});
			return;
		}
		...
		send(res, status, { error: status === 500 ? "practice operation failed" : error.message });
	}
}
//#endregion
//#region src/provider-grants.ts
/**
...
const AGENT_TEAM_SETTINGS_NS = settingsNamespace("agent-team-web");
...
* 显式类型注解避免声明发射引用深层 pnpm 路径(TS2742)。 */
const AgentTeamSettingsFields = {
	enabledModels: enabledModelsSchema.volatile(),
	roleDefaults: roleDefaultsSchema.volatile(),
	modelCapabilities: modelCapabilitiesSchema.volatile()
};
z.object({
...
function modelCapabilityFromValue(value, provider, model) {
	const key = modelKey(provider, model);
	const configured = value?.modelCapabilities?.[key];
	if (configured !== void 0) return provider === "deepseek-official" ? {
		...configured,
		enabled: true
	} : configured;
	return {
		enabled: provider === "deepseek-official" || value?.enabledModels?.[key] === true,
		legacy: true
	};
}
function modelGrantedFromValue(value, provider, model) {
...
function settingsAccessFromConfig(config) {
	const enabledModels = () => unwrapVolatile(config.enabledModels) ?? {};
	const roleDefaults = () => unwrapVolatile(config.roleDefaults) ?? {};
	const modelCapabilities = () => unwrapVolatile(config.modelCapabilities) ?? {};
	return {
		modelGrantedFor: (provider, model) => modelGrantedFromValue({
			enabledModels: enabledModels(),
			modelCapabilities: modelCapabilities()
		}, provider, model),
		roleDefaultsFor: (roleKey) => roleDefaults()[roleKey],
		enabledModels,
		roleDefaults,
		modelCapabilities,
		modelCapabilityFor: (provider, model) => modelCapabilityFromValue({
			enabledModels: enabledModels(),
			modelCapabilities: modelCapabilities()
		}, provider, model)
	};
...
function wireAgentTeamSettings(settingsCtx, access) {
	const settings = settingsCtx.settings;
	if (settings === void 0) return;
	access.setModelGrant = async (provider, model, enabled) => {
		...
		await settings.update(String(AGENT_TEAM_SETTINGS_NS), {
	...
	access.setRoleDefault = async (roleKey, value) => {
	...
	const dispose = () => {
		access.setModelGrant = void 0;
		access.setRoleDefault = void 0;
	};
	const effect = settingsCtx.effect;
	if (effect !== void 0) effect(() => dispose);
}
//#endregion
...
function listProvidersSafe(ctx) {
	try {
		const llm = ctx.llm;
		if (llm?.listProviders === void 0) return [];
...
async function listModelsSafe(ctx, providerId) {
	try {
		const llm = ctx.llm;
		if (llm?.listModels === void 0) return [];
		const models = await llm.listModels(providerId);
		return Promise.all(models.map(async (model) => {
			try {
				const reasoning = (await llm.resolveModelInfo?.(providerId, model.id))?.reasoning;
				return {
					id: model.id,
...
*/
async function collectProviders(ctx, enabledModels) {
	...
	for (const provider of providers) {
		...
		const grantedAny = provider.id === "deepseek-official" || models.some((model) => enabledMap[`${provider.id}/${model.id}`] === true) || Object.keys(enabledMap).some((key) => key.startsWith(`${provider.id}/`) && enabledMap[key] === true);
		views.push({
			id: provider.id,
			name: provider.name,
			enabled: grantedAny,
...
async function assembleTeamSnapshot(ctx, stateRoot, workspace, state, options = {}) {
	...
	const mapMember = (member) => {
		...
		return {
			...
			...member.provider !== void 0 && member.provider !== "" ? { provider: member.provider } : {},
			...member.model !== void 0 && member.model !== "" ? { model: member.model } : {},
	...
	const base = {
		workspace,
		workspaceId: options.workspaceId ?? await canonicalWorkspaceId(stateRoot),
		teamId: state.id,
		name: state.name,
		...state.description !== void 0 ? { description: state.description } : {},
		...
		tasks: tasks.map((task) => {
			...
			return {
				...
				budgetFact: (() => {
					const budget = estimateBudgetMs(task.estimateLevel, task.estimatedMs);
					if (budget === void 0 || task.actualMs === void 0) return "unknown";
					return task.actualMs > budget ? "over_budget" : "within_budget";
				})(),
...
*/
async function collectTeamsActivity(ctx, roots, enabledModels) {
	const snapshots = [];
	for (const root of roots) {
		let entries;
		try {
...
*/
async function collectArchivedTeamsActivity(ctx, roots, enabledModels) {
	const snapshots = [];
...
const Config = z.object({
	...
	registration: z.union(["lazy", "eager"]).default("lazy"),
	enabledModels: AgentTeamSettingsFields.enabledModels,
	roleDefaults: AgentTeamSettingsFields.roleDefaults,
	modelCapabilities: AgentTeamSettingsFields.modelCapabilities
});
function apply(ctx, config) {
	...
	const settingsAccess = settingsAccessFromConfig(config);
	ctx.inject(["settings"], (settingsCtx) => {
		wireAgentTeamSettings(settingsCtx, settingsAccess);
	});
	const toolsConfig = {
		...resolved,
		modelGrantedFor: (provider, model) => settingsAccess.modelGrantedFor?.(provider, model) ?? provider === "deepseek-official",
	...
	if (eager) registerAgentTeamsSurface(ctx, toolsConfig, sectionOrder, runtime);
	else registerAgentTeamsActivation(ctx, toolsConfig, sectionOrder, runtime);
	const activateAgent = eager ? () => void 0 : (agent) => {
	...
	const registerWebSurface = () => {
		...
		ctx.effect(() => webServer.register({
			...
			handler: async (req, res) => {
				...
				const snapshots = url.searchParams.get("archived") === "1" ? await collectArchivedTeamsActivity(ctx, roots, settingsAccess.enabledModels) : await collectTeamsActivity(ctx, roots, settingsAccess.enabledModels);
				...
				const roleDefaultsBase = {};
				for (const roleKey of roleKeys) roleDefaultsBase[roleKey] = resolved.roleLlmDefaults?.[roleKey] ?? DEFAULT_ROLE_LLM[roleKey];
				const roleDefaults = roleKeys.map((roleKey) => {
					return {
						role: roleKey,
						...roleOverrides[roleKey] ?? roleDefaultsBase[roleKey] ?? {},
						overridden: roleOverrides[roleKey] !== void 0
					};
				});
