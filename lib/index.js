	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			if (explicitlyRouted && selection.provider !== "deepseek-official") {
				if (!(config.providerGrantedFor?.(selection.provider) ?? selection.provider === "deepseek-official")) {
...
async function handleProviderGrant(access, req, res, auth) {
	...
	if (access.setProviderGrant === void 0) {
...
const AGENT_TEAM_PROVIDERS_NS = settingsNamespace("agent-team-web-providers");
/** 设置页表单 schema：open map —— 任意 provider id → 布尔开关。
* 显式类型注解避免声明发射引用深层 pnpm 路径(TS2742)。 */
const ProviderGrantsSchema = z.object({ enabledProviders: z.dict(z.boolean()).default({}) });
/** 注册 Provider 授权命名空间，返回命名空间 scope（设置页渲染 + spawn 校验共用）。 */
function registerProviderGrantsSettings(sctx) {
	const settings = sctx.settings;
	if (settings === void 0) throw new Error("settings service is not available to register agent-team-web-providers");
...
function grantedFromScope(scope, provider) {
	if (provider === "deepseek-official") return true;
	return scope.get()?.enabledProviders?.[provider] === true;
}
...
* access 读写授权。 */
function wireSettingsGranted(settingsCtx, access) {
	const scope = registerProviderGrantsSettings(settingsCtx);
	access.providerGrantedFor = (provider) => grantedFromScope(scope, provider);
	access.enabledProviders = () => {
		return scope.get()?.enabledProviders ?? {};
	};
	access.setProviderGrant = async (provider, enabled) => {
		if (provider === "deepseek-official") return;
		const current = scope.get();
		await scope.update({ enabledProviders: {
			...current?.enabledProviders ?? {},
			[provider]: enabled
		} });
	};
	const dispose = () => {
		access.providerGrantedFor = void 0;
		access.enabledProviders = void 0;
		access.setProviderGrant = void 0;
	};
...
async function assembleTeamSnapshot(ctx, stateRoot, workspace, state, options = {}) {
	...
	const providers = listProvidersSafe(ctx).map((provider) => ({
		id: provider.id,
		name: provider.name,
...
*/
async function collectTeamsActivity(ctx, roots, enabledProviders) {
	const snapshots = [];
	for (const root of roots) {
		let entries;
		try {
...
*/
async function collectArchivedTeamsActivity(ctx, roots, enabledProviders) {
	const snapshots = [];
...
function apply(ctx, config) {
	...
	registerAgentTeamsTools(ctx, {
		...resolved,
		providerGrantedFor: (provider) => grantsAccess.providerGrantedFor?.(provider) ?? provider === "deepseek-official"
	});
	...
	const registerWebSurface = () => {
		...
		ctx.effect(() => webServer.register({
			...
			handler: async (req, res) => {
				...
				const snapshots = url.searchParams.get("archived") === "1" ? await collectArchivedTeamsActivity(ctx, roots, grantsAccess.enabledProviders) : await collectTeamsActivity(ctx, roots, grantsAccess.enabledProviders);
