window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const historyDiagnostics = /* @__PURE__ */ new Map();
		const diagnosticListeners = /* @__PURE__ */ new Set();
		let diagnosticSnapshot = /* @__PURE__ */ new Map();
		function subscribeHistoryDiagnostics(listener) {
			diagnosticListeners.add(listener);
			return () => {
				diagnosticListeners.delete(listener);
			};
		}
		function getHistoryDiagnosticsSnapshot() {
			return diagnosticSnapshot;
		}
		function publishHistoryDiagnostics() {
			diagnosticSnapshot = new Map(historyDiagnostics);
			for (const listener of diagnosticListeners) listener();
		}
		function recordHistoryDiagnostic(sessionId, diagnostic) {
			const id = sessionId.trim();
			if (id === "") return;
			historyDiagnostics.set(id, diagnostic);
			publishHistoryDiagnostics();
		}
		function clearHistoryDiagnostic(sessionId) {
			const id = sessionId.trim();
		...
		*/
		...
		function updateActivitySnapshots(update) {
			...
			if (next.teams === activitySnapshots.teams && next.archivedTeams === activitySnapshots.archivedTeams && next.connection === activitySnapshots.connection && next.lastSuccessAt === activitySnapshots.lastSuccessAt) return;
		...
		function startActivityPolling(monitorTargets, runtime = {}) {
			...
			const tick = async () => {
				...
				try {
					...
					publishSnapshots({
						archivedTeams: archivedBody.teams,
						connection: "ready",
						lastSuccessAt: now()
					});
		...
		*/
		...
		const zh = {
			...
			"activity.closeConfirm": "确定结束并归档该团队？此操作不可撤销。",
			"activity.closeDisabled": "任务执行中，暂不可关闭",
			"activity.connection.loading": "正在加载活动…",
			"activity.connection.ready": "已连接 · 更新于 {time}",
			"activity.connection.disconnected": "连接中断 · 等待重试",
			"activity.connection.authFailed": "认证失败 · 请重新打开页面",
			"activity.closing": "归档中…",
			"activity.closeError": "归档失败，请重试",
			"activity.empty": "暂无团队活动",
			"deck.now": "现在",
			"deck.nowAria": "当前主要行动",
			"deck.nowEmpty": "等待队长安排下一项行动",
			"deck.nowTask": "{taskId} · {subject}",
			"deck.nowOwner": "{member} 正在处理",
			"deck.nowReady": "可立即领取",
			"deck.taskFlow": "任务流",
			"task.detail.expand": "展开全文",
			"task.detail.collapse": "收起详情",
			"task.detail.copy": "复制详情",
			"task.detail.copied": "已复制",
			"task.detail.copyFailed": "复制失败，请手动选择文本复制",
			"history.diagnostic.legacy": "历史会话格式较旧，无法打开成员记录",
			"history.diagnostic.unavailable": "历史会话暂不可用",
			"history.diagnostic.copy": "复制诊断信息",
			"history.diagnostic.retry": "重试导航",
			"history.diagnostic.copyOk": "已复制",
			"history.diagnostic.copyFail": "复制失败",
			"history.diagnostic.disabledReason": "上次打开失败，点击可重试",
			...
			"settings.agentTeam.title": "AgentTeam",
			"settings.agentTeam.modelGrant": "模型选择",
			"settings.agentTeam.modelSelection": "模型选择",
			"settings.agentTeam.modelSelectionHelp": "允许 Agent 为 Subagent 选择模型：勾选可用模型，并可限制每个模型的最高思考档位。",
			"settings.agentTeam.modelPresetReference": "模型预设参考",
			"settings.agentTeam.modelPresetReferenceHelp": "以下模板仅供查阅，不会更改模型授权或角色路由。",
			"settings.agentTeam.modelPresetReadOnly": "只读参考",
			"settings.agentTeam.roleDutyLink": "职责说明",
			"settings.agentTeam.roleDutyUnavailable": "暂无职责说明。",
			"settings.agentTeam.rolePreset": "角色预设",
			"settings.agentTeam.toggleAria": "授权开关",
			"settings.agentTeam.modelAria": "模型",
			"settings.agentTeam.effortAria": "思考深度",
			"settings.agentTeam.inherit": "继承",
			"settings.agentTeam.unauthorized": "未授权",
			"settings.agentTeam.reset": "恢复默认",
			"settings.agentTeam.applyPreset": "套用预设",
			"settings.agentTeam.presetTitle": "套用预设",
			"settings.agentTeam.presetHelp": "从预定义模板批量写入角色模型与思考深度，应用后仍可继续逐行微调。",
			"settings.agentTeam.presetGroup.ds": "DS 模型为主",
			"settings.agentTeam.presetGroup.gpt": "GPT 模型为主",
			"settings.agentTeam.presetGroup.mixed": "DS + GPT 混编",
			"settings.agentTeam.preset.currentDefault": "当前默认",
			"settings.agentTeam.preset.apply": "应用预设",
			"settings.agentTeam.preset.cancel": "取消",
			"settings.agentTeam.preset.diff": "将变更",
			"settings.agentTeam.viewAria": "查看",
			"settings.agentTeam.viewSteps": "工作方式",
			"settings.agentTeam.viewDeliverable": "交付物",
			"settings.agentTeam.viewRules": "核心准则",
			"settings.agentTeam.viewCurrent": "当前档位",
			"settings.agentTeam.viewEmpty": "该角色暂无职责说明。",
			"settings.agentTeam.modelDefault": "模型默认",
			"settings.agentTeam.close": "关闭",
			"settings.agentTeam.growth": "自成长",
			"settings.agentTeam.growthEmpty": "暂无经验积累——完成任务并校准复盘后，团队经验会自动沉淀到这里。",
			"settings.agentTeam.growthCount": "经验库共 {total} 条 · 已校准 {calibrated}",
			"settings.agentTeam.growthFrom": "来自",
			"settings.agentTeam.growthVerdict.useful": "已确认",
			"settings.agentTeam.growthVerdict.revised": "已修订",
			...
			"retro.budgetFact.unknown": "预算事实：未结算",
			"retro.budgetFact.withinBudget": "预算事实：未超预算",
			...
			"retro.causeSource.unknown": "归因来源未知",
			...
			"team.ended": "已结束",
			"team.mode.light": "轻量",
			"team.mode.standard": "标准",
			"team.mode.governed": "治理",
			"team.mode.aria": "协作模式：{mode}",
			"team.mode.hint.light": "轻量模式：无政委，不允许高风险/里程碑任务",
			"team.mode.hint.standard": "标准模式：政委在岗，高风险/里程碑任务需复核",
			"team.mode.hint.governed": "治理模式：政委在岗且不可移除，高风险/里程碑任务需复核",
		...
		const en = {
			...
			"activity.closeConfirm": "End and archive this team? This cannot be undone.",
			"activity.closeDisabled": "Tasks in progress — close disabled",
			"activity.connection.loading": "Loading activity…",
			"activity.connection.ready": "Connected · updated {time}",
			"activity.connection.disconnected": "Disconnected · retrying",
			"activity.connection.authFailed": "Authentication failed · reopen the page",
			"activity.closing": "Archiving…",
			"activity.closeError": "Archive failed, try again",
			"activity.empty": "No team activity",
			"deck.now": "Now",
			"deck.nowAria": "Primary action now",
			"deck.nowEmpty": "Waiting for the captain to set the next action",
			"deck.nowTask": "{taskId} · {subject}",
			"deck.nowOwner": "{member} is working on it",
			"deck.nowReady": "Ready to claim",
			"deck.taskFlow": "Task flow",
			"task.detail.expand": "Expand details",
			"task.detail.collapse": "Collapse details",
			"task.detail.copy": "Copy details",
			"task.detail.copied": "Copied",
			"task.detail.copyFailed": "Could not copy — select the text manually",
			"history.diagnostic.legacy": "This history uses an older session format; member records cannot open",
			"history.diagnostic.unavailable": "This history session is unavailable",
			"history.diagnostic.copy": "Copy diagnostic",
			"history.diagnostic.retry": "Retry navigation",
			"history.diagnostic.copyOk": "Copied",
			"history.diagnostic.copyFail": "Copy failed",
			"history.diagnostic.disabledReason": "Last open failed — click to retry",
			"format.listSeparator": ", ",
			...
			"retro.budgetFact.unknown": "Budget fact: not settled",
			"retro.budgetFact.withinBudget": "Budget fact: within budget",
			"retro.budgetFact.overBudget": "Budget fact: over budget",
			"retro.causeSource.auto": "Automatic analysis",
			"retro.causeSource.member": "Member attribution",
			"retro.causeSource.captain": "Captain calibration",
			"retro.causeSource.unknown": "Attribution source unknown",
			...
			"team.ended": "Ended",
			"team.mode.light": "Light",
			"team.mode.standard": "Standard",
			"team.mode.governed": "Governed",
			"team.mode.aria": "Collaboration mode: {mode}",
			"team.mode.hint.light": "Light: no commissar; high-risk/milestone tasks are refused",
			"team.mode.hint.standard": "Standard: commissar on duty; high-risk/milestone tasks need review",
			"team.mode.hint.governed": "Governed: commissar on duty and cannot be removed; high-risk/milestone tasks need review",
			...
			"settings.agentTeam.title": "AgentTeam",
			"settings.agentTeam.modelGrant": "Model selection",
			"settings.agentTeam.modelSelection": "Model selection",
			"settings.agentTeam.modelSelectionHelp": "Allow Agents to choose models for Subagents: enable available models and optionally limit each model’s maximum reasoning effort.",
			"settings.agentTeam.modelPresetReference": "Model preset reference",
			"settings.agentTeam.modelPresetReferenceHelp": "Templates below are for reference only; they do not change model grants or role routing.",
			"settings.agentTeam.modelPresetReadOnly": "Read-only reference",
			"settings.agentTeam.roleDutyLink": "Role duties",
			"settings.agentTeam.roleDutyUnavailable": "No duty description is available.",
			"settings.agentTeam.rolePreset": "Role presets",
			"settings.agentTeam.toggleAria": "authorization switch",
			"settings.agentTeam.modelAria": "model",
			"settings.agentTeam.effortAria": "reasoning effort",
			"settings.agentTeam.inherit": "Inherit",
			"settings.agentTeam.unauthorized": "unauthorized",
			"settings.agentTeam.reset": "Reset to defaults",
			"settings.agentTeam.applyPreset": "Apply preset",
			"settings.agentTeam.presetTitle": "Apply preset",
			"settings.agentTeam.presetHelp": "Bulk-apply one predefined role routing template, then keep fine-tuning individual rows if needed.",
			"settings.agentTeam.presetGroup.ds": "DS-first",
			"settings.agentTeam.presetGroup.gpt": "GPT-first",
			"settings.agentTeam.presetGroup.mixed": "DS + GPT mixed",
			"settings.agentTeam.preset.currentDefault": "Current default",
			"settings.agentTeam.preset.apply": "Apply preset",
			"settings.agentTeam.preset.cancel": "Cancel",
			"settings.agentTeam.preset.diff": "Changes",
			"settings.agentTeam.viewAria": "View",
			"settings.agentTeam.viewSteps": "How it works",
			"settings.agentTeam.viewDeliverable": "Deliverable",
			"settings.agentTeam.viewRules": "Key rules",
			"settings.agentTeam.viewCurrent": "Current preset",
			"settings.agentTeam.viewEmpty": "No duty description for this role.",
			"settings.agentTeam.modelDefault": "Model default",
			"settings.agentTeam.close": "Close",
			"settings.agentTeam.growth": "Self-growth",
			"settings.agentTeam.growthEmpty": "No experience yet — complete tasks and calibrate retros; team lessons will accumulate here.",
			"settings.agentTeam.growthCount": "{total} lessons · {calibrated} calibrated",
			"settings.agentTeam.growthFrom": "from",
			"settings.agentTeam.growthVerdict.useful": "confirmed",
			"settings.agentTeam.growthVerdict.revised": "revised",
		...
		* names (`技术员 一号`, legacy `技术员一号`). */
		...
		const ROLE_DUTY = {
			captain: {
				slogan: "负责带团队：建队、拆任务、调度成员、协调门禁、收口汇报——不亲自执行成员任务（委派制）。",
				steps: [
					{
						title: "建队与拆解",
						desc: "创建团队，把目标拆成带依赖的任务，按角色分派给成员。"
					},
					{
						title: "调度与协调",
						desc: "用状态和消息调度成员：派活、转派、处理阻塞、协调成员间协作。"
					},
					{
						title: "门禁与把关",
						desc: "对高风险/关键任务走政委门禁复核；验收成员产出，必要时接管或重派。"
					},
					{
						title: "收口汇报",
						desc: "任务完成后向用户汇报团队成果；需要时复盘沉淀经验。"
					}
				],
				deliverable: "团队成果 + 任务闭环；不替成员写实现（那是执行角色的职责）。",
				rules: ["不亲自执行成员任务——委派给执行角色，保持分工。", "一个队长带一个团队；改动任务归属先 reassign 再接管。"]
			},
			researcher: {
				...
				steps: [
					{
						title: "先读再想",
						desc: "下结论前先读相关代码、文档和团队状态，每个判断都要有依据（注明文件路径和关键行）。"
					},
					{
						title: "根因 + 计划",
						desc: "给出问题的根因，然后是具体计划：要改哪些文件、关键实现点、预期效果。是提问就用证据回答。"
					},
					{
						title: "自查后交棒",
						desc: "对照证据复查计划是否成立，标注假设和风险，然后交给队长或技术员。"
					}
				],
				deliverable: "根因分析 + 带依据的实施方案。",
				rules: ["不直接动手实现——实现交给技术员。", "没有把握的地方要说明，不把猜测当结论。"]
			},
			engineer: {
				...
				steps: [
					{
						title: "按方案行事",
						desc: "先读任务描述（和侦察参谋的方案）。方案不清楚就先问，不要猜。"
					},
					{
						title: "实现",
						desc: "用可用工具完成改动，改动范围集中在任务本身，不夹带无关修改。"
					},
					{
						title: "自测",
						desc: "验证自己的改动：typecheck / 测试 / 能跑的直接验证。报告前先修好自己引入的问题。"
					},
					{
						title: "交付说明",
						desc: "汇报改动摘要：改了哪些文件 + 关键决定。偏离方案的地方要明确说明。"
					}
				],
				deliverable: "可运行的实现 + 自测结果 + 改动说明。",
				rules: ["没测试过就不算完成。", "偏离方案必须说明，不能悄悄改道。"]
			},
			qa: {
				...
				steps: [
					{
						title: "先列清单",
						desc: "验收前先从需求整理出具体的检查清单，每一项都可核对。"
					},
					{
						title: "逐项核验",
						desc: "对照清单检查实际产物：运行命令、看输出、查文件片段。"
					},
					{
						title: "带依据的结论",
						desc: "逐项给出通过/不通过 + 依据（命令、输出、摘录）。不通过时列出具体问题。"
					}
				],
				deliverable: "检查清单 + 逐项依据 + 通过/不通过结论。",
				rules: ["不自己动手修——把问题报给负责人处理，保持验收的客观性。", "没有依据不下结论。"]
			},
			designer: {
				slogan: "负责视觉：先给具体设计方案（颜色、尺寸、间距等），再交给技术员实现。",
				steps: [{
					title: "具体方案",
					desc: "产出带具体值的视觉方案：颜色（hex）、间距、字号、排版、文案。不说\"弄得好看点\"这种空话。"
				}, {
					title: "交棒",
					desc: "把方案交给技术员实现。检查视觉成果时，对照方案给出具体意见。"
				}],
				deliverable: "具体的视觉规格（数值 + 理由）。",
				rules: ["不给半成品方向。", "纯视觉检查时，按方案给出通过/不通过 + 依据。"]
			},
			data: {
				...
				steps: [
					{
						title: "先定指标",
						desc: "收集数据前先说明要回答的指标/问题，以及每个指标怎么算。"
					},
					{
						title: "收集",
						desc: "收集数据：测量、计数、样本或代码仓库里的证据——记录方法和来源。"
					},
					{
						title: "可复查报告",
						desc: "产出可复查的报告：指标定义、方法、原始数字、结论——别人能按你的方法复现。"
					}
				],
				deliverable: "指标定义 + 方法 + 原始数字 + 结论。",
				rules: ["不给没依据的数字。", "估算要标注是估算。"]
			},
			docs: {
				...
				steps: [
					{
						title: "先定结构",
						desc: "动笔前先定文档结构（章节、标题、各节写什么）。受众和目的不清楚就先确认。"
					},
					{
						title: "按结构写",
						desc: "按既定结构写：用词一致、给具体例子、不写空话。引用实际的代码/方案/决定（注明路径或键）。"
					},
					{
						title: "同步核对",
						desc: "写完对照当前实现/方案/验证结果核对，发现不一致要标出来。"
					}
				],
				deliverable: "结构清晰、内容准确的文档（设计稿/手册/变更记录等）。",
				rules: ["不编造内容——只写实际存在或已决定的东西。"]
			},
			security: {
				...
				steps: [
					{
						title: "理清信任边界",
						desc: "判断前先弄清楚哪些输入不可信（路由、成员消息、文件路径等），哪些操作是特权（队长工具、状态文件等），各层怎么连。"
					},
					{
						title: "找暴露点",
						desc: "逐条检查每个边界：未授权访问、令牌泄露、路径穿越、注入、权限过宽、密钥暴露。每个发现都要有文件路径和行号。"
					},
					{
						title: "分级 + 场景",
						desc: "每个问题给严重程度（高/中/低）+ 具体被利用的场景 + 修复建议。区分真问题和建议性防护。"
					},
					{
						title: "确认安全面",
						desc: "同时确认哪些防护是有效的（运行时复查、令牌比对、路径清理等），让报告客观平衡。"
					}
				],
				deliverable: "按严重程度分级的问题清单（问题 + 位置 + 利用场景 + 修复建议）+ 确认有效的防护清单。",
				rules: ["不自己修——报告给队长决定。", "每个发现都要有文件路径和行号。"]
			},
			reviewer: {
				slogan: "负责审查：按验收标准检查交付物，给出通过/不通过的结论和需要的修改。",
				steps: [{
					title: "对照标准",
					desc: "针对分配给你的交付物，对照需求/验收标准逐项检查。"
				}, {
					title: "带依据的结论",
					desc: "给出通过/不通过 + 依据：查了什么、什么通过、什么有问题、需要怎么改。"
				}],
				deliverable: "结论 + 依据 + 需要的修改项。",
				rules: ["不重写别人的工作——负责人根据你的意见去改。"]
			},
			commissar: {
				...
				steps: [
					{
						title: "盯目标与风险",
						desc: "检查方案和任务拆解是否始终围绕团队目标，发现风险及时提示。"
					},
					{
						title: "门禁把关",
						desc: "对高风险/关键节点任务把关：用 agent_teams_review_task 给通过/不通过，必须附依据（复核意见）。"
					},
					{
						title: "上报分歧",
						desc: "有争议或担忧上报队长。保持独立，不受队长任务分派影响。"
					}
				],
				deliverable: "监督结论（通过/不通过 + 依据）+ 需要时的上报。",
				rules: ["不执行任务本身——政委是监督角色。", "不审查自己参与过的任务。"]
			}
		};
		//#endregion
		...
		*/
		...
		function budgetFactText(task, t) {
			if (task.budgetFact === void 0) return null;
			return t(task.budgetFact === "over_budget" ? "retro.budgetFact.overBudget" : task.budgetFact === "within_budget" ? "retro.budgetFact.withinBudget" : "retro.budgetFact.unknown");
		}
		...
		function retroDetailText(task, t) {
			...
			const sourceKey = retro.causeSource === "member" ? "retro.causeSource.member" : retro.causeSource === "captain" ? "retro.causeSource.captain" : retro.causeSource === "auto" ? "retro.causeSource.auto" : retro.causeSource === "unknown" ? "retro.causeSource.unknown" : void 0;
			const parts = [t("retro.causeLabel", { cause: sourceKey === void 0 ? retroCauseLabel(retro.cause, t) : `${t(sourceKey)} · ${retroCauseLabel(retro.cause, t)}` })];
		...
		*/
		...
		const cssText$2 = ".UXHWJq_root{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:10px;flex-direction:column;gap:8px;width:100%;min-width:0;padding:10px 12px;display:flex}.UXHWJq_head{align-items:center;gap:8px;min-width:0;display:flex}.UXHWJq_leadAvatar{object-fit:contain;filter:drop-shadow(0 1px 1px #122d4833);background:0 0;border:0;border-radius:0;flex:none;width:30px;height:30px}.UXHWJq_teamName{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:0 auto;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.UXHWJq_memberCount{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;margin-left:auto;font-size:11px;line-height:16px}.UXHWJq_panelButton{border:1px solid var(--dsw-alias-line-strong);background:var(--dsw-alias-bg-module);color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;flex:none;padding:2px 8px;font-size:10.5px;font-weight:600;line-height:16px;transition:border-color .12s,color .12s}.UXHWJq_panelButton:hover{border-color:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-state-business-primary)}.UXHWJq_panelButton:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.UXHWJq_members{flex-wrap:wrap;gap:6px;min-width:0;display:flex}.UXHWJq_member{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);max-width:160px;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;align-items:center;gap:5px;padding:3px 8px 3px 3px;font-size:11px;font-weight:500;line-height:16px;transition:border-color .12s,background-color .12s;display:inline-flex}.UXHWJq_member:hover{border-color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-bg-fill-neutral)}.UXHWJq_member:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.UXHWJq_memberArt{object-fit:contain;filter:drop-shadow(0 1px 1px #122d482e);background:0 0;border:0;border-radius:0;width:24px;height:24px}.UXHWJq_memberInitial{background:var(--dsw-alias-bg-fill-business);width:20px;height:20px;color:var(--dsw-alias-label-on-fill);border-radius:50%;justify-content:center;align-items:center;font-size:10px;font-weight:600;line-height:20px;display:inline-flex}.UXHWJq_memberName{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}";
		...
		var AgentTeamsCard_module_css_default = {
			"head": "UXHWJq_head",
			"leadAvatar": "UXHWJq_leadAvatar",
			"member": "UXHWJq_member",
			"memberArt": "UXHWJq_memberArt",
			"memberCount": "UXHWJq_memberCount",
			"memberInitial": "UXHWJq_memberInitial",
			"memberName": "UXHWJq_memberName",
			"members": "UXHWJq_members",
			"panelButton": "UXHWJq_panelButton",
			"root": "UXHWJq_root",
			"teamName": "UXHWJq_teamName"
		};
		//#endregion
		...
		* (historical session review). */
		...
		function AgentTeamsCard({ node, openMember, t }) {
			...
			(0, react.useEffect)(() => {
				return owner === "" ? void 0 : monitorAgentTeam(owner, data.teamId);
			}, [data.teamId, owner]);
			const snapshot = owner === "" ? void 0 : teams.find((team) => team.teamId === data.teamId && team.captainSessionId === owner) ?? archivedTeams.find((team) => team.teamId === data.teamId && team.captainSessionId === owner);
		...
		//#endregion
		//#region src/client/task-detail-copy.ts
		/** Build deterministic plain text for task-detail clipboard actions. */
		function taskDetailClipboardText(task, t) {
			return [
				task.subject,
		...
		async function copyTaskDetail(text, clipboard) {
			if (clipboard === void 0) return "failed";
			try {
		...
		//#endregion
		//#region src/client/diagnostic-copy.ts
		async function copyDiagnosticText(text, clipboard = typeof navigator === "undefined" ? void 0 : navigator.clipboard) {
			if (clipboard === void 0) return "failed";
			try {
				await clipboard.writeText(text);
				return "copied";
		...
		//#endregion
		//#region src/client/session-diagnostics.ts
		...
		function redactDiagnosticText(value) {
			return value.replace(ABSOLUTE_PATH, "[path]").replace(TOKEN_RUN, "[token]").replace(/\s+/g, " ").trim().slice(0, 240);
		}
		function sessionReference(sessionId) {
			const value = sessionId.trim();
			return value === "" ? "unknown" : `${value.slice(0, 8)}…`;
		}
		function classifySessionFailure(error, options) {
			const detail = error instanceof Error ? error.message : String(error);
			const state = LEGACY_CUES.test(detail) ? "legacy-session-events" : "unavailable";
			return {
				state,
		...
		function relevantDiagnostic(teams, diagnostics, _currentSessionId) {
			for (const team of teams) for (const member of team.members) {
				const diagnostic = diagnostics.get(member.id);
				if (diagnostic !== void 0) return diagnostic;
			}
		}
		function diagnosticBannerText(diagnostic, t) {
			return t(diagnostic.state === "legacy-session-events" ? "history.diagnostic.legacy" : "history.diagnostic.unavailable");
		}
		function diagnosticClipboardText(diagnostic) {
			return [
				diagnostic.code,
				diagnostic.state,
				diagnostic.sessionRef,
				diagnostic.detail ?? ""
			].join(" · ");
		}
		/**
		...
		*/
		function navigationPanelEffect(opened) {
			return opened ? {
				hidePanel: true,
				keepRetry: false
			} : {
		...
		const PANEL_DISMISSAL_STORAGE_KEY = "agent-team-web:activity-panel:dismissal:v1";
		...
		*/
		...
		const cssText$1 = "html{--agent-team-web-panel-shift:420px}html[data-agent-team-web-panel-open] div[data-phase]{box-sizing:border-box;padding-right:var(--agent-team-web-panel-shift)}.I7RNMW_badge,.I7RNMW_panel{--at-bg-panel:#fff;--at-bg-raised:#fafbfc;--at-bg-fill:#f4f6f9;--at-border:#e8ebf0;--at-border-strong:#d9dee8;--at-text-primary:#1b1f27;--at-text-secondary:#5a6474;--at-text-tertiary:#8f97a6;--at-accent:#2d5ca8;--at-accent-hover:#3568b8;--at-accent-strong:#244a8a;--at-success:#15875a;--at-warning:#a4681a;--at-danger:#c23b42;--at-on-accent:#fff;--at-shadow-badge:0 8px 24px #17203014;--at-shadow-panel:0 12px 32px #17203012, 0 32px 72px #1720301c;--at-shadow-active:0 16px 40px #1720301a, 0 36px 80px #17203024;--dsw-alias-bg-module:var(--at-bg-panel);--dsw-alias-bg-module-platform:var(--at-bg-raised);--dsw-alias-bg-fill-neutral:var(--at-bg-fill);--dsw-alias-bg-fill-business:var(--at-accent);--dsw-alias-bg-fill-success:var(--at-success);--dsw-alias-bg-fill-warning:var(--at-warning);--dsw-alias-bg-fill-danger:var(--at-danger);--dsw-alias-state-business-primary:var(--at-accent);--dsw-alias-state-success:var(--at-success);--dsw-alias-state-warning:var(--at-warning);--dsw-alias-state-danger:var(--at-danger);--dsw-alias-label-primary:var(--at-text-primary);--dsw-alias-label-secondary:var(--at-text-secondary);--dsw-alias-label-tertiary:var(--at-text-tertiary);--dsw-alias-label-on-fill:var(--at-on-accent);--dsw-alias-line-normal:var(--at-border);--dsw-alias-line-strong:var(--at-border-strong)}@media (prefers-color-scheme:dark){.I7RNMW_badge,.I7RNMW_panel{--at-bg-panel:#1e1f22;--at-bg-raised:#202125;--at-bg-fill:#2a2c30;--at-border:#2e3136;--at-border-strong:#3a3e45;--at-text-primary:#f5f6f8;--at-text-secondary:#b9bdc6;--at-text-tertiary:#8b90a0;--at-accent:#5b8dd9;--at-accent-hover:#6d9be0;--at-accent-strong:#4a7cc9;--at-success:#3ecf7f;--at-warning:#f0a83c;--at-danger:#f0606a;--at-on-accent:#fff;--at-shadow-badge:0 8px 24px #00000073;--at-shadow-panel:0 12px 32px #00000073, 0 32px 72px #0000008c;--at-shadow-active:0 16px 40px #00000080, 0 36px 80px #0009}.I7RNMW_stateArt{filter:drop-shadow(0 0 1px var(--dsw-alias-bg-module)) drop-shadow(0 1px 1px #00000073)}}.I7RNMW_badge{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:color-mix(in srgb, var(--dsw-alias-bg-module-platform) 92%, transparent);backdrop-filter:blur(16px);height:34px;box-shadow:var(--at-shadow-badge);color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;align-items:center;gap:7px;padding:0 12px;font-size:12px;font-weight:600;line-height:20px;transition:border-color .12s,transform .12s;display:inline-flex;position:absolute;top:64px;right:18px}.I7RNMW_badge:hover{border-color:var(--dsw-alias-line-strong);transform:translateY(-1px)}.I7RNMW_badge:active{transform:translateY(0)scale(.97)}.I7RNMW_badge:focus-visible,.I7RNMW_iconButton:focus-visible,.I7RNMW_memberRow:focus-visible,.I7RNMW_membersToggle:focus-visible,.I7RNMW_sectionToggleTitle:focus-visible,.I7RNMW_historyToggle:focus-visible,.I7RNMW_dagNode:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.I7RNMW_badgeDot,.I7RNMW_panelDot{background:var(--dsw-alias-label-tertiary);border-radius:50%;width:7px;height:7px}.I7RNMW_badgeDot[data-busy=true],.I7RNMW_panelDot[data-busy=true]{background:var(--dsw-alias-state-business-primary);animation:1.25s ease-in-out infinite I7RNMW_agentTeamsPulse}.I7RNMW_badgeCount,.I7RNMW_memberCount,.I7RNMW_teamStats,.I7RNMW_stageLabel,.I7RNMW_taskId{font-variant-numeric:tabular-nums}.I7RNMW_panel{box-sizing:border-box;border:1px solid color-mix(in srgb, var(--dsw-alias-line-strong) 58%, transparent);background:color-mix(in srgb, var(--dsw-alias-bg-module) 95%, transparent);backdrop-filter:blur(20px)saturate(1.08);box-shadow:var(--at-shadow-panel);will-change:transform;border-radius:16px;flex-direction:column;animation:.16s ease-out I7RNMW_agentTeamsPanelIn;display:flex;position:absolute;top:0;left:0;overflow:hidden}.I7RNMW_panel[data-dragging],.I7RNMW_panel[data-resizing]{user-select:none;box-shadow:var(--at-shadow-active)}@keyframes I7RNMW_agentTeamsPanelIn{0%{opacity:0}to{opacity:1}}@keyframes I7RNMW_agentTeamsPulse{0%,to{opacity:.42}50%{opacity:1}}.I7RNMW_panelHead{border-bottom:1px solid var(--dsw-alias-line-normal);cursor:grab;touch-action:none;flex:none;justify-content:space-between;align-items:center;min-height:44px;padding:0 14px;display:flex}.I7RNMW_panelHead:active,.I7RNMW_panel[data-dragging] .I7RNMW_panelHead{cursor:grabbing}.I7RNMW_panel[data-compact] .I7RNMW_panelHead{cursor:default;touch-action:auto}.I7RNMW_panelTitle{color:var(--dsw-alias-label-primary);align-items:center;gap:8px;font-size:14px;font-weight:600;line-height:20px;display:inline-flex}.I7RNMW_panelControls{flex:none;align-items:center;gap:2px;display:inline-flex}.I7RNMW_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:7px;justify-content:center;align-items:center;padding:0;transition:background-color .12s,color .12s,transform .12s;display:inline-flex}.I7RNMW_iconButton:hover{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-primary)}.I7RNMW_iconButton:active{transform:scale(.94)}.I7RNMW_iconButton[data-control=dock][data-mode=docked] svg{transform:scaleX(-1)}.I7RNMW_iconButton[data-control=close]:hover:not(:disabled){background:var(--dsw-alias-bg-fill-danger);color:var(--dsw-alias-label-on-fill)}.I7RNMW_iconButton[data-control=close]:disabled{opacity:.45;cursor:not-allowed}.I7RNMW_closeError{color:var(--dsw-alias-state-danger);border-top:1px solid var(--dsw-alias-line-normal);padding:4px 14px;font-size:12px;line-height:16px}.I7RNMW_resizeHandle{z-index:1;touch-action:none;position:absolute}.I7RNMW_resizeHandle[data-resize-edge=left]{cursor:ew-resize;width:8px;top:44px;bottom:8px;left:0}.I7RNMW_resizeHandle[data-resize-edge=bottom]{cursor:ns-resize;height:8px;bottom:0;left:12px;right:12px}.I7RNMW_resizeHandle[data-resize-edge=corner]{cursor:nwse-resize;width:18px;height:18px;bottom:0;right:0}.I7RNMW_resizeHandle[data-resize-edge=corner]:after{border-right:1px solid var(--dsw-alias-label-tertiary);border-bottom:1px solid var(--dsw-alias-label-tertiary);content:\"\";opacity:.52;width:7px;height:7px;position:absolute;bottom:4px;right:4px}.I7RNMW_teams{overscroll-behavior:contain;scrollbar-color:color-mix(in srgb, var(--dsw-alias-label-tertiary) 28%, transparent) transparent;scrollbar-width:thin;flex-direction:column;min-height:0;display:flex;overflow-y:auto}.I7RNMW_teams::-webkit-scrollbar{width:6px}.I7RNMW_teams::-webkit-scrollbar-track{background:0 0}.I7RNMW_teams::-webkit-scrollbar-thumb{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 28%, transparent);background-clip:padding-box;border:2px solid #0000;border-radius:999px}.I7RNMW_teams:hover::-webkit-scrollbar-thumb{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 44%, transparent);background-clip:padding-box}.I7RNMW_team{border-bottom:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:12px;padding:12px 14px 14px;display:flex}.I7RNMW_team:last-child{border-bottom:0}.I7RNMW_teamHead{align-items:center;gap:10px;min-width:0;display:flex}.I7RNMW_teamName{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:1;font-size:13px;font-weight:600;line-height:18px;overflow:hidden}.I7RNMW_teamStats{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;gap:8px;font-size:10px;line-height:16px;display:inline-flex}.I7RNMW_sectionHead{justify-content:space-between;align-items:center;gap:8px;min-width:0;display:flex}.I7RNMW_sectionTitle{color:var(--dsw-alias-label-secondary);align-items:center;gap:6px;font-size:11px;font-weight:600;line-height:16px;display:inline-flex}.I7RNMW_sectionHint{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}.I7RNMW_delegationSection{flex-direction:column;gap:10px;min-width:0;display:flex}.I7RNMW_captainNode{box-sizing:border-box;border:1px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 32%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 7%, var(--dsw-alias-bg-module));border-radius:10px;grid-template-columns:48px minmax(0,1fr) auto;align-items:center;gap:9px;min-height:56px;padding:6px 10px;display:grid}.I7RNMW_commandLayer{grid-template-columns:1fr;gap:10px;display:grid}.I7RNMW_commandLayer[data-leadership=solo]{grid-template-columns:1fr}.I7RNMW_commissarNode{box-sizing:border-box;border:1px solid color-mix(in srgb, var(--at-accent) 32%, var(--at-border));background:color-mix(in srgb, var(--at-accent) 5%, var(--at-bg-panel));border-radius:10px;grid-template-columns:48px minmax(0,1fr) auto;align-items:center;gap:9px;min-height:56px;padding:6px 10px;display:grid;position:relative}.I7RNMW_commissarNode:before{background:var(--at-accent);content:\"\";border-radius:2px;width:2px;position:absolute;top:8px;bottom:8px;left:0}.I7RNMW_commissarState{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;text-align:right;font-size:10px;line-height:14px;overflow:hidden}.I7RNMW_commissarState[data-activity=working]{color:var(--dsw-alias-state-business-primary);font-weight:600}.I7RNMW_captainAvatar,.I7RNMW_memberAvatar{flex:none;justify-content:center;align-items:center;display:inline-flex;position:relative}.I7RNMW_captainAvatar{width:46px;height:46px}.I7RNMW_leadAvatar,.I7RNMW_memberArt{object-fit:contain;filter:drop-shadow(0 1px 1px #122d4833);background:0 0;border:0;border-radius:0}.I7RNMW_leadAvatar{width:44px;height:44px}.I7RNMW_memberArt{width:40px;height:40px}.I7RNMW_captainInfo,.I7RNMW_memberInfo{flex-direction:column;min-width:0;display:flex}.I7RNMW_captainInfo{gap:2px}.I7RNMW_captainLine,.I7RNMW_memberLine{align-items:center;gap:6px;min-width:0;display:flex}.I7RNMW_captainName,.I7RNMW_memberName{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:12.5px;font-weight:600;line-height:18px;overflow:hidden}.I7RNMW_captainRole,.I7RNMW_memberRole{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}.I7RNMW_memberModel{color:color-mix(in srgb, var(--dsw-alias-label-tertiary) 78%, transparent);text-overflow:ellipsis;white-space:nowrap;flex:none;margin-left:4px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:8px;line-height:12px;overflow:hidden}.I7RNMW_captainSummary,.I7RNMW_memberStatusLine{color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:15px;overflow:hidden}.I7RNMW_memberElapsed{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-secondary);white-space:nowrap;border-radius:4px;flex:none;align-items:center;margin-left:6px;padding:0 4px;font-size:8.5px;font-weight:600;line-height:14px;display:inline-flex}.I7RNMW_memberElapsed[data-timing=warn]{background:color-mix(in srgb, var(--dsw-alias-state-warning) 14%, transparent);color:var(--dsw-alias-state-warning)}.I7RNMW_memberElapsed[data-timing=over]{background:color-mix(in srgb, var(--dsw-alias-state-danger) 14%, transparent);color:var(--dsw-alias-state-danger)}.I7RNMW_captainState,.I7RNMW_memberState{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;align-items:center;gap:5px;font-size:10px;font-weight:500;line-height:15px;display:inline-flex}.I7RNMW_captainState[data-busy=true],.I7RNMW_memberState[data-activity=working]{color:var(--dsw-alias-state-business-primary)}.I7RNMW_workGlyph rect{opacity:.5}.I7RNMW_workGlyph[data-active=true] rect{animation:1.1s ease-in-out infinite I7RNMW_agentTeamsDot}@keyframes I7RNMW_agentTeamsDot{0%,to{opacity:.25}50%{opacity:1}}.I7RNMW_progressOverview{flex-direction:column;gap:7px;display:flex}.I7RNMW_progressTitle{color:var(--dsw-alias-label-secondary);font-size:11px;font-weight:600;line-height:16px}.I7RNMW_progressSegments{gap:3px;display:flex}.I7RNMW_progressSegments>span,.I7RNMW_progressEmpty{background:var(--dsw-alias-line-strong);border-radius:2px;flex:1;height:5px}.I7RNMW_progressEmpty{width:100%;display:block}.I7RNMW_progressSegments>span[data-state=running]{background:var(--dsw-alias-state-business-primary)}.I7RNMW_progressSegments>span[data-state=blocked]{background:var(--dsw-alias-state-warning)}.I7RNMW_progressSegments>span[data-state=completed]{background:var(--dsw-alias-state-success)}.I7RNMW_progressSegments>span[data-state=failed]{background:var(--dsw-alias-state-danger)}.I7RNMW_progressSegments>span[data-state=cancelled]{opacity:.55}.I7RNMW_progressLegend{color:var(--dsw-alias-label-tertiary);gap:10px;font-size:9px;line-height:14px;display:flex}.I7RNMW_progressLegend>span[data-state=running]{color:var(--dsw-alias-state-business-primary)}.I7RNMW_progressLegend>span[data-state=blocked]{color:var(--dsw-alias-state-warning)}.I7RNMW_progressLegend>span[data-state=completed]{color:var(--dsw-alias-state-success)}.I7RNMW_progressSummary{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 7%, var(--dsw-alias-bg-module));min-width:0;color:var(--dsw-alias-label-secondary);border-radius:8px;align-items:center;gap:6px;padding:5px 8px;font-size:10px;font-weight:600;line-height:15px;display:flex}.I7RNMW_progressSummary[data-state=warning]{background:color-mix(in srgb, var(--dsw-alias-state-warning) 8%, var(--dsw-alias-bg-module))}.I7RNMW_progressSummary[data-state=completed]{background:color-mix(in srgb, var(--dsw-alias-state-success) 8%, var(--dsw-alias-bg-module))}.I7RNMW_progressSummary>span:last-child{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.I7RNMW_progressSummaryDot{background:var(--dsw-alias-state-business-primary);border-radius:50%;flex:none;width:5px;height:5px}.I7RNMW_progressSummary[data-state=warning] .I7RNMW_progressSummaryDot{background:var(--dsw-alias-state-warning)}.I7RNMW_progressSummary[data-state=completed] .I7RNMW_progressSummaryDot{background:var(--dsw-alias-state-success)}.I7RNMW_membersToggle{background:var(--dsw-alias-bg-module-platform);width:100%;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border:0;border-radius:8px;justify-content:space-between;align-items:center;gap:8px;padding:6px 8px;font-size:10px;font-weight:600;line-height:15px;transition:background-color .12s;display:flex}.I7RNMW_membersToggle:hover{background:var(--dsw-alias-bg-fill-neutral)}.I7RNMW_membersToggle>span{align-items:center;gap:5px;display:inline-flex}.I7RNMW_membersToggle>span:last-child{color:var(--dsw-alias-state-business-primary)}.I7RNMW_historyToggle{background:var(--dsw-alias-bg-module);width:100%;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border:0;border-radius:8px;justify-content:space-between;align-items:center;gap:8px;padding:6px 8px;font-size:10px;font-weight:600;line-height:15px;transition:background-color .12s;display:flex}.I7RNMW_historyToggle:hover{background:var(--dsw-alias-bg-fill-neutral)}.I7RNMW_historyToggle>span:last-child{color:var(--dsw-alias-state-business-primary)}.I7RNMW_chevron{flex:none;transition:transform .12s}.I7RNMW_chevron[data-open=true]{transform:rotate(90deg)}.I7RNMW_delegationTree{flex-direction:column;gap:2px;margin-left:18px;padding:0 0 0 20px;display:flex;position:relative}.I7RNMW_delegationTree:before{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 48%, var(--dsw-alias-line-normal));content:\"\";width:1px;position:absolute;top:0;bottom:22px;left:0}.I7RNMW_memberBlock{flex-direction:column;min-width:0;padding:3px 0 7px;display:flex;position:relative}.I7RNMW_memberBranch{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 48%, var(--dsw-alias-line-normal));width:20px;height:1px;display:block;position:absolute;top:27px;right:100%}.I7RNMW_memberBranch:before{background:var(--dsw-alias-state-business-primary);content:\"\";border-radius:50%;width:5px;height:5px;position:absolute;top:-2px;right:-1px}.I7RNMW_memberRow{box-sizing:border-box;width:100%;min-width:0;min-height:48px;color:inherit;font:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:8px;grid-template-columns:46px minmax(0,1fr) auto;align-items:center;gap:8px;padding:4px 6px;transition:background-color .12s,transform .12s;display:grid}.I7RNMW_memberRow:hover,.I7RNMW_memberRow[data-activity=working]{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 6%, var(--dsw-alias-bg-module))}.I7RNMW_memberRow:active{transform:scale(.99)}.I7RNMW_memberAvatar{width:42px;height:42px}.I7RNMW_memberAvatar[data-unread=true]:after{box-sizing:border-box;border:1px solid var(--dsw-alias-bg-module);background:var(--dsw-alias-state-business-primary);content:\"\";border-radius:50%;width:6px;height:6px;animation:1.8s ease-in-out infinite I7RNMW_agentTeamsUnreadPulse;position:absolute;top:0;right:-1px}@keyframes I7RNMW_agentTeamsUnreadPulse{0%,to{opacity:.78;transform:scale(.92)}50%{opacity:1;transform:scale(1.16)}}.I7RNMW_memberInitial{background:var(--dsw-alias-bg-fill-business);width:38px;height:38px;color:var(--dsw-alias-label-on-fill);border-radius:50%;justify-content:center;align-items:center;font-size:15px;font-weight:600;line-height:20px;display:inline-flex}.I7RNMW_stateArt{box-sizing:border-box;object-fit:contain;width:22px;height:22px;filter:drop-shadow(0 0 1px var(--dsw-alias-bg-module)) drop-shadow(0 1px 1px #122d483d);background:0 0;border:0;border-radius:0;position:absolute;bottom:-3px;right:-5px}.I7RNMW_stateArt[data-activity=working]{animation:2.4s ease-in-out infinite I7RNMW_agentTeamsFloat}.I7RNMW_stateArt[data-activity=idle]{animation:4.2s ease-in-out infinite I7RNMW_agentTeamsBreathe}.I7RNMW_stateArt[data-activity=unknown]{animation:2.8s ease-in-out infinite I7RNMW_agentTeamsThink}@keyframes I7RNMW_agentTeamsFloat{0%,to{transform:translateY(0)rotate(-4deg)}50%{transform:translateY(-2px)rotate(4deg)}}@keyframes I7RNMW_agentTeamsBreathe{0%,to{opacity:.82;transform:scale(1)}50%{opacity:1;transform:scale(1.06)}}@keyframes I7RNMW_agentTeamsThink{0%,to{transform:rotate(-7deg)}50%{transform:rotate(7deg)}}.I7RNMW_memberRight{flex:none;align-items:center;gap:8px;display:inline-flex}.I7RNMW_memberCount{color:var(--dsw-alias-label-tertiary);font-size:10px;line-height:16px}.I7RNMW_assignmentLine{align-items:center;gap:7px;min-width:0;padding:0 6px 0 60px;display:flex}.I7RNMW_assignmentLabel{color:var(--dsw-alias-label-tertiary);flex:none;font-size:9px;line-height:14px}.I7RNMW_assignmentTasks{flex-wrap:wrap;flex:1;gap:4px;min-width:0;display:flex}.I7RNMW_assignmentChip{background:var(--dsw-alias-bg-fill-neutral);min-height:16px;color:var(--dsw-alias-label-secondary);border-radius:4px;align-items:center;padding:0 5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;font-weight:600;line-height:14px;display:inline-flex}.I7RNMW_assignmentChip[data-state=running]{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill)}.I7RNMW_assignmentChip[data-state=completed]{background:var(--dsw-alias-bg-fill-success);color:var(--dsw-alias-label-on-fill)}.I7RNMW_assignmentChip[data-state=blocked]{background:var(--dsw-alias-bg-fill-warning);color:var(--dsw-alias-label-on-fill)}.I7RNMW_assignmentChip[data-state=failed]{background:var(--dsw-alias-bg-fill-danger);color:var(--dsw-alias-label-on-fill)}.I7RNMW_assignmentChip[data-state=cancelled]{color:var(--dsw-alias-label-tertiary);text-decoration:line-through}.I7RNMW_assignmentChip[data-review=pending]{outline:1px solid color-mix(in srgb, var(--dsw-alias-state-warning) 55%, transparent);outline-offset:-1px}.I7RNMW_assignmentChip[data-helping=true]{outline:1px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 50%, transparent);outline-offset:-1px}.I7RNMW_reviewChip,.I7RNMW_helpingChip,.I7RNMW_calibrationChip,.I7RNMW_blockedChip,.I7RNMW_inputChip{box-sizing:border-box;white-space:nowrap;min-height:16px;color:inherit;background:0 0;border-radius:4px;flex:none;align-items:center;margin-left:4px;padding:0 3px;font-family:inherit;font-size:8.5px;font-weight:700;line-height:16px;display:inline-flex}.I7RNMW_reviewChip{outline:1px dashed color-mix(in srgb, var(--dsw-alias-state-warning) 70%, transparent);outline-offset:-1px}.I7RNMW_blockedChip{outline:1px solid color-mix(in srgb, var(--dsw-alias-state-warning) 75%, transparent);outline-offset:-1px}.I7RNMW_inputChip{outline:1px dashed color-mix(in srgb, var(--dsw-alias-label-secondary) 70%, transparent);outline-offset:-1px}.I7RNMW_helpingChip{outline:1px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 65%, transparent);outline-offset:-1px}.I7RNMW_calibrationChip{outline:1px solid color-mix(in srgb, var(--dsw-alias-label-secondary) 70%, transparent);outline-offset:-1px}.I7RNMW_assignmentChip[data-timing=warn]{outline:1px solid color-mix(in srgb, var(--dsw-alias-state-warning) 55%, transparent);outline-offset:-1px}.I7RNMW_assignmentChip[data-timing=over]{outline:1px solid color-mix(in srgb, var(--dsw-alias-state-danger) 62%, transparent);outline-offset:-1px}.I7RNMW_timingChip{box-sizing:border-box;white-space:nowrap;border-radius:4px;flex:none;align-items:center;min-height:16px;margin-left:4px;padding:0 3px;font-family:inherit;font-size:8.5px;font-weight:700;line-height:16px;display:inline-flex}.I7RNMW_timingChip[data-timing=warn]{background:color-mix(in srgb, var(--dsw-alias-state-warning) 16%, transparent);color:var(--dsw-alias-state-warning)}.I7RNMW_timingChip[data-timing=over]{background:color-mix(in srgb, var(--dsw-alias-state-danger) 16%, transparent);color:var(--dsw-alias-state-danger)}.I7RNMW_unreadPill{color:var(--dsw-alias-state-business-primary);white-space:nowrap;flex:none;font-size:9px;font-weight:600;line-height:14px}.I7RNMW_taskEmpty{color:var(--dsw-alias-label-tertiary);font-size:9px;line-height:14px}.I7RNMW_dependencySection{border-top:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:7px;min-width:0;padding-top:10px;display:flex}.I7RNMW_sectionToggleTitle{color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;background:0 0;border:0;align-items:center;gap:6px;padding:0;font-size:11px;font-weight:600;line-height:16px;display:inline-flex}.I7RNMW_dagViewport{scrollbar-width:thin;min-width:0;padding:2px 0 4px;overflow-x:auto}.I7RNMW_dagCanvas{min-width:100%;position:relative}.I7RNMW_dagCanvas[data-layout=parallel]{flex-wrap:wrap;gap:8px;display:flex}.I7RNMW_dagCanvas[data-layout=parallel] .I7RNMW_dagNode{flex:92px;min-width:92px;position:relative}.I7RNMW_dagEdges{pointer-events:none;position:absolute;inset:0;overflow:visible}.I7RNMW_dagEdges path{fill:none;stroke:var(--dsw-alias-line-strong);stroke-width:1px;transition:opacity .12s,stroke .12s,stroke-width .12s}.I7RNMW_dagEdges path[data-active=true]{stroke:var(--dsw-alias-state-business-primary);stroke-width:1.6px}.I7RNMW_dagEdges path[data-dimmed=true]{opacity:.24}.I7RNMW_dagNode{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);color:var(--dsw-alias-label-primary);font:inherit;text-align:left;cursor:pointer;border-radius:6px;flex-direction:column;justify-content:center;gap:1px;padding:0 6px;transition:border-color .12s,background-color .12s,opacity .12s;display:flex;position:absolute}.I7RNMW_dagNode:hover,.I7RNMW_dagNode[data-focused=true]{border-color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 6%, var(--dsw-alias-bg-module))}.I7RNMW_dagNode[data-dimmed=true]{opacity:.3}.I7RNMW_dagNode[data-state=running][data-dimmed=true]{opacity:.58}.I7RNMW_dagNode[data-state=completed]{border-color:color-mix(in srgb, var(--dsw-alias-state-success) 48%, var(--dsw-alias-line-normal))}.I7RNMW_dagNode[data-state=blocked]{border-color:color-mix(in srgb, var(--dsw-alias-state-warning) 52%, var(--dsw-alias-line-normal))}.I7RNMW_dagNode[data-review=pending]{border-color:color-mix(in srgb, var(--dsw-alias-state-warning) 78%, var(--dsw-alias-line-normal));box-shadow:0 0 0 1px color-mix(in srgb, var(--dsw-alias-state-warning) 42%, transparent)}.I7RNMW_dagNode[data-helping=true]{border-color:color-mix(in srgb, var(--dsw-alias-state-business-primary) 70%, var(--dsw-alias-line-normal));box-shadow:0 0 0 1px color-mix(in srgb, var(--dsw-alias-state-business-primary) 38%, transparent)}.I7RNMW_dagNode[data-timing=warn]{border-color:color-mix(in srgb, var(--dsw-alias-state-warning) 78%, var(--dsw-alias-line-normal));box-shadow:0 0 0 1px color-mix(in srgb, var(--dsw-alias-state-warning) 42%, transparent)}.I7RNMW_dagNode[data-timing=over]{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 78%, var(--dsw-alias-line-normal));box-shadow:0 0 0 1px color-mix(in srgb, var(--dsw-alias-state-danger) 46%, transparent)}.I7RNMW_dagNode[data-state=failed]{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 56%, var(--dsw-alias-line-normal))}.I7RNMW_dagNodeHead{color:var(--dsw-alias-label-primary);align-items:center;gap:4px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;font-weight:700;display:flex}.I7RNMW_dagNodeDot{background:var(--dsw-alias-line-strong);border-radius:2px;flex:none;width:5px;height:5px}.I7RNMW_dagNode[data-state=running] .I7RNMW_dagNodeDot{background:var(--dsw-alias-state-business-primary)}.I7RNMW_dagNode[data-state=running] .I7RNMW_dagNodeHead{padding-right:12px}.I7RNMW_dagRunningState{width:9px;height:9px;color:var(--dsw-alias-state-business-primary);pointer-events:none;justify-content:center;align-items:center;display:inline-flex;position:absolute;top:4px;right:5px}.I7RNMW_dagRunningState .I7RNMW_workGlyph{width:9px;height:9px}.I7RNMW_dagNode[data-state=blocked] .I7RNMW_dagNodeDot{background:var(--dsw-alias-state-warning)}.I7RNMW_dagNode[data-state=completed] .I7RNMW_dagNodeDot{background:var(--dsw-alias-state-success)}.I7RNMW_dagNode[data-state=failed] .I7RNMW_dagNodeDot{background:var(--dsw-alias-state-danger)}.I7RNMW_dagNodeLabel{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:11px;overflow:hidden}.I7RNMW_taskDetail{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:8px;flex-direction:column;gap:3px;min-width:0;padding:7px 9px;display:flex}.I7RNMW_taskDetailHead{align-items:center;gap:6px;min-width:0;display:flex}.I7RNMW_taskDetailId{color:var(--dsw-alias-state-business-primary);flex:none;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;font-weight:700}.I7RNMW_taskDetailSubject{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-weight:600;line-height:16px;overflow:hidden}.I7RNMW_taskDetailBadge{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;padding:0 5px;font-size:8.5px;font-weight:600;line-height:14px}.I7RNMW_taskDetailBadge[data-state=running]{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill)}.I7RNMW_taskDetailBadge[data-state=blocked]{background:var(--dsw-alias-bg-fill-warning);color:var(--dsw-alias-label-on-fill)}.I7RNMW_taskDetailBadge[data-state=completed]{background:var(--dsw-alias-bg-fill-success);color:var(--dsw-alias-label-on-fill)}.I7RNMW_taskDetailBadge[data-state=failed]{background:var(--dsw-alias-bg-fill-danger);color:var(--dsw-alias-label-on-fill)}.I7RNMW_taskDetailLine,.I7RNMW_taskDetailMeta{color:var(--dsw-alias-label-secondary);font-size:9px;line-height:14px}.I7RNMW_taskDetailMeta{color:var(--dsw-alias-label-tertiary)}.I7RNMW_archiveAction{color:var(--dsw-alias-state-danger);background:color-mix(in srgb, var(--dsw-alias-state-danger) 12%, transparent);cursor:pointer;border:0;border-radius:5px;padding:3px 6px;font-size:9px}.I7RNMW_archiveAction:disabled{opacity:.55;cursor:not-allowed}.I7RNMW_diagnosticBanner{color:var(--dsw-alias-label-primary);background:color-mix(in srgb, var(--dsw-alias-state-warning) 10%, var(--dsw-alias-bg-module));flex-wrap:wrap;align-items:flex-start;gap:6px 8px;padding:7px 10px;font-size:11px;line-height:16px;display:flex}.I7RNMW_diagnosticBanner button{color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-fill-neutral);cursor:pointer;border:0;border-radius:4px;padding:2px 6px}.I7RNMW_archiveConfirm{color:var(--dsw-alias-label-primary);background:color-mix(in srgb, var(--dsw-alias-state-danger) 10%, var(--dsw-alias-background-base));justify-content:space-between;gap:8px;padding:7px 10px;font-size:10px;display:flex}.I7RNMW_archiveConfirmActions{gap:5px;display:flex}.I7RNMW_archiveConfirmActions button{cursor:pointer;border:0;border-radius:4px;padding:3px 6px}.I7RNMW_connectionStatus{color:var(--dsw-alias-label-tertiary);margin-left:5px;font-size:9px;font-weight:400}.I7RNMW_connectionStatus[data-connection=stale],.I7RNMW_connectionStatus[data-connection=offline],.I7RNMW_connectionStatus[data-connection=auth-failed]{color:var(--dsw-alias-state-warning)}.I7RNMW_taskDetailActions{gap:6px;margin-top:3px;display:flex}.I7RNMW_taskDetailActions button{color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:0;padding:1px 4px;font-size:9px}.I7RNMW_taskDetailTiming{color:var(--dsw-alias-label-secondary);font-size:9px;line-height:14px}.I7RNMW_taskDetailTiming[data-timing=warn]{color:var(--dsw-alias-state-warning)}.I7RNMW_taskDetailTiming[data-timing=over]{color:var(--dsw-alias-state-danger);font-weight:600}.I7RNMW_taskDetailBudgetFact{color:var(--dsw-alias-label-secondary);font-size:11px;line-height:16px}.I7RNMW_taskDetailBudgetFact[data-budget-fact=over_budget]{color:var(--dsw-alias-state-warning);font-weight:700}.I7RNMW_taskDetailBudgetFact[data-budget-fact=within_budget]{color:var(--dsw-alias-state-success)}.I7RNMW_taskDetailSignals,.I7RNMW_taskDetailRetro{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:13px;overflow:hidden}.I7RNMW_taskDetail[data-expanded] .I7RNMW_taskDetailSignals,.I7RNMW_taskDetail[data-expanded] .I7RNMW_taskDetailRetro{overflow-wrap:anywhere;text-overflow:clip;white-space:normal;font-size:12px;line-height:18px;overflow:visible}.I7RNMW_taskDetail[data-expanded] .I7RNMW_taskDetailActions{margin-top:7px}.I7RNMW_taskDetailActions button{min-height:32px}.I7RNMW_taskDetailActions button:focus-visible,.I7RNMW_archiveAction:focus-visible,.I7RNMW_archiveConfirmActions button:focus-visible{outline:2px solid var(--dsw-alias-state-primary);outline-offset:2px}.I7RNMW_taskDetailRetro[data-cause=underestimated],.I7RNMW_taskDetailRetro[data-cause=dependency-blocked]{color:var(--dsw-alias-state-danger)}.I7RNMW_taskDetailRetro[data-cause=on_time]{color:var(--dsw-alias-state-success)}.I7RNMW_taskDetailReview{color:var(--dsw-alias-state-warning);font-size:9px;font-weight:600;line-height:14px}.I7RNMW_taskDetailReview[data-verdict=pass]{color:var(--dsw-alias-state-success)}.I7RNMW_taskDetailReview[data-verdict=reject]{color:var(--dsw-alias-state-danger)}.I7RNMW_taskDetailCalibration{color:var(--dsw-alias-label-secondary);font-size:9px;font-weight:600;line-height:14px}.I7RNMW_taskDetailBlocked{color:var(--dsw-alias-state-warning);font-size:9px;font-weight:700;line-height:14px}.I7RNMW_taskDetailInput{color:var(--dsw-alias-label-secondary);font-size:9px;font-weight:600;line-height:14px}.I7RNMW_emptyHint{color:var(--dsw-alias-label-tertiary);padding:10px 14px;font-size:11px;line-height:16px}.I7RNMW_modeHint{color:var(--dsw-alias-label-secondary);padding:0 14px 6px;font-size:11px;line-height:16px;display:block}.I7RNMW_modeBadge{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:1px 7px;font-size:11px;font-weight:600;line-height:16px}.I7RNMW_modeBadge[data-mode=light]{color:var(--dsw-alias-label-secondary)}.I7RNMW_modeBadge[data-mode=standard]{color:var(--dsw-alias-state-business-primary)}.I7RNMW_modeBadge[data-mode=governed]{color:var(--dsw-alias-state-warning)}.I7RNMW_historicPill{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;margin-left:auto;padding:1px 7px;font-size:9px;font-weight:600;line-height:15px}.I7RNMW_members{flex-direction:column;gap:3px;display:flex}.I7RNMW_archiveLabel{color:var(--dsw-alias-label-tertiary);padding:5px 14px 0;font-size:9px;font-weight:600;line-height:14px;display:block}.I7RNMW_archiveFilterBar{flex-wrap:wrap;align-items:center;gap:6px 8px;padding:6px 14px 2px;display:flex}.I7RNMW_archiveFilterField{color:var(--dsw-alias-label-tertiary);align-items:center;gap:4px;font-size:9px;line-height:14px;display:inline-flex}.I7RNMW_archiveFilterCaption{white-space:nowrap;font-weight:600}.I7RNMW_archiveSelect{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);max-width:120px;color:var(--dsw-alias-label-primary);border-radius:4px;padding:1px 2px;font-size:9px;line-height:14px}.I7RNMW_archiveFilterCount{color:var(--dsw-alias-label-tertiary);white-space:nowrap;margin-left:auto;font-size:9px}.I7RNMW_archiveEmpty{color:var(--dsw-alias-label-tertiary);padding:4px 14px 2px;font-size:9px;display:block}@media (prefers-reduced-motion:reduce){.I7RNMW_panel,.I7RNMW_badge,.I7RNMW_badgeDot,.I7RNMW_panelDot,.I7RNMW_workGlyph rect,.I7RNMW_stateArt,.I7RNMW_memberAvatar[data-unread=true]:after{transition:none;animation:none}}@media (width<=960px){html[data-agent-team-web-panel-open] div[data-phase]{padding-right:0}.I7RNMW_team{padding:12px 12px 14px}.I7RNMW_delegationTree{margin-left:12px;padding-left:15px}.I7RNMW_memberBranch{width:15px}}@media (width<=640px){.I7RNMW_badge{top:56px;right:10px}.I7RNMW_teamStats span[data-stat=messages]{display:none}.I7RNMW_captainNode{grid-template-columns:48px minmax(0,1fr)}.I7RNMW_captainState{display:none}.I7RNMW_assignmentLine{padding-left:60px}}.I7RNMW_healthChip{box-sizing:border-box;background:var(--dsw-alias-bg-module);border:1px solid var(--dsw-alias-line-normal);border-radius:999px;flex:none;align-items:center;gap:5px;padding:1px 8px;font-size:11px;line-height:16px;display:inline-flex}.I7RNMW_healthChip strong{color:var(--dsw-alias-label-primary);font-variant-numeric:tabular-nums;font-size:11px;font-weight:700}.I7RNMW_healthChip em{color:var(--dsw-alias-label-secondary);font-style:normal}.I7RNMW_healthChip[data-level=ok] strong{color:var(--dsw-alias-state-success)}.I7RNMW_healthChip[data-level=warn] strong{color:var(--dsw-alias-state-warning)}.I7RNMW_healthChip[data-level=critical] strong,.I7RNMW_riskStat{color:var(--dsw-alias-state-danger)}.I7RNMW_prioritySection{flex-direction:column;gap:5px;min-width:0;display:flex}.I7RNMW_priorityCard{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);border-radius:8px;align-items:flex-start;gap:8px;padding:6px 8px;display:flex}.I7RNMW_priorityCard[data-severity=high]{border-color:color-mix(in srgb,var(--dsw-alias-state-danger) 45%,var(--dsw-alias-line-normal))}.I7RNMW_priorityCard[data-severity=medium]{border-color:color-mix(in srgb,var(--dsw-alias-state-warning) 40%,var(--dsw-alias-line-normal))}.I7RNMW_priorityBadge{background:var(--dsw-alias-bg-fill-business);min-width:22px;height:18px;color:var(--dsw-alias-label-on-fill);border-radius:6px;flex:none;justify-content:center;align-items:center;font-size:11px;font-weight:700;display:inline-flex}.I7RNMW_priorityBody{flex-direction:column;flex:1;gap:3px;min-width:0;display:flex}.I7RNMW_priorityTitle{flex-wrap:wrap;align-items:center;gap:5px;min-width:0;display:flex}.I7RNMW_priorityTitle strong{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-weight:600;line-height:16px;overflow:hidden}.I7RNMW_chip{background:var(--dsw-alias-bg-module);border:1px solid var(--dsw-alias-line-normal);border-radius:999px;padding:0 6px;font-size:11px;font-style:normal;font-weight:600;line-height:16px;display:inline-flex}.I7RNMW_chip[data-readiness=blocked],.I7RNMW_chip[data-readiness=orphaned],.I7RNMW_chip[data-readiness=stalled],.I7RNMW_chip[data-readiness=failed],.I7RNMW_chip[data-severity=high]{border-color:var(--dsw-alias-state-danger);color:var(--dsw-alias-state-danger)}.I7RNMW_chip[data-severity=medium],.I7RNMW_chip[data-readiness=ready]{border-color:var(--dsw-alias-state-warning);color:var(--dsw-alias-state-warning)}.I7RNMW_chip[data-severity=low],.I7RNMW_chip[data-readiness=cancelled]{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-tertiary)}.I7RNMW_priorityReasons{min-width:0;color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:16px}.I7RNMW_milestoneRow{min-width:0;color:var(--dsw-alias-label-secondary);align-items:center;gap:6px;font-size:11px;line-height:16px;display:flex}.I7RNMW_milestoneRow strong{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-weight:600;overflow:hidden}.I7RNMW_loadBar{background:var(--dsw-alias-line-strong);border-radius:999px;flex:none;align-items:stretch;width:44px;min-width:0;height:6px;display:inline-flex;overflow:hidden}.I7RNMW_loadBar>span{display:block}.I7RNMW_loadActive{background:var(--dsw-alias-state-business-primary)}.I7RNMW_loadPending{background:var(--dsw-alias-state-warning)}.I7RNMW_loadStalled,.I7RNMW_loadOrphaned{background:var(--dsw-alias-state-danger)}";
		...
		var ActivityPanel_module_css_default = {
			"agentTeamsBreathe": "I7RNMW_agentTeamsBreathe",
			"agentTeamsDot": "I7RNMW_agentTeamsDot",
			"agentTeamsFloat": "I7RNMW_agentTeamsFloat",
			"agentTeamsPanelIn": "I7RNMW_agentTeamsPanelIn",
			"agentTeamsPulse": "I7RNMW_agentTeamsPulse",
			"agentTeamsThink": "I7RNMW_agentTeamsThink",
			"agentTeamsUnreadPulse": "I7RNMW_agentTeamsUnreadPulse",
			"archiveAction": "I7RNMW_archiveAction",
			"archiveConfirm": "I7RNMW_archiveConfirm",
			"archiveConfirmActions": "I7RNMW_archiveConfirmActions",
			"archiveEmpty": "I7RNMW_archiveEmpty",
			"archiveFilterBar": "I7RNMW_archiveFilterBar",
			"archiveFilterCaption": "I7RNMW_archiveFilterCaption",
			"archiveFilterCount": "I7RNMW_archiveFilterCount",
			"archiveFilterField": "I7RNMW_archiveFilterField",
			"archiveLabel": "I7RNMW_archiveLabel",
			"archiveSelect": "I7RNMW_archiveSelect",
			"assignmentChip": "I7RNMW_assignmentChip",
			"assignmentLabel": "I7RNMW_assignmentLabel",
			"assignmentLine": "I7RNMW_assignmentLine",
			"assignmentTasks": "I7RNMW_assignmentTasks",
			"badge": "I7RNMW_badge",
			"badgeCount": "I7RNMW_badgeCount",
			"badgeDot": "I7RNMW_badgeDot",
			"blockedChip": "I7RNMW_blockedChip",
			"calibrationChip": "I7RNMW_calibrationChip",
			"captainAvatar": "I7RNMW_captainAvatar",
			"captainInfo": "I7RNMW_captainInfo",
			"captainLine": "I7RNMW_captainLine",
			"captainName": "I7RNMW_captainName",
			"captainNode": "I7RNMW_captainNode",
			"captainRole": "I7RNMW_captainRole",
			"captainState": "I7RNMW_captainState",
			"captainSummary": "I7RNMW_captainSummary",
			"chevron": "I7RNMW_chevron",
			"chip": "I7RNMW_chip",
			"closeError": "I7RNMW_closeError",
			"commandLayer": "I7RNMW_commandLayer",
			"commissarNode": "I7RNMW_commissarNode",
			"commissarState": "I7RNMW_commissarState",
			"connectionStatus": "I7RNMW_connectionStatus",
			"dagCanvas": "I7RNMW_dagCanvas",
			"dagEdges": "I7RNMW_dagEdges",
			"dagNode": "I7RNMW_dagNode",
			"dagNodeDot": "I7RNMW_dagNodeDot",
			"dagNodeHead": "I7RNMW_dagNodeHead",
			"dagNodeLabel": "I7RNMW_dagNodeLabel",
			"dagRunningState": "I7RNMW_dagRunningState",
			"dagViewport": "I7RNMW_dagViewport",
			"delegationSection": "I7RNMW_delegationSection",
			"delegationTree": "I7RNMW_delegationTree",
			"dependencySection": "I7RNMW_dependencySection",
			"diagnosticBanner": "I7RNMW_diagnosticBanner",
			"emptyHint": "I7RNMW_emptyHint",
			"healthChip": "I7RNMW_healthChip",
			"helpingChip": "I7RNMW_helpingChip",
			"historicPill": "I7RNMW_historicPill",
			"historyToggle": "I7RNMW_historyToggle",
			"iconButton": "I7RNMW_iconButton",
			"inputChip": "I7RNMW_inputChip",
			"leadAvatar": "I7RNMW_leadAvatar",
			"loadActive": "I7RNMW_loadActive",
			"loadBar": "I7RNMW_loadBar",
			"loadOrphaned": "I7RNMW_loadOrphaned",
			"loadPending": "I7RNMW_loadPending",
			"loadStalled": "I7RNMW_loadStalled",
			"memberArt": "I7RNMW_memberArt",
			"memberAvatar": "I7RNMW_memberAvatar",
			"memberBlock": "I7RNMW_memberBlock",
			"memberBranch": "I7RNMW_memberBranch",
			"memberCount": "I7RNMW_memberCount",
			"memberElapsed": "I7RNMW_memberElapsed",
			"memberInfo": "I7RNMW_memberInfo",
			"memberInitial": "I7RNMW_memberInitial",
			"memberLine": "I7RNMW_memberLine",
			"memberModel": "I7RNMW_memberModel",
			"memberName": "I7RNMW_memberName",
			"memberRight": "I7RNMW_memberRight",
			"memberRole": "I7RNMW_memberRole",
			"memberRow": "I7RNMW_memberRow",
			"memberState": "I7RNMW_memberState",
			"memberStatusLine": "I7RNMW_memberStatusLine",
			"members": "I7RNMW_members",
			"membersToggle": "I7RNMW_membersToggle",
			"milestoneRow": "I7RNMW_milestoneRow",
			"modeBadge": "I7RNMW_modeBadge",
			"modeHint": "I7RNMW_modeHint",
			"panel": "I7RNMW_panel",
			"panelControls": "I7RNMW_panelControls",
			"panelDot": "I7RNMW_panelDot",
			"panelHead": "I7RNMW_panelHead",
			"panelTitle": "I7RNMW_panelTitle",
			"priorityBadge": "I7RNMW_priorityBadge",
			"priorityBody": "I7RNMW_priorityBody",
			"priorityCard": "I7RNMW_priorityCard",
			"priorityReasons": "I7RNMW_priorityReasons",
			"prioritySection": "I7RNMW_prioritySection",
			"priorityTitle": "I7RNMW_priorityTitle",
			"progressEmpty": "I7RNMW_progressEmpty",
			"progressLegend": "I7RNMW_progressLegend",
			"progressOverview": "I7RNMW_progressOverview",
			"progressSegments": "I7RNMW_progressSegments",
			"progressSummary": "I7RNMW_progressSummary",
			"progressSummaryDot": "I7RNMW_progressSummaryDot",
			"progressTitle": "I7RNMW_progressTitle",
			"resizeHandle": "I7RNMW_resizeHandle",
			"reviewChip": "I7RNMW_reviewChip",
			"riskStat": "I7RNMW_riskStat",
			"sectionHead": "I7RNMW_sectionHead",
			"sectionHint": "I7RNMW_sectionHint",
			"sectionTitle": "I7RNMW_sectionTitle",
			"sectionToggleTitle": "I7RNMW_sectionToggleTitle",
			"stageLabel": "I7RNMW_stageLabel",
			"stateArt": "I7RNMW_stateArt",
			"taskDetail": "I7RNMW_taskDetail",
			"taskDetailActions": "I7RNMW_taskDetailActions",
			"taskDetailBadge": "I7RNMW_taskDetailBadge",
			"taskDetailBlocked": "I7RNMW_taskDetailBlocked",
			"taskDetailBudgetFact": "I7RNMW_taskDetailBudgetFact",
			"taskDetailCalibration": "I7RNMW_taskDetailCalibration",
			"taskDetailHead": "I7RNMW_taskDetailHead",
			"taskDetailId": "I7RNMW_taskDetailId",
			"taskDetailInput": "I7RNMW_taskDetailInput",
			"taskDetailLine": "I7RNMW_taskDetailLine",
			"taskDetailMeta": "I7RNMW_taskDetailMeta",
			"taskDetailRetro": "I7RNMW_taskDetailRetro",
			"taskDetailReview": "I7RNMW_taskDetailReview",
			"taskDetailSignals": "I7RNMW_taskDetailSignals",
			"taskDetailSubject": "I7RNMW_taskDetailSubject",
			"taskDetailTiming": "I7RNMW_taskDetailTiming",
			"taskEmpty": "I7RNMW_taskEmpty",
			"taskId": "I7RNMW_taskId",
			"team": "I7RNMW_team",
			"teamHead": "I7RNMW_teamHead",
			"teamName": "I7RNMW_teamName",
			"teamStats": "I7RNMW_teamStats",
			"teams": "I7RNMW_teams",
			"timingChip": "I7RNMW_timingChip",
			"unreadPill": "I7RNMW_unreadPill",
			"workGlyph": "I7RNMW_workGlyph"
		};
		//#endregion
		...
		*/
		function applyDockLayout(shouldYield, width, doc) {
			const root = (doc ?? document).documentElement;
			if (shouldYield && width !== void 0) {
		...
		*/
		function dismissalTransition(prev, liveCount) {
		...
		* model 缺失 → null(旧数据不显示小字)。 */
		...
		function primaryTask(team) {
			return team.tasks.find((task) => task.state === "running") ?? team.tasks.find((task) => task.state === "blocked" && task.status !== "completed") ?? team.tasks.find((task) => task.state === "open" && task.status !== "completed") ?? null;
		}
		function NowAction({ team, t }) {
			const task = primaryTask(team);
			if (task === null) return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: ActivityPanel_module_css_default.nowAction,
				"data-now-action": true,
				"data-state": "empty",
				"aria-label": t("deck.nowAria"),
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: ActivityPanel_module_css_default.deckEyebrow,
					children: t("deck.now")
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
					className: ActivityPanel_module_css_default.nowEmpty,
					children: t("deck.nowEmpty")
				})]
			});
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
						className: ActivityPanel_module_css_default.nowBody,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", {
							title: task.subject,
							children: t("deck.nowTask", {
								taskId: task.id,
								subject: task.subject
							})
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: owner })]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: ActivityPanel_module_css_default.nowStatus,
						children: taskStatusLabel(task.status, t)
					})
				]
			});
		}
		...
		function DependencyMap({ tasks, t, compact = false }) {
			const [open, setOpen] = (0, react.useState)(() => tasks.some((task) => task.state === "blocked"));
			...
			const [detailExpanded, setDetailExpanded] = (0, react.useState)(false);
			const [detailCopied, setDetailCopied] = (0, react.useState)(false);
			const [detailCopyFailed, setDetailCopyFailed] = (0, react.useState)(false);
			...
			const copyDetail = () => {
				...
				copyTaskDetail(detailText, clipboard).then((result) => {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
					...
					children: [
						...
						(detailExpanded || !compact) && taskSignalsText(detailTask, t) !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: ActivityPanel_module_css_default.taskDetailSignals,
							"data-timing": timingData(detailTask),
							children: taskSignalsText(detailTask, t)
						}),
						(detailExpanded || !compact) && budgetFactText(detailTask, t) !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: ActivityPanel_module_css_default.taskDetailBudgetFact,
							"data-budget-fact": detailTask.budgetFact,
							children: budgetFactText(detailTask, t)
						}),
						(detailExpanded || !compact) && retroDetailText(detailTask, t) !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: ActivityPanel_module_css_default.taskDetailActions,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									"aria-expanded": detailExpanded,
									"aria-controls": `task-detail-${detailTask.id}`,
									onClick: () => {
										setDetailExpanded((open) => !open);
									},
									children: t(detailExpanded ? "task.detail.collapse" : "task.detail.expand")
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									onClick: copyDetail,
									children: detailCopied ? t("task.detail.copied") : t("task.detail.copy")
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									role: "status",
									"aria-live": "polite",
									children: detailCopied ? t("task.detail.copied") : detailCopyFailed ? t("task.detail.copyFailed") : ""
								})
							]
						}),
		...
		function TeamSection({ team, onNavigate, t, historic = false, compact = false, diagnostics }) {
			const [membersOpen, setMembersOpen] = (0, react.useState)(() => team.members.some((member) => member.activity === "working" || member.unread > 0));
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
						...
						children: [
							...
							(() => {
								...
								return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: ActivityPanel_module_css_default.modeBadge,
									"data-mode": mode,
									title: t(teamModeHintKey(mode)),
									"aria-label": t("team.mode.aria", { mode: label }),
									children: label
								});
							})(),
					...
					effectiveTeamMode(team.mode) === "light" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: ActivityPanel_module_css_default.modeHint,
						"data-mode": "light",
						children: t("team.mode.hint.light")
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						...
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)(NowAction, {
								team,
								t
							}),
							...
							membersOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [execMembers.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									const diagnostic = diagnostics?.get(member.id);
									const navigation = {
										parentId: team.captainSessionId,
										childId: member.id
									};
									return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [
											...
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
												...
												"data-activity": member.activity,
												"data-diagnostic": diagnostic === void 0 ? void 0 : "pending",
												title: diagnostic === void 0 ? void 0 : t("history.diagnostic.disabledReason"),
												onClick: () => {
													if (member.id !== "") onNavigate(navigation.parentId, navigation.childId);
												},
												children: [
													/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
		...
		* durable roster through the same rebuilt panel instead of a second UI. */
		...
		function ActivityPanel({ sessionsList, isCurrentSession, openMember, t }) {
			const navigateToSession = (parentId, childId) => {
				setDiagnosticRetry({
					parentId,
					childId
				});
				openMember(parentId, childId).then((opened) => {
			...
			const [archiveConfirm, setArchiveConfirm] = (0, react.useState)(false);
			const [diagnosticCopyResult, setDiagnosticCopyResult] = (0, react.useState)(null);
			const [diagnosticRetry, setDiagnosticRetry] = (0, react.useState)(null);
			...
			const current = (0, react.useSyncExternalStore)(sessionsList.subscribe, () => {
				return sessionsList.getSnapshot().ids.find(isCurrentSession);
			});
			const monitorTargets = (0, react.useSyncExternalStore)(subscribeActivityMonitorTargets, getActivityMonitorTargetsSnapshot);
			const { teams, archivedTeams, connection, lastSuccessAt } = (0, react.useSyncExternalStore)(subscribeActivitySnapshots, getActivitySnapshotsSnapshot);
			const historyDiagnostics = (0, react.useSyncExternalStore)(subscribeHistoryDiagnostics, getHistoryDiagnosticsSnapshot);
			...
			(0, react.useLayoutEffect)(() => {
				...
				const measure = () => {
					frame = null;
					const conversation = document.querySelector("div[data-phase]");
			...
			(0, react.useLayoutEffect)(() => {
				applyDockLayout(expanded && geometry.mode === "docked" && !compact, geometry.width);
				return () => {
			...
			const currentDiagnostic = relevantDiagnostic((0, react.useMemo)(() => [
				...visibleTeams,
				...visibleArchived,
				...visibleHistoric.map(({ data }) => historicCardTeam(data, current ?? ""))
			], [
				visibleTeams,
				visibleArchived,
				visibleHistoric,
				current
			]), historyDiagnostics, current ?? "");
			...
			const [dismissal, setDismissal] = (0, react.useState)(() => {
				if (typeof window === "undefined") return {
					hadLive: false,
					dismissed: false
				};
			...
			const connectionText = connection === "loading" ? t("activity.connection.loading") : connection === "auth-failed" ? t("activity.connection.authFailed") : connection === "stale" || connection === "offline" ? t("activity.connection.disconnected") : t("activity.connection.ready", { time: lastSuccessAt === void 0 ? "—" : new Date(lastSuccessAt).toLocaleTimeString() });
				...
				try {
					...
					const response = await fetch("/plugins/agent-team-web/close", {
						...
						body: JSON.stringify({
							...
							...liveTeam.workspaceId === void 0 ? {} : { workspaceId: liveTeam.workspaceId }
						})
					});
					if (!response.ok) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [!expanded && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CollapsedBadge, {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: ActivityPanel_module_css_default.connectionStatus,
							...
							children: [
								!compact && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									...
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconPanelLeftOutlineMedium, {})
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: ActivityPanel_module_css_default.iconButton,
								...
								liveTeam !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: ActivityPanel_module_css_default.archiveAction,
					...
					archiveConfirm && liveTeam !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: ActivityPanel_module_css_default.archiveConfirm,
						role: "alertdialog",
						"aria-label": t("activity.close"),
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("activity.closeConfirm") }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							className: ActivityPanel_module_css_default.archiveConfirmActions,
					...
					currentDiagnostic !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						role: "status",
						"aria-live": "polite",
						className: ActivityPanel_module_css_default.diagnosticBanner,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: diagnosticBannerText(currentDiagnostic, t) }),
							diagnosticRetry !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								onClick: () => {
									navigateToSession(diagnosticRetry.parentId, diagnosticRetry.childId);
								},
								children: t("history.diagnostic.retry")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								onClick: () => {
									copyDiagnosticText(diagnosticClipboardText(currentDiagnostic)).then(setDiagnosticCopyResult);
								},
								children: t("history.diagnostic.copy")
							}),
							diagnosticCopyResult !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								role: "status",
								"aria-live": "polite",
								children: t(diagnosticCopyResult === "copied" ? "history.diagnostic.copyOk" : "history.diagnostic.copyFail")
							})
						]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: visibleCount === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							...
							visibleArchived.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
								type: "button",
								className: ActivityPanel_module_css_default.historyToggle,
								"data-history-toggle": true,
								"aria-expanded": historyOpen,
								onClick: () => {
									setHistoryOpen((open) => !open);
								},
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("archive.label") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: historyOpen ? "⌄" : "›" })]
							}),
							historyOpen && visibleArchived.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							historyOpen && visibleArchived.length > 0 && filteredArchived.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: ActivityPanel_module_css_default.archiveEmpty,
								children: t("archive.filterEmpty")
							}),
							historyOpen && filteredArchived.map((team) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								"data-team-id": team.teamId,
								"data-historic": true,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
		...
		function parseAgentTeamsCreateResult(value) {
			const visit = (candidate) => {
				...
				if (typeof record["team_id"] === "string" && record["team_id"].trim() !== "" && typeof record["team_name"] === "string" && record["team_name"].trim() !== "" && typeof record["captain_session_id"] === "string" && record["captain_session_id"].trim() !== "") return {
		...
		const agentTeamsCardDefinition = {
			...
			update: (context, match) => {
				...
				if (match.event.data.error !== void 0) return context.state;
				const parsed = parseAgentTeamsCreateResult(match.event.data.message.content);
				return parsed === void 0 ? context.state : {
					...context.state,
					teamId: parsed.teamId,
					...parsed.name === void 0 ? {} : { name: parsed.name },
					...parsed.captainSessionId === void 0 ? {} : { captainSessionId: parsed.captainSessionId },
					accepted: true
				};
			},
		...
		*/
		async function openAgentTeamMember(navigator, parentSessionId, childSessionId) {
			const retained = navigator.sessions.subagentAddress(childSessionId);
			navigator.openSession(retained?.parentSessionId === parentSessionId ? retained : {
				parentSessionId,
				childSessionId,
				mode: "continuable"
			});
		...
		const cssText = ".NSrc7q_section{flex-direction:column;gap:14px;display:flex}.NSrc7q_card{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;flex-direction:column;gap:10px;padding:14px 16px;display:flex}.NSrc7q_head{align-items:center;gap:8px;display:flex}.NSrc7q_title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:600;line-height:20px}.NSrc7q_list{flex-direction:column;gap:4px;margin:0;padding:0;list-style:none;display:flex}.NSrc7q_row{box-sizing:border-box;border-radius:8px;justify-content:space-between;align-items:center;gap:8px;min-width:0;min-height:36px;padding:0 10px;display:flex}.NSrc7q_nameWrap{flex:1;align-items:center;gap:8px;min-width:0;display:flex}.NSrc7q_name{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:500;line-height:20px;overflow:hidden}.NSrc7q_rowSub{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;line-height:16px}.NSrc7q_pill{box-sizing:border-box;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-secondary);border-radius:999px;flex:none;padding:1px 8px;font-size:11px;font-weight:500;line-height:17px}.NSrc7q_switch{box-sizing:border-box;background:var(--dsw-alias-border-l2);cursor:pointer;border:none;border-radius:999px;flex:none;width:32px;height:18px;padding:0;transition:background .12s ease-in-out;position:relative}.NSrc7q_switch[data-on=true]{background:var(--dsw-alias-state-business-primary)}.NSrc7q_switch:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.NSrc7q_switchThumb{background:var(--dsw-alias-bg-layer-1);border-radius:50%;width:14px;height:14px;transition:transform .12s ease-in-out;position:absolute;top:2px;left:2px}.NSrc7q_switch[data-on=true] .NSrc7q_switchThumb{transform:translate(14px)}.NSrc7q_select{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);min-width:0;max-width:200px;color:var(--dsw-alias-label-primary);border-radius:6px;flex:1;padding:2px 4px;font-size:11px;line-height:16px}.NSrc7q_select:disabled{opacity:.55}@media (prefers-color-scheme:dark){.NSrc7q_select{color:#e8e8e8;background:#26282b}}.NSrc7q_resetBtn{color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:1px solid #0000;border-radius:6px;flex:none;margin-left:auto;padding:2px 10px;font-size:11px;line-height:16px;transition:color .12s ease-in-out,border-color .12s ease-in-out}.NSrc7q_resetBtn:hover:not(:disabled){border-color:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-state-business-primary)}.NSrc7q_resetBtn:disabled{opacity:.45;cursor:default}.NSrc7q_presetPopover{border:1px solid var(--dsw-alias-border-l2);background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 56%, transparent);border-radius:10px;flex-direction:column;gap:8px;margin:2px 0 10px;padding:10px;display:flex}.NSrc7q_presetTitle{color:var(--dsw-alias-label-primary);font-size:12px;font-weight:600;line-height:18px}.NSrc7q_presetHelp{color:var(--dsw-alias-label-tertiary);margin:0;font-size:11px;line-height:16px}.NSrc7q_presetTabs{flex-wrap:wrap;gap:6px;display:flex}.NSrc7q_presetTab{border:1px solid var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border-radius:999px;padding:3px 8px;font-size:11px;line-height:16px}.NSrc7q_presetTab[data-active=true]{border-color:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 10%, transparent)}.NSrc7q_presetList{flex-direction:column;gap:6px;display:flex}.NSrc7q_presetItem{border:1px solid var(--dsw-alias-border-l2);color:inherit;text-align:left;cursor:pointer;background:0 0;border-radius:8px;flex-direction:column;gap:4px;padding:8px 10px;display:flex}.NSrc7q_presetItem[data-active=true]{border-color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 8%, transparent)}.NSrc7q_presetItemHead{justify-content:space-between;align-items:center;gap:8px;display:flex}.NSrc7q_presetItemName{color:var(--dsw-alias-label-primary);font-size:11px;font-weight:600;line-height:16px}.NSrc7q_presetMetaRow{flex-wrap:wrap;gap:6px;display:flex}.NSrc7q_presetMetaChip{background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 46%, transparent);color:var(--dsw-alias-label-secondary);border-radius:999px;padding:1px 6px;font-size:10px;line-height:15px}.NSrc7q_presetItemDesc{color:var(--dsw-alias-label-tertiary);font-size:10px;line-height:15px}.NSrc7q_presetDiffBadge{color:var(--dsw-alias-label-secondary);flex:none;font-size:10px;line-height:15px}.NSrc7q_presetActions{justify-content:flex-end;gap:8px;display:flex}.NSrc7q_presetActionGhost,.NSrc7q_presetActionPrimary{cursor:pointer;border-radius:6px;padding:3px 10px;font-size:11px;line-height:16px}.NSrc7q_presetActionGhost{color:var(--dsw-alias-label-secondary);background:0 0;border:1px solid #0000}.NSrc7q_presetActionPrimary{border:1px solid var(--dsw-alias-state-business-primary);background:var(--dsw-alias-state-business-primary);color:#fff}.NSrc7q_empty{color:var(--dsw-alias-label-tertiary);margin:0;padding:6px 8px;font-size:11px;line-height:16px}.NSrc7q_viewBtn{color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:7px;flex:none;justify-content:center;align-items:center;padding:6px;display:inline-flex;position:relative}.NSrc7q_viewBtn:hover{color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 10%, transparent)}.NSrc7q_viewBtn:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.NSrc7q_modalMask{z-index:100;background:#00000073;justify-content:center;align-items:center;padding:24px;display:flex;position:fixed;inset:0}.NSrc7q_modal{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-2);border-radius:12px;width:460px;max-width:100%;padding:16px;box-shadow:0 12px 40px #00000080}.NSrc7q_modalHead{align-items:center;gap:8px;margin-bottom:12px;display:flex}.NSrc7q_modalTitle{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:600;line-height:20px}.NSrc7q_modalClose{color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:6px;margin-left:auto;padding:2px 6px;font-size:13px;line-height:16px}.NSrc7q_modalClose:hover{color:var(--dsw-alias-label-primary);background:#ffffff0f}.NSrc7q_roleSlogan{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 16%, transparent);color:var(--dsw-alias-state-business-primary);border-radius:8px;padding:8px 10px;font-size:13px;font-weight:500;line-height:20px}.NSrc7q_modalSectionTitle{color:var(--dsw-alias-label-tertiary);margin:12px 0 6px;font-size:11px;line-height:16px}.NSrc7q_dutyRow{gap:8px;padding:6px 10px;font-size:12px;line-height:20px;display:flex}.NSrc7q_dutyRow+.NSrc7q_dutyRow{border-top:1px dashed var(--dsw-alias-border-l2)}.NSrc7q_dutyStep{color:var(--dsw-alias-label-secondary);flex:none;font-weight:500}.NSrc7q_dutyDesc{color:var(--dsw-alias-label-primary)}.NSrc7q_modalKv{align-items:center;gap:8px;padding:4px 10px;font-size:12px;line-height:20px;display:flex}.NSrc7q_modalK{width:72px;color:var(--dsw-alias-label-tertiary);flex:none}.NSrc7q_modalV{color:var(--dsw-alias-label-secondary);flex:1;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px}.NSrc7q_modalEmpty{color:var(--dsw-alias-label-tertiary);padding:6px 10px;font-size:12px;line-height:20px}.NSrc7q_modalBody{max-height:min(52vh,420px);margin-right:-4px;padding-right:4px;overflow:auto}.NSrc7q_ruleRow{gap:8px;padding:4px 10px;font-size:12px;line-height:20px;display:flex}.NSrc7q_ruleBullet{color:var(--dsw-alias-state-business-primary);flex:none}.NSrc7q_growthStat{color:var(--dsw-alias-label-primary);margin:0 0 8px;padding:0 10px;font-size:12px;line-height:20px}.NSrc7q_growthList{flex-direction:column;gap:6px;margin:0;padding:0;list-style:none;display:flex}.NSrc7q_growthItem{background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 40%, transparent);border-radius:8px;flex-direction:column;gap:2px;padding:8px 10px;display:flex}.NSrc7q_growthPractice{color:var(--dsw-alias-label-primary);font-size:12px;line-height:20px}.NSrc7q_growthMeta{color:var(--dsw-alias-label-tertiary);font-size:10px;line-height:16px}.NSrc7q_roleBox{border:1px solid var(--dsw-alias-border-l2);background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 42%, transparent);border-radius:10px;flex-direction:column;gap:8px;min-width:0;padding:12px;display:flex}.NSrc7q_roleBoxHead{justify-content:space-between;align-items:baseline;gap:12px;display:flex}.NSrc7q_roleBoxTitle{color:var(--dsw-alias-label-primary);font-size:13px;font-weight:650;line-height:20px}.NSrc7q_roleBoxId{color:var(--dsw-alias-label-tertiary);text-align:right;text-overflow:ellipsis;white-space:nowrap;font:10px/16px ui-monospace,SFMono-Regular,Menlo,monospace;overflow:hidden}.NSrc7q_roleBoxDescription{color:var(--dsw-alias-label-secondary);-webkit-line-clamp:2;-webkit-box-orient:vertical;margin:0;font-size:11px;line-height:17px;display:-webkit-box;overflow:hidden}.NSrc7q_roleBoxActions{border-top:1px solid var(--dsw-alias-border-l2);align-items:center;gap:10px;padding-top:6px;display:flex}.NSrc7q_roleDutyLink{color:var(--dsw-alias-state-business-primary);font:inherit;cursor:pointer;background:0 0;border:0;padding:3px 0;font-size:11px}.NSrc7q_roleDutyLink:hover{text-decoration:underline}.NSrc7q_roleBoxActions .NSrc7q_viewBtn{margin-left:auto}.NSrc7q_modelHelp{color:var(--dsw-alias-label-secondary);margin:0;padding:0 10px;font-size:11px;line-height:17px}.NSrc7q_modelPresetReference{border-top:1px solid var(--dsw-alias-border-l2);flex-direction:column;gap:4px;padding-top:8px;display:flex}.NSrc7q_modelProvider{flex-direction:column;gap:4px;display:flex}.NSrc7q_modelProviderTitle{color:var(--dsw-alias-label-primary);align-items:center;gap:8px;margin:4px 0;font-size:12px;display:flex}.NSrc7q_modelRow{background:color-mix(in srgb, var(--dsw-alias-bg-layer-1) 35%, transparent);border-radius:8px;align-items:center;gap:10px;min-height:36px;padding:4px 10px;display:flex}.NSrc7q_modelName{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;flex:1;font:11px/16px ui-monospace,SFMono-Regular,Menlo,monospace;overflow:hidden}.NSrc7q_modelEffort{color:var(--dsw-alias-label-secondary);align-items:center;gap:6px;font-size:10px;display:flex}";
		...
		var ProviderGrantsSection_module_css_default = {
			"card": "NSrc7q_card",
			"dutyDesc": "NSrc7q_dutyDesc",
			"dutyRow": "NSrc7q_dutyRow",
			"dutyStep": "NSrc7q_dutyStep",
			"empty": "NSrc7q_empty",
			"growthItem": "NSrc7q_growthItem",
			"growthList": "NSrc7q_growthList",
			"growthMeta": "NSrc7q_growthMeta",
			"growthPractice": "NSrc7q_growthPractice",
			"growthStat": "NSrc7q_growthStat",
			"head": "NSrc7q_head",
			"list": "NSrc7q_list",
			"modal": "NSrc7q_modal",
			"modalBody": "NSrc7q_modalBody",
			"modalClose": "NSrc7q_modalClose",
			"modalEmpty": "NSrc7q_modalEmpty",
			"modalHead": "NSrc7q_modalHead",
			"modalK": "NSrc7q_modalK",
			"modalKv": "NSrc7q_modalKv",
			"modalMask": "NSrc7q_modalMask",
			"modalSectionTitle": "NSrc7q_modalSectionTitle",
			"modalTitle": "NSrc7q_modalTitle",
			"modalV": "NSrc7q_modalV",
			"modelEffort": "NSrc7q_modelEffort",
			"modelHelp": "NSrc7q_modelHelp",
			"modelName": "NSrc7q_modelName",
			"modelPresetReference": "NSrc7q_modelPresetReference",
			"modelProvider": "NSrc7q_modelProvider",
			"modelProviderTitle": "NSrc7q_modelProviderTitle",
			"modelRow": "NSrc7q_modelRow",
			"name": "NSrc7q_name",
			"nameWrap": "NSrc7q_nameWrap",
			"pill": "NSrc7q_pill",
			"presetActionGhost": "NSrc7q_presetActionGhost",
			"presetActionPrimary": "NSrc7q_presetActionPrimary",
			"presetActions": "NSrc7q_presetActions",
			"presetDiffBadge": "NSrc7q_presetDiffBadge",
			"presetHelp": "NSrc7q_presetHelp",
			"presetItem": "NSrc7q_presetItem",
			"presetItemDesc": "NSrc7q_presetItemDesc",
			"presetItemHead": "NSrc7q_presetItemHead",
			"presetItemName": "NSrc7q_presetItemName",
			"presetList": "NSrc7q_presetList",
			"presetMetaChip": "NSrc7q_presetMetaChip",
			"presetMetaRow": "NSrc7q_presetMetaRow",
			"presetPopover": "NSrc7q_presetPopover",
			"presetTab": "NSrc7q_presetTab",
			"presetTabs": "NSrc7q_presetTabs",
			"presetTitle": "NSrc7q_presetTitle",
			"resetBtn": "NSrc7q_resetBtn",
			"roleBox": "NSrc7q_roleBox",
			"roleBoxActions": "NSrc7q_roleBoxActions",
			"roleBoxDescription": "NSrc7q_roleBoxDescription",
			"roleBoxHead": "NSrc7q_roleBoxHead",
			"roleBoxId": "NSrc7q_roleBoxId",
			"roleBoxTitle": "NSrc7q_roleBoxTitle",
			"roleDutyLink": "NSrc7q_roleDutyLink",
			"roleSlogan": "NSrc7q_roleSlogan",
			"row": "NSrc7q_row",
			"rowSub": "NSrc7q_rowSub",
			"ruleBullet": "NSrc7q_ruleBullet",
			"ruleRow": "NSrc7q_ruleRow",
			"section": "NSrc7q_section",
			"select": "NSrc7q_select",
			"switch": "NSrc7q_switch",
			"switchThumb": "NSrc7q_switchThumb",
			"title": "NSrc7q_title",
			"viewBtn": "NSrc7q_viewBtn"
		};
		//#endregion
		...
		*/
		const PROVIDER_GRANTS_NAMESPACE = "agent-team-web";
		function modelCapabilityValue(value, provider, model) {
		...
		function toggleModelCapability(current, provider, model, enabled, maxReasoningEffort) {
			...
			const requested = maxReasoningEffort === void 0 ? prior.maxReasoningEffort : maxReasoningEffort || void 0;
			const effort = requested !== void 0 && allowed.has(requested) ? requested : void 0;
			capabilities[key] = {
				enabled,
				...effort !== void 0 && allowed.has(effort) ? { maxReasoningEffort: effort } : {}
			};
			return capabilities;
		}
		function modelEnabled(provider, model, value) {
			return modelCapabilityValue(value, provider, model.id).enabled;
		}
		/** 纯函数(t20):实时合并角色档位——显示值 = 实时覆盖(scope snapshot)
		* ?? base(/state 的 profile ?? DEFAULT,不含覆盖);overridden 由实时覆盖
		* 判定(驱动「恢复默认」disabled 态与选中回显)。 */
		const MEMBER_PRESET_ROLES = [
			"researcher",
			"engineer",
			"qa",
			"designer",
		...
		function roleDutyDescription(role, t) {
			return ROLE_DUTY[role]?.slogan ?? t("settings.agentTeam.roleDutyUnavailable");
		}
		function mergeRoleDefaults(base, overrides) {
		...
		function rolePresetTemplatesByGroup(group) {
		...
		async function fetchPractices(workspace) {
			const token = agentTeamsWebToken();
			const url = new URL("/plugins/agent-team-web/practices", window.location.origin);
			if (workspace !== void 0) url.searchParams.set("workspace", workspace);
			const response = await fetch(`${url.pathname}${url.search}`, { headers: token === void 0 ? {} : { [TOKEN_HEADER]: token } });
			if (!response.ok) throw new Error(`http-${response.status}`);
			return await response.json();
		}
		/** Persist before changing UI state; stale/forbidden/error responses never become success. */
		async function submitPracticeMutation(body, token, send = fetch) {
			try {
		...
		function validatePracticePatch(patch) {
			if (patch.practice.trim().length === 0) return "实践内容不能为空";
			if (patch.practice.length > 4e3) return "实践内容不能超过 4000 字";
			if (patch.appliesWhen.length > 2e3 || patch.counterexamples.length > 2e3) return "条件和反例不能超过 2000 字";
		...
		* 中该 provider 的所有 `${provider}/${model}` key)。 */
		function ModelGrantCard({ providers, scope, snapshot, t }) {
			...
			const [presetGroup, setPresetGroup] = (0, react.useState)("ds");
			const [selectedPresetId, setSelectedPresetId] = (0, react.useState)(rolePresetTemplatesByGroup("ds")[0]?.id ?? "");
			const toggle = async (provider, model, enabled, effort) => {
				...
				try {
					...
					setFailure(`${provider}/${model.id} 保存失败；请重试`);
				} finally {
					setBusy(void 0);
				}
			};
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: ProviderGrantsSection_module_css_default.card,
				"aria-label": t("settings.agentTeam.modelSelection"),
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("header", {
						className: ProviderGrantsSection_module_css_default.head,
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: ProviderGrantsSection_module_css_default.title,
							children: t("settings.agentTeam.modelSelection")
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: ProviderGrantsSection_module_css_default.modelHelp,
						children: t("settings.agentTeam.modelSelectionHelp")
					}),
					failure && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						role: "alert",
						className: ProviderGrantsSection_module_css_default.growthError,
						children: failure
					}),
					providers.map((provider) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: ProviderGrantsSection_module_css_default.modelProvider,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("h4", {
							className: ProviderGrantsSection_module_css_default.modelProviderTitle,
							children: [provider.name, /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: ProviderGrantsSection_module_css_default.rowSub,
							...
							const configuredEffort = current.maxReasoningEffort && efforts.some((item) => item.id === current.maxReasoningEffort) ? current.maxReasoningEffort : "";
							const key = modelKeyOf(provider.id, normalized.id);
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: ProviderGrantsSection_module_css_default.modelRow,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: ProviderGrantsSection_module_css_default.modelName,
										children: normalized.id
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										role: "switch",
										"aria-checked": enabled,
										"aria-label": `${provider.id}/${normalized.id} 授权`,
										disabled: busy === key,
										className: ProviderGrantsSection_module_css_default.switch,
										"data-on": enabled,
										onClick: () => {
											toggle(provider.id, normalized, !enabled);
										},
									...
									efforts.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										className: ProviderGrantsSection_module_css_default.modelEffort,
										children: ["最高思考档位", /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
											className: ProviderGrantsSection_module_css_default.select,
											"aria-label": `${provider.id}/${normalized.id} 最高思考档位`,
											value: configuredEffort,
											disabled: !enabled || busy === key || scope === void 0,
											onChange: (event) => {
												toggle(provider.id, normalized, enabled, event.target.value);
											},
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
												value: "",
												children: "模型默认（未显式设置上限）"
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: ProviderGrantsSection_module_css_default.modelPresetReference,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: ProviderGrantsSection_module_css_default.resetBtn,
								"aria-expanded": presetOpen,
								onClick: () => {
									setPresetOpen((open) => !open);
								},
								children: t("settings.agentTeam.modelPresetReference")
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: ProviderGrantsSection_module_css_default.presetHelp,
								children: t("settings.agentTeam.modelPresetReferenceHelp")
							}),
							presetOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: ProviderGrantsSection_module_css_default.presetPopover,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: ProviderGrantsSection_module_css_default.presetTabs,
									children: [
										"ds",
										"gpt",
										"mixed"
									].map((group) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: ProviderGrantsSection_module_css_default.presetTab,
										...
										children: t(group === "ds" ? "settings.agentTeam.presetGroup.ds" : group === "gpt" ? "settings.agentTeam.presetGroup.gpt" : "settings.agentTeam.presetGroup.mixed")
									}, group))
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: ProviderGrantsSection_module_css_default.presetList,
									children: rolePresetTemplatesByGroup(presetGroup).map((template) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										className: ProviderGrantsSection_module_css_default.presetItem,
										"data-active": selectedPresetId === template.id,
										children: [
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
												className: ProviderGrantsSection_module_css_default.presetItemHead,
												children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: ProviderGrantsSection_module_css_default.presetItemName,
													children: template.label
												}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
													className: ProviderGrantsSection_module_css_default.presetDiffBadge,
													children: t("settings.agentTeam.modelPresetReadOnly")
												})]
											}),
											/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
												className: ProviderGrantsSection_module_css_default.presetMetaRow,
												children: [
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
														className: ProviderGrantsSection_module_css_default.presetMetaChip,
														children: `成本：${template.cost}`
													}),
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
														className: ProviderGrantsSection_module_css_default.presetMetaChip,
														children: `速度：${template.speed}`
													}),
													/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
														className: ProviderGrantsSection_module_css_default.presetMetaChip,
														children: `质量：${template.quality}`
													})
												]
											}),
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: ProviderGrantsSection_module_css_default.presetItemDesc,
												children: template.description
											}),
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: ProviderGrantsSection_module_css_default.presetItemDesc,
												children: Object.entries(template.roleDefaults).map(([role, route]) => `${role}: ${route.provider ?? ""}/${route.model ?? ""}${route.reasoningEffort ? ` · ${route.reasoningEffort}` : ""}`).join("；")
											})
										]
									}, template.id))
								})]
		...
		function RolePresetCard({ rows, t }) {
			...
			const viewedDuty = viewedRole === void 0 ? void 0 : ROLE_DUTY[viewedRole];
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: ProviderGrantsSection_module_css_default.card,
				"aria-label": t("settings.agentTeam.rolePreset"),
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("header", {
						className: ProviderGrantsSection_module_css_default.head,
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: ProviderGrantsSection_module_css_default.title,
							children: t("settings.agentTeam.rolePreset")
						})
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
						className: ProviderGrantsSection_module_css_default.list,
						children: rows.map((row) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
							className: ProviderGrantsSection_module_css_default.roleBox,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
									className: ProviderGrantsSection_module_css_default.roleBoxHead,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: ProviderGrantsSection_module_css_default.roleBoxTitle,
										children: roleTitle(row.role, t)
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: ProviderGrantsSection_module_css_default.roleBoxId,
										children: row.role
									})]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									className: ProviderGrantsSection_module_css_default.roleBoxDescription,
									children: roleDutyDescription(row.role, t)
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", {
									className: ProviderGrantsSection_module_css_default.roleBoxActions,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: ProviderGrantsSection_module_css_default.roleDutyLink,
										onClick: () => {
											setViewing(row);
										},
										children: t("settings.agentTeam.roleDutyLink")
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: ProviderGrantsSection_module_css_default.viewBtn,
					...
					viewing !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: ProviderGrantsSection_module_css_default.modalMask,
						role: "presentation",
						onClick: () => {
							setViewing(void 0);
						},
						children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: ProviderGrantsSection_module_css_default.modal,
							role: "dialog",
							"aria-modal": "true",
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
									className: ProviderGrantsSection_module_css_default.modalHead,
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: ProviderGrantsSection_module_css_default.modalTitle,
											children: roleTitle(viewing.role, t)
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											className: ProviderGrantsSection_module_css_default.rowSub,
											children: viewing.role
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											type: "button",
											className: ProviderGrantsSection_module_css_default.modalClose,
								...
								viewedDuty === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									className: ProviderGrantsSection_module_css_default.modalEmpty,
									children: t("settings.agentTeam.viewEmpty")
								}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									className: ProviderGrantsSection_module_css_default.modalBody,
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
											className: ProviderGrantsSection_module_css_default.roleSlogan,
											children: viewedDuty.slogan
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
											className: ProviderGrantsSection_module_css_default.modalSectionTitle,
											children: t("settings.agentTeam.viewSteps")
										}),
										viewedDuty.steps.map((step, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: ProviderGrantsSection_module_css_default.dutyRow,
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: ProviderGrantsSection_module_css_default.dutyStep,
												children: `${index + 1}. ${step.title}`
											}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: ProviderGrantsSection_module_css_default.dutyDesc,
												children: step.desc
											})]
										}, step.title)),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
											className: ProviderGrantsSection_module_css_default.modalSectionTitle,
											children: t("settings.agentTeam.viewDeliverable")
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
											className: ProviderGrantsSection_module_css_default.dutyDesc,
											style: { padding: "0 10px" },
											children: viewedDuty.deliverable
										}),
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
											className: ProviderGrantsSection_module_css_default.modalSectionTitle,
											children: t("settings.agentTeam.viewRules")
										}),
										viewedDuty.rules.map((rule) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
											className: ProviderGrantsSection_module_css_default.ruleRow,
											children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: ProviderGrantsSection_module_css_default.ruleBullet,
												children: "•"
											}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
												className: ProviderGrantsSection_module_css_default.dutyDesc,
												children: rule
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									...
									children: "职责预设不设置新成员模型；新成员默认继承队长路由。"
								})
							]
						})
					})
		...
		function GrowthCard({ t }) {
			...
			const load = async (path) => {
				...
				try {
					...
					if (path === void 0) setAvailable(false);
					setError(cause instanceof Error && (cause.message === "http-401" || cause.message === "http-403") ? "经验治理不可用：请确认当前浏览器已通过 DSH operator 登录" : "加载经验失败，请重试");
				} finally {
					setLoading(false);
				}
			};
			...
			const beginEdit = (entry) => {
				...
				setDraft({
					...
					appliesWhen: (entry.appliesWhen ?? []).join("\n"),
					counterexamples: (entry.counterexamples ?? []).map((item) => `${item.context} | ${item.reason}`).join("\n"),
					expiresAt: entry.expiresAt == null ? "" : new Date(entry.expiresAt).toISOString().slice(0, 10),
					reason: ""
				});
			};
			const mutate = async (action) => {
				...
				try {
					const token = agentTeamsWebToken();
					const response = await submitPracticeMutation({
						workspace,
						id: selected.id,
						...
						...action === "edit" ? { patch: {
							...
							counterexamples: draft.counterexamples.split("\n").map((value) => value.trim()).filter(Boolean).map((value) => {
								...
								return {
									context: value.slice(0, split).trim(),
									reason: value.slice(split + 1).trim()
								};
							}),
							expiresAt: draft.expiresAt ? (/* @__PURE__ */ new Date(`${draft.expiresAt}T00:00:00`)).getTime() : null
						} } : {},
					...
					if (response.kind === "forbidden") {
						setError("无权修改此工作区的经验");
						return;
					}
					if (response.kind === "failed") {
						setError(`保存失败${response.status === void 0 ? "（网络或服务错误）" : `（HTTP ${response.status}）`}，内容未更改`);
						return;
					}
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
				className: ProviderGrantsSection_module_css_default.card,
				"aria-label": t("settings.agentTeam.growth"),
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
						className: ProviderGrantsSection_module_css_default.head,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: ProviderGrantsSection_module_css_default.title,
							children: t("settings.agentTeam.growth")
						}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: ProviderGrantsSection_module_css_default.resetBtn,
							onClick: () => {
								setExpanded((value) => !value);
								if (!expanded && workspaces.length === 0) load();
							},
							children: expanded ? "收起" : "查看全部"
						})]
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
						className: ProviderGrantsSection_module_css_default.growthMeta,
						children: "单机单 operator 模式：已登录本机 DSH 的操作者可管理所有已注册工作区；变更仅影响后续新成员。"
					}),
					expanded && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: ProviderGrantsSection_module_css_default.growthControls,
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
								className: ProviderGrantsSection_module_css_default.growthMeta,
								children: ["工作区", /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
									className: ProviderGrantsSection_module_css_default.select,
									value: workspace,
							...
							loading && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: ProviderGrantsSection_module_css_default.empty,
								children: "正在加载…"
							}),
							...
							notice && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								role: "status",
								className: ProviderGrantsSection_module_css_default.growthNotice,
								children: notice
							}),
							workspace && !loading && entries.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
								className: ProviderGrantsSection_module_css_default.empty,
								children: "当前工作区暂无经验。"
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", {
								className: ProviderGrantsSection_module_css_default.growthList,
								children: entries.map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", {
									className: ProviderGrantsSection_module_css_default.growthItem,
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: ProviderGrantsSection_module_css_default.growthPractice,
										onClick: () => {
											setSelected(entry);
											setEditing(false);
											setDraft((d) => ({
										...
										children: entry.practice
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										className: ProviderGrantsSection_module_css_default.growthMeta,
										children: `${entry.sourceTeamId} · ${entry.role} · ${entry.sourceTaskSubject} · ${entry.verdict}${entry.disabledAt ? " · 已撤销" : ""}`
									})]
								}, entry.id))
							})
						]
					...
					selected && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: ProviderGrantsSection_module_css_default.growthDetail,
						role: "dialog",
						"aria-label": "经验详情",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: ProviderGrantsSection_module_css_default.growthDetailHead,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: "经验详情" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									onClick: () => {
										setSelected(void 0);
										setEditing(false);
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "实践：" }), selected.practice] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "来源：" }),
								selected.sourceTeamId,
								" · ",
								selected.sourceTaskSubject,
								" · ",
								selected.role
							] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "适用条件：" }), (selected.appliesWhen ?? []).join("；") || "未填写"] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "反例：" }), (selected.counterexamples ?? []).map((value) => `${value.context}：${value.reason}`).join("；") || "未填写"] }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("p", { children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: "状态：" }),
								selected.disabledAt ? `已撤销：${selected.disabledReason ?? ""}` : selected.verdict,
								" · revision ",
								selected.revision
							] }),
							!editing ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: ProviderGrantsSection_module_css_default.growthActions,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["操作原因", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
										value: draft.reason,
									...
									selected.disabledAt ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										disabled: busy || !workspace,
										onClick: () => {
											if (window.confirm("恢复该经验？恢复后仍需重新审核，且仅影响后续成员。")) mutate("restore");
										},
										children: "恢复"
									}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										disabled: busy || !workspace,
										onClick: () => {
											if (window.confirm("撤销此经验？将停止注入后续新成员。")) mutate("disable");
										},
										children: "撤销"
									})
								]
							}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: ProviderGrantsSection_module_css_default.growthEditor,
								children: [
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["实践", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["适用条件", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["反例（每行一条）", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
										value: draft.counterexamples,
										onChange: (event) => setDraft((d) => ({
											...d,
											counterexamples: event.target.value
										}))
									})] }),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["过期日期", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
										type: "date",
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: ["修改原因", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("textarea", {
										value: draft.reason,
										onChange: (event) => setDraft((d) => ({
											...d,
											reason: event.target.value
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											disabled: busy,
											onClick: () => {
												if (window.confirm("提交修改？修改将重新进入待审核状态。")) mutate("edit");
											},
											children: busy ? "提交中…" : "保存修改"
										})]
									})
								]
							}),
		...
		*/
		function ProviderGrantsSection(props) {
			...
			const [center, setCenter] = (0, react.useState)({
				providers: [],
				roleDefaultsBase: {},
				selfGrowth: {
					total: 0,
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [center.providers.length === 0 && roleRows.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: ProviderGrantsSection_module_css_default.empty,
					children: t(loading ? "settings.agentTeam.loading" : "settings.agentTeam.empty")
				}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(RolePresetCard, {
					rows: roleRows,
					t
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ModelGrantCard, {
					providers: center.providers,
					scope,
					snapshot,
					t
				})] }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(GrowthCard, { t })]
			});
		}
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			const openMember = async (parentId, childId) => {
				try {
			...
			const agentTeamSettingsScope = ctx.configForms.get(PROVIDER_GRANTS_NAMESPACE);
