 ...
 */
export type TaskRetroCause = 'underestimated' | 'dependency-blocked' | 'requirement-change' | 'member-efficiency' | 'environment' | 'on_time' | 'other';
/** 全部允许的复盘原因(工具枚举与校验共用)。 */
export declare const TASK_RETRO_CAUSES: readonly TaskRetroCause[];
/**
 * 预估工作量等级(对外口径,方向决策 2)。
...
export declare const ESTIMATE_LEVEL_RANGES: Readonly<Record<EstimateLevel, {
    readonly maxMinutes: number;
    readonly maxMs: number;
    readonly label: string;
}>>;
/** 等级的最大参考毫秒(超时判定基准)。 */
export declare function estimateLevelMaxMs(level: EstimateLevel): number;
/** 实际耗时落入的等级(S/M/L)。 */
export declare function estimateLevelOf(actualMs: number): EstimateLevel;
/** 等级索引:0=S / 1=M / 2=L。 */
export declare function estimateLevelIndex(level: EstimateLevel): number;
/**
 * 等级偏差:实际等级索引 − 预估等级索引(-1/0/+1 等)。
 * 如 S 预估 → 实际 M = +1 级;L 预估 → 实际 S = -2 级。
 */
export declare function estimateLevelDeviation(actualMs: number, estimatedLevel: EstimateLevel): number;
/**
