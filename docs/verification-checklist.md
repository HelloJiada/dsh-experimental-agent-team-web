# 真实 Profile 端到端验证清单（本 bundle 联调）

本文档把「在真实 DSH Profile 中安装本 bundle 做端到端验证」整理成可执行的步骤与预期对照。在没有真实 Profile 的环境里，可用仓库内的测试套件代替（见下文「无真实 Profile 时的替代验证」）。如果你只想先做 3–5 分钟的最小确认，先看 [real-profile-smoke-check.md](real-profile-smoke-check.md)。
...
## 6. 无真实 Profile 时的替代验证

仓库内置测试套件覆盖 profile patch 骨架、文档引用与包布局：
