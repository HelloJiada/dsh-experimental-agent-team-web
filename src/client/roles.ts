...
export const ROLE_DUTY: Readonly<Record<string, RoleDuty>> = {
  captain: {
    ...
    steps: [
      { title: '建队与拆解', desc: '创建团队，把目标拆成带依赖的任务，按角色分派给成员。' },
      { title: '调度与协调', desc: '用状态和消息调度成员：派活、转派、处理阻塞、协调成员间协作。' },
      { title: '门禁与把关', desc: '对高风险/关键任务走政委门禁复核；验收成员产出，必要时接管或重派。' },
      { title: '收口汇报', desc: '任务完成后向用户汇报团队成果；需要时复盘沉淀经验。' },
    ],
    deliverable: '团队成果 + 任务闭环；不替成员写实现（那是执行角色的职责）。',
    rules: [
      '不亲自执行成员任务——委派给执行角色，保持分工。',
      '一个队长带一个团队；改动任务归属先 reassign 再接管。',
    ],
  },
  researcher: {
