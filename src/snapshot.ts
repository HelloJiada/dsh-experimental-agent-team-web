...
export async function collectProviders(
  ...
  for (const provider of providers) {
    ...
    const grantedAny = models.some(model => enabledMap[`${provider.id}/${model.id}`] === true)
...
export async function assembleTeamSnapshot(
  ...
  const mapMember = (member: TeamState['members'][number]): TeamActivityMember => {
    ...
    return {
      ...
      // t7:透出成员落盘 LLM 路由(完整值,面板小字标注;旧数据缺省不输出)。
      ...member.provider !== undefined && member.provider !== '' ? { provider: member.provider } : {},
      ...member.model !== undefined && member.model !== '' ? { model: member.model } : {},
      ...member.reasoningEffort !== undefined && member.reasoningEffort !== ''
